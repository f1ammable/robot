 ...
 *
 * <p>Waypoint extends Point, as a way point can just be a point.
 *
 * <p>However, a Waypoint can optionally specify a heading that the robot must achieve when it
 * reaches the way points.
 *
 * <p>It can also optionally specify how close the robot must get to the way point in order for it
 * to be deemed to have reached it.
 */
public class Waypoint extends Point implements Transmittable {
  ...
  public boolean checkValidity(Pose p) {
    if (maxPositionError >= 0 && p.distanceTo(this) > maxPositionError) return false;
    if (headingRequired
        && maxHeadingError >= 0
