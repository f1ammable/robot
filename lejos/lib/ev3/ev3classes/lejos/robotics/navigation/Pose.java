 ...
 * This class includes methods for updating the Pose to in response to basic robot movements. It
 * also contains utility methods for use in navigation, such as the direction and distance to a
 * point from the location of the pose, and also the location of a point at a given distance and
 * direction from the location of the pose.<br>
 * All directions and angles are in degrees and use the standard convention in mathematics:
 * direction 0 is parallel to the X axis, and direction +90 is parallel to the Y axis. <br>
 *
...
public class Pose implements Transmittable {
  ...
  /**
   * Sets the pose location and heading to the correct values resulting from travel in a circular
   * arc. The radius is calculated from the distance and turn angle
   *
  ...
  /**
   * Returns the point at <code> distance </code> from the location of this pose, in the direction
   * <code>bearing</code> relative to the X axis.
   *
