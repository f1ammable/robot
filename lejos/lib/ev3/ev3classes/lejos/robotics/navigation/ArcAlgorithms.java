...
/**
 * The static methods in this class can be used to to calculate theoretical routes and for
 * displaying graphical representations of the path of a robot. The methods getAvailablePaths() and
 * getBestPath() are useful for this.
 *
 * @author bb
...
public class ArcAlgorithms { // TODO Change access from public to package level when done testing?
  ...
  /**
   * Find the shortest path for a steering vehicle between two points. The start Pose and
   * destination Pose include the heading, so the robot will execute an arc, then travel a straight
   * line, followed by another arc to obtain the final heading. This algorithm searches through 16
   * combinations of paths in order to find the shortest path. Even though the radii supplied are
   * positive (left turn), this algorithm searches through negative radii too (right turns)
   *
   ...
   */
  public static Move[] getBestPath(
      Pose start, float turnRadius1, Pose destination, float turnRadius2) {
    // Get all paths TODO: This can probably be streamlined with arrays. Sort out Path (Move)
    // container first.
  ...
  /**
   * This method gets all the available paths given a start Pose and destination Post. Only four
   * possible paths will be generated. The two turn radii can be positive (left hand turn) or
   * negative (right hand turn). Each path consists of an arc, then a straight line, followed by
   * another arc to obtain the final heading.
   *
   * @param start The starting Pose
   * @param turnRadius1 The turning radius for the first arc
   * @param destination The destination Pose
   ...
   * @return the sequence of moves
   */
  public static Move[][] getAvailablePaths(
      Pose start, float turnRadius1, Pose destination, float turnRadius2) {

    // TODO: These constants can perhaps be calculated based on existing parameters?
    final int PATHS =
        4; // Currently doesn't calculate Pilot.backward() movement along P2 to P3. Would make this
           // 8.
    ...
    // Draw start circle:
    Point startCircle =
        ArcAlgorithms.findCircleCenter(start.getLocation(), turnRadius1, start.getHeading());

    // Draw target circle:
    Point targetCircle =
        ArcAlgorithms.findCircleCenter(
            destination.getLocation(), turnRadius2, destination.getHeading());
    ...
    if (innerRadius == 0) {
      ...
      // To find arcLength, need to make new p1 that sits on inner circle. NOTE: turnRadius1 was a
      // bug. Now turnRadius2.
      Point p1inner =
          ArcAlgorithms.findPointOnHeading(
              start.getLocation(), start.getHeading() + 90, turnRadius2);
    ...
    Point p2 =
        ArcAlgorithms.findPointOnHeading(
            p2inner, newHeading - 90, turnRadius2); // TODO: turnRadius2? Not turnRadius1?
    Point p3 =
        ArcAlgorithms.findPointOnHeading(
            targetCircle, newHeading - 90, turnRadius2); // TODO: turnRadius2? Not turnRadius1?
    ...
    float startArcF =
        ArcAlgorithms.getArc(start.getLocation(), p2, turnRadius1, start.getHeading(), true);
    float startArcB = ArcAlgorithms.getArcBackward(startArcF);

    // Find arc lengths (forward and backward) to drive on targetCircle: TODO: Swap p3 and
    // destination & remove -ve?
    float targetArcF =
        -ArcAlgorithms.getArc(
            destination.getLocation(), p3, turnRadius2, destination.getHeading(), false);
    float targetArcB =
        ArcAlgorithms.getArcBackward(targetArcF); // Prefer this for speed. It is exact.
  ...
  /**
   * This method calculates the moves needed to drive from a starting Pose to a final Point. The
   * final heading is indeterminate at the destination. To arrive at the destination, the robot
   * drives an arc, then a straight line to the destination point. If the destination point is
   * within the turning circle, the moves generated by that circle will all have Float.NaN for the
   * distanceTraveled and arcAngle values.
   *
   * @param start The starting Pose
  ...
  public static Move[][] getAvailablePaths(Pose start, Point destination, float turnRadius) {
    ...
    // the Point destination below should really return float, not double. Not sure why Laurie
    // returns a double.
    Point p3 = new Point((float) destination.getX(), (float) destination.getY());

    for (int i = 0; i < PATHS; i++) {
      float radius = turnRadius;
      // TODO: This method does too much. It should only get the paths with the sign of the radius
      // parameter.
      // getBestPath needs to call it twice in order to get the negative version, then amalgamate
      // the arrays.
  ...
  /**
   * This method generates the shortest path from a starting Pose to a final Point. The final
   * heading is indeterminate at the destination. To arrive at the destination, the robot drives an
   * arc, then a straight line to the destination point.
   *
   * @param start The starting Pose
   * @param destination The destination Point
  ...
  /**
   * Given a starting point and heading, this method will calculate another Point that is distance
  ...
  public static Point findPointOnHeading(Point original, float heading, float distance) {
    ...
    // TODO: Do calculation to set theta angle according to "quadrant" of destination point?
    // Probably not needed.
  ...
  /**
   * This method calculates the angle generated by three points. The point pa is the center point in
   * the angle, while p1 and p2 are outside the angle.
   *
  ...
  public static float getTriangleAngle(Point p1, Point p2, Point pa) {
    ...
    // TODO: At this point if angle is outside -1 to +1 then Math.acos() causes NaN. I think
    // artifact decimal numbers
   ...
   * directly at the target point, this method calculates the angle to travel along the circle (an
   * arc) to get from p1 to p2.
   *
  ...
  public static float getArc(Point p1, Point p2, float radius, float heading, boolean forward) {
    // I accidently got the radius sign confused. +ve radius is supposed to have circle center to
    // left of robot:
    ...
    if (!forward) {
      ...
      if (arcLength < 0) arcLength = arcLength + 360;
      else arcLength = arcLength - 360;
    }
    return arcLength;
  }
  ...
  public static float getArcBackward(float forwardArc) {
    ...
    if (forwardArc < 0) backwardArc = 360 + forwardArc;
    else if (forwardArc > 0) backwardArc = -360 + forwardArc;
   ...
   * @return Length of travel along circle, in degrees
   * @deprecated This method is no longer used because it can't calculate >180 angles. Delete any
   *     time.
   */
  @Deprecated
  public static double getArcOld(Point p1, Point p2, double radius) {
    ...
    // This equation can't generate angles >180 (the major angle), so if angle is actually >180 it
    // will
  ...
  /**
   * Assume p2 is the "takeoff" point and p3 is the final target point, this private helper method
   * is used to calculate the distance between p2 to p3, given only the radius and an intermediate
   * value z. The methods findP2() and getAvailablePaths() both use this method.
   *
   ...
   * It can calculate a circle to the right OR left of the heading tangent. To calculate a circle on
   * the left side of the heading tangent, feed it a negative radius.
   *
   * @param p1 the starting point of the vehicle.
   * @param radius Turning radius of vehicle. A negative value produces a circle to the right of the
   *     heading.
