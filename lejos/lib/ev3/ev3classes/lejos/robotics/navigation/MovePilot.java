...
/**
 * The Pilot class is a software abstraction of the Pilot mechanism of a robot. It contains methods
 * to control robot movements: travel forward or backward in a straight line or a circular path or
 * rotate to a new direction.<br>
 * This class will work with any chassis. Some types of chassis might not support all the movements
 * this pilot support. An object of this class assumes that it has exclusive control of its motors.
 * If any other object makes calls to its motors, the results are unpredictable. <br>
...
public class MovePilot implements ArcRotateMoveController {
  ...
  /**
   * Allocates a Pilot object, and sets the physical parameters of the robot.<br>
  ...
  public MovePilot(
    ...
    this(
        new WheeledChassis(
            new Wheel[] {
              WheeledChassis.modelWheel(leftMotor, leftWheelDiameter)
                  .offset(trackWidth / 2)
                  .invert(reverse),
              WheeledChassis.modelWheel(rightMotor, rightWheelDiameter)
                  .offset(-trackWidth / 2)
                  .invert(reverse)
            },
  ...
  /**
   * The monitor class detects end-of-move situations when non blocking move call were made and
   * makes sure these are dealt with.
   */
  private class Monitor extends Thread {
    ...
    public synchronized void run() {
      while (more) {
        if (_moveActive) {
          if (chassis.isStalled()) MovePilot.this.stop();
