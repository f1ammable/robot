...
/**
 * Use the OmniPilot class to control holonomic vehicles with three omnidirectional wheels that
 * allow the robot to move in any direction without changing heading. The robot can also spin while
 * driving straight, and perform any kind of maneuvre the other steering and differential drive
 * vehicles can do. The odometry is computed by this class directly. For the class to work properly,
 * take care to design the robot symmetrically, so that the three wheel axes meet in the center of
 * the robot.
 *
...
public class OmniPilot implements ArcRotateMoveController, RegulatedMotorListener {
  ...
  private Pose pose =
      new Pose(); // TODO: Technically this variable should be removed. Navigator handles Pose.
  ...
  private double minTurnRadius =
      0; // This vehicle can turn withgout moving therefore minimum turn radius = 0
  ...
  /**
   * Instantiates a new omnidirectional pilot. This class also keeps track of the odometry Express
   * the distances in the units you prefer (mm, in, cm ...)
   *
  ...
  /**
   * Instantiates a new omnidirectional pilot. This class also keeps track of the odometry Express
   * the distances in the units you prefer (mm, in, cm ...) This constructor allows you to add a
   * cruizcore gyro for accurate odometry and spinning
   *
   * @param wheelDistanceFromCenter the wheel distance from center
   * @param wheelDiameter the wheel diameter
   * @param centralMotor the central motor
  ...
  */
  ...
  private synchronized void startMotors() {
    if (motor1Speed > 0) motor1.forward();
    else motor1.backward();
    if (motor2Speed > 0) motor2.forward();
    else motor2.backward();
    if (motor3Speed > 0) motor3.forward();
  ...
  /**
   * This method causes the robot to move in a direction while keeping the front of the robot
   * pointed in the current direction it is facing.
   *
   * @param linSpeed the lin speed
  ...
  /**
   * Causes the robot to spin while moving along a linear path. This method is similar to {@link
   * OmniPilot#moveStraight(float, int)} except the robot will spin instead of holding the robot in
  ...
  /**
   * Sets the move direction. This value is then used by subsequent calls to {@link
   * OmniPilot#steer(float)} (all three overloaded methods).
   *
  ...
  public double getMaxLinearSpeed() {
    ...
    return (float)
        Math.sqrt(
            robotSpeeds.get(0, 0) * robotSpeeds.get(0, 0)
   ...
   */
  private void move(
      final double distance, double direction, final double angle, boolean immediateReturn) {
    ...
    spinningMode = false;
    double[] dsp = {
      distance * Math.cos(Math.toRadians(direction)),
      distance * Math.sin(Math.toRadians(direction)),
  ...
  /**
   * This method causes the robot to travel in a linear path, similar to other travel() methods,
   * except you can specify which direction to move (relative to the current robot heading).
   *
   * <p><b>NOTE: This method is not part of the MoveController interface.</b>
   *
   * @param distance
   * @param direction
   */
  ...
  private float getTravelDistance() {
    ...
    float d =
        (float)
            Math.sqrt(
                distances.get(0, 0) * distances.get(0, 0)
                    + distances.get(1, 0)
                        * distances.get(1, 0)); // TODO is this correct for omni odometry?
    //		LCD.drawString(t1+" "+t2+" "+t3+"      ", 0, 5);
  ...
  /**
   * This method moves the robot in an arc, similar to the other {@link OmniPilot#travelArc(double,
   * double)} methods, except you can choose any of the 360 degree directions relative to the
   * current heading (0) of the robot, while keeping the heading of the robot pointed in the same
   * direction during the move.
   *
   * <p><b>NOTE: This method is not part of the MoveController interface.</b>
   *
   * @param radius
  ...
  /**
   * This method moves the robot in an arc, similar to the other {@link OmniPilot#arc(double,
   * double)} methods, except you can choose any of the 360 degree directions relative to the
   * current heading (0) of the robot, while keeping the heading of the robot pointed in the same
   * direction during the move.
   *
   ...
   * @deprecated in 0.8, use setRotateSpeed() and setTravelSpeed(). The method was deprecated, as
   *     this it requires knowledge of the robots physical construction, which this interface should
   *     hide!
   */
  @Deprecated
  ...
  private class Odometer extends Thread {
    ...
    private void updatePose() {
      ...
      int t3 = motor3.getTachoCount();
      double[] angles = {
        Math.toRadians(t1 - t1old), Math.toRadians(t2 - t2old), Math.toRadians(t3 - t3old)
      };
  ...
  public Move getMovement() {
    return new Move(
        previousMoveType,
        getTravelDistance() - previousDistance,
        getAngle() - previousAngle,
        isMoving());
  }

  public void rotationStarted(
      RegulatedMotor motor, int tachoCount, boolean stalled, long timeStamp) {
    ...
    if (!motor1.isMoving() && !motor2.isMoving() && !motor3.isMoving()) {
      ...
      Move finalMove =
          new Move(
              ...
              isMoving());
      for (MoveListener ml : listeners) ml.moveStopped(finalMove, this);
