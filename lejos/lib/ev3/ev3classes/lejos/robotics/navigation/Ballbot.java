...
/**
 * This class dynamically stabilizes a ballbot type of robot. The ballbot robot uses two motors to
 * drive a ball with a similar configuration to a mechanical mouse.
 *
 * <p>A ballbot needs good motors, sensors, a grippy ball/rollers (wheels) and should be as
 * symmetrical as possible in all directions (like a circle) to optimize stability. If using a
 * billiard ball or LEGO ball, it is recommended to apply a layer of rubber glue to the tires to add
 * grip.
 *
 * <p>To start the robot balancing:
 * <li>1. Run the program. You will be prompted to lay it down.
 * <li>2. Lay it down (orientation doesn't matter). When it detects it is not moving it will
 *     automatically calibrate the gyro sensors.
 * <li>3. When the beeping begins, stand it up on the ball so it is vertically balanced.
 * <li>4. When the beeping stops, let go and it will begin balancing on its own.
 *
 *     <p><i>This code is based on the Segoway class.
 *
 * @author BB
 */
public class Ballbot
    extends Thread { // TODO: Thread should be a private inner class, especially given that Ballbot
                     // constructor creates two instances of Ballbot threads.
  ...
  /**
   * Loop wait time. WAIT_TIME is the time in ms passed to the Wait command. NOTE: Balance control
   * loop only takes 1.128 MS in leJOS NXJ to execute.
   */
  ...
  /**
   * This constant aids in drive control. When the robot starts moving because of user control, this
   * constant helps get the robot leaning in the right direction. Similarly, it helps bring robot to
   * a stop when stopping.
   */
  private static final double KDRIVE = -0.02;

  /**
   * Power differential used for steering based on difference of target steering and actual motor
   * difference.
   */
  private static final double KSTEER = 0.25;

  /**
   * Gyro offset control The gyro sensor will drift with time. This constant is used in a simple
   * long term averaging to adjust for this drift. Every time through the loop, the current gyro
   * sensor value is averaged into the gyro offset weighted according to this constant.
   */
  private static final double EMAOFFSET = 0.0005;

  /**
   * If robot power is saturated (over +/- 100) for over this time limit then robot must have
   * fallen. In milliseconds.
   */
  ...
  /**
   * This constant is in degrees/second for maximum speed. Note that position and speed are measured
   * as the sum of the two motors, in other words, 600 would actually be 300 degrees/second for each
   * motor.
   */
  ...
  /** motorControlDrive is the target speed for the sum of the two motors in degrees per second. */
  private double motorControlDrive = 0.0;

  /** motorControlSteer is the target change in difference for two motors in degrees per second. */
  private double motorControlSteer = 0.0;

  /**
   * This global contains the target motor differential, essentially, which way the robot should be
   * pointing. This value is updated every time through the balance loop based on motorControlSteer.
   */
  ...
  /**
   * ratioWheel stores the relative wheel size compared to a standard NXT 1.0 wheel. RCX 2.0 wheel
   * has ratio of 0.7 while large RCX wheel is 1.4.
   */
  ...
  private Ballbot(
      EncoderMotor motor,
      Gyroscope gyro,
      double wheelDiameter) { // TODO: Wheel diam will be unnecessary?
    this.my_motor = motor;

    this.gyro = gyro;
    this.ratioWheel =
        wheelDiameter / 5.6; // Original algorithm was tuned for 5.6 cm NXT 1.0 wheels.
  ...
  /**
   * This function returns a suitable initial gyro offset. It takes 100 gyro samples over a time of
   * 1/2 second and averages them to get the offset. It also check the max and min during that time
  ...
  /**
   * Get the data from the gyro. Fills the pass by reference gyroSpeed and gyroAngle based on
   * updated information from the Gyro Sensor. Maintains an automatically adjusted gyro offset as
   * well as the integrated gyro angle.
   */
  ...
  /**
   * Global variables used to control the amount of power to apply to each wheel. Updated by the
   * steerControl() method.
   */
  private int powerLeft, powerRight; // originally local variables

  /**
   * This function determines the left and right motor power that should be used based on the
   * balance power and the steering control.
   */
  ...
  /**
   * Calculate the interval time from one iteration of the loop to the next. Note that first time
   * through, cLoop is 0, and has not gone through the body of the loop yet. Use it to save the
   * start time. After the first iteration, take the average time and convert it to seconds for use
   * as interval time.
   */
  ...
  public void run() {
    ...
    while (true) {
      ...
      power =
          (int)
              ((KGYROSPEED * gyroSpeed
                          + // Deg/Sec from Gyro sensor
                          KGYROANGLE * gyroAngle)
                      / ratioWheel
                  + // Deg from integral of gyro
                  KPOS * motorPos
                  + // From MotorRotaionCount of both motors
                  KDRIVE * motorControlDrive
                  + // To improve start/stop performance
                  KSPEED * motorSpeed); // Motor speed in Deg/Sec

      if (Math.abs(power) < 100) tMotorPosOK = System.currentTimeMillis();

      steerControl(power); // Movement control. Not used for balancing.

      // Apply the power values to the motors
      // NOTE: It would be easier/faster to use MotorPort.controlMotorById(), but it needs to be
      // public.
  ...
  /**
   * This method does not actually apply direct power to the wheels. Control is filtered through to
   * each wheel, allowing the robot to drive forward/backward and make turns. Higher values are
   * faster. Negative values cause the wheel to rotate backwards. Values between -200 and 200 are
   * good. If values are too high it can make the robot balance unstable.
   *
  ...
  /**
   * Causes movement along either the xaxis or y axis. Normally power for each of these values is
   * zero in order to keep the ballbot roughly stationary.
   *
   * <p>This method does not actually apply direct power to the roller wheels. Control is filtered
   * through to each wheel, allowing the robot to move. Higher values are faster. Negative values
   * cause movement in the opposite direction. Values between -200 and 200 are acceptable. If values
   * are too high it can make the robot balance unstable. Try starting with values around 10 or so.
   * A ballbot needs good motors, sensors, a grippy ball/roller and be extremely symmetrical if you
   * want to use higher values.
   *
   * @param x_axisPower
   * @param y_axisPower
   */
