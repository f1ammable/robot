...
/**
 * This class controls a robot to traverse a Path, a sequence of {@link
 * lejos.robotics.navigation.Waypoint}s. It's default mode for a new path is continuous movement (no
 * stopping at waypoints) but see also {@link #singleStep(boolean)}. To interrupt the path
 * traversal, call stop(). It uses an inner class running its own thread to issue movement commands
 * to its {@link lejos.robotics.navigation.MoveController}, which can be either a {@link
 * lejos.robotics.navigation.DifferentialPilot} or {@link lejos.robotics.navigation.SteeringPilot}.
 * It also uses a {@link lejos.robotics.localization.PoseProvider} Calls its {@link
 * lejos.robotics.navigation.NavigationListener}s when a Waypoint is reached or the robot stops.
...
public class Navigator implements WaypointListener {
  ...
  public Navigator(MoveController pilot, PoseProvider poseProvider) {
    ...
    _radius =
        (_pilot instanceof ArcMoveController ? ((ArcMoveController) _pilot).getMinRadius() : 0);
    _nav = new Nav();
    _nav.setDaemon(true);
    _nav.start();
  }
  ...
  /**
   * Sets the path that the Navigator will traverse. By default, the robot will not stop along the
   * way. If the robot is moving when this method is called, it stops and the current path is
  ...
  /**
   * Controls whether the robot stops at each Waypoint; applies to the current path only. The robot
   * will move to the next Waypoint if you call {@link #followPath()}.
   *
  ...
  /**
   * Starts the robot moving toward the destination. If no path exists, a new one is created
   * consisting of the destination, otherwise the destination is added to the path. This method is
   * non-blocking, and is equivalent to <code>
   * {@linkplain #addWaypoint(Waypoint) addWaypoint(destination);}
  ...
  /**
   * Starts the moving toward the destination Waypoint created from the parameters. If no path
   * exists, a new one is created, otherwise the new Waypoint is added to the path. This method is
   * non-blocking, and is equivalent to <code>add(float x, float y);   followPath(); </code>
   *
  ...
  /**
   * Rotates the robot to a new absolute heading. For example, rotateTo(0) will line the robot with
   * the x-axis, while rotateTo(90) lines it with the y-axis. If the robot is currently on the move
   * to a coordinate, this method will not attempt to rotate and it will return false.
   *
   * @param angle The absolute heading to rotate the robot to. Value is 0 to 360.
   * @return true if the rotation happened, false if the robot was moving while this method was
   *     called.
   */
  public boolean rotateTo(double angle) {
    ...
    if (_pilot instanceof RotateMoveController) ((RotateMoveController) _pilot).rotate(diff, false);
    return true;
  }

  /**
   * Adds a Waypoint to the end of the path. Call {@link #followPath()} to start moving the along
   * the current path.
   *
  ...
  /**
   * Constructs an new Waypoint from the parameters and adds it to the end of the path. Call {@link
   * #followPath()} to start moving the along the current path.
   *
  ...
  /**
   * Stops the robot. The robot will resume its path traversal if you call {@link #followPath()}.
   */
  public void stop() {
    _keepGoing = false;
    _pilot.stop();
  ...
  /**
   * Waits for the robot to stop for any reason ; returns <code>true</code> if the robot stopped at
   * the final Waypoint of the path.
   *
   * @return <code> true </code> if the path is completed
   */
  ...
  private void callListeners() {
    if (_listeners != null) {
      ...
      //            RConsole.println("listener called interrupt"+_interrupted +" done
      // "+_path.isEmpty()+" "+_pose);
      for (NavigationListener l : _listeners)
        if (_interrupted) l.pathInterrupted(_destination, _pose, _sequenceNr);
        else {
          l.atWaypoint(_destination, _pose, _sequenceNr);
          if (_path.isEmpty()) l.pathComplete(_destination, _pose, _sequenceNr);
        }
    }
  }
      ...
      while (more) {
        while (_keepGoing && _path != null && !_path.isEmpty()) {
          ...
          } else // begin arc direction change
          {
            ...
            Move[] moves;
            double minRadius =
                (_pilot instanceof ArcMoveController
                    ? ((ArcMoveController) _pilot).getMinRadius()
                    : 0);

            if (_destination.headingRequired) {
              moves =
                  ArcAlgorithms.getBestPath(
                      poseProvider.getPose(),
                      (float) minRadius,
                      _destination.getPose(),
                      (float) minRadius);
            } else {
              moves =
                  ArcAlgorithms.getBestPath(
                      poseProvider.getPose(), _destination, (float) minRadius);
            }
            // 2. Drive the path
            for (int i = 0; i < moves.length; i++) {
              ((ArcMoveController) _pilot)
                  .travelArc(moves[i].getArcRadius(), moves[i].getDistanceTraveled(), false);
              if (!_keepGoing) break;
            }
            while (_pilot.isMoving() && _keepGoing) Thread.yield();
          ...
          if (_radius == 0) // differential pilot is used
          {
            ...
            if (_destination.headingRequired) {
              ...
              ((RotateMoveController) _pilot)
                  .rotate(_destination.getHeading() - _pose.getHeading(), false);
            }
          }

          if (_keepGoing && !_path.isEmpty()) {
            //        	  RConsole.println("NAV keep going "+_keepGoing+" pathlength
            // "+_path.size());
            ...
            //             callListeners();  *** if called here,  sets _keepGoing = true - very
            // strange
  ...
  /**
   * frequently tested by Nav.run() to break out of primary control loop reset by stop(), and in Nav
   * if _singleStep is set. or end of path is reached set by followPath(xx) and goTo(xx)
   */
