...
// TODO: Wikipedia article on Homotopy principle has some ideas about calculating position and
// steering of vehicle:
...
/**
 * Vehicles that are controlled by the SteeringPilot class use a similar steering mechanism to a
 * car, in which the front wheels pivot from side to side to control direction.
 *
 * <p>If you issue a command for travel(1000) and then issue a command travel(-500) before it
 * completes the travel(1000) movement, it will call stop, properly inform movement listeners that
 * the forward movement was halted, and then start moving backward 500 units. This makes movements
 * from the SteeringPilot leak-proof and incorruptible.
 *
 * <p>Note: A DifferentialPilot robot can simulate a SteeringPilot robot by calling {@link
 * DifferentialPilot#setMinRadius(double)} and setting the value to something greater than zero
 * (example: 15 cm).
 *
...
public class SteeringPilot implements ArcMoveController, RegulatedMotorListener {
  ...
  /**
   * Creates an instance of the SteeringPilot. The drive wheel measurements are written on the side
   * of the LEGO tire, such as 56 x 26 (= 56 mm or 5.6 centimeters). The accuracy of this class is
   * dependent on physical factors:
   * <li>the surface the vehicle is driving on (hard smooth surfaces are much better than carpet)
   * <li>the accuracy of the steering vehicle (backlash in the steering mechanism will cause
   *     turn-angle accuracy problems)
   * <li>the ability of the steering robot to drive straight (if you see your robot trying to drive
   *     straight and it is driving a curve instead, accuracy will be thrown off significantly)
   * <li>When using SteeringPilot with ArcPoseController, the starting position of the robot is also
   *     important. Is it truly lined up with the x axis? Are your destination targets on the floor
   *     accurately measured?
   *
   *     <p>Note: If your drive motor is geared for faster movement, you must multiply the wheel
   *     size by the gear ratio. e.g. If gear ratio is 3:1, multiply wheel diameter by 3. If your
   *     drive motor is geared for slower movement, divide wheel size by gear ratio. *
   *
   ...
   * @param leftTurnTacho The tachometer the steering motor must turn to in order to turn left with
   *     the minimum turn radius.
   * @param rightTurnTacho The tachometer the steering motor must turn to in order to turn right
   *     with the minimum turn radius.
   */
  public SteeringPilot(
      double driveWheelDiameter,
      lejos.robotics.RegulatedMotor driveMotor,
      lejos.robotics.RegulatedMotor steeringMotor,
  ...
  /**
   * This method calibrates the steering mechanism by turning the wheels all the way to the right
   * and left until they encounter resistance and recording the tachometer values. These values
   * determine the outer bounds of steering. The center steering value is the average of the two.
   * NOTE: This method only works with steering robots that are symmetrical (same maximum steering
   * threshold left and right). TODO: Should be able to get steering parity right from this class!
   * No need to fish for boolean.
   *
   * <p>When you run the method, if you notice the wheels turn left first, then right, it means you
   * need to set the reverse parameter to true for proper calibration. NOTE: The next time you run
   * the calibrate method it will still turn left first, but...
   */
  public void calibrateSteering() {

    // TODO: Not really necessary to check for stall. Could just rotate for about 2 seconds and take
    // a tacho reading.
    // This would help with RemoteMotor and remote SteeringPilot, which doesn't implement
    // isStalled().
    ...
    int center =
        (l + r) / 2; // TODO: Maybe reset tacho to zero? Seems like there is no center variable.
    ...
    */
    ...
    // Adjust values so they are still meaningful when tachocount is reset to zero below (0 =
    // center):
    r -= center;
    l -= center;
    /* System.out.println("LEFT " + l);
  ...
  /**
   * In practice, a robot might steer tighter with one turn than the other. Currently returns
   * minimum steering radius for the least tight turn direction.
   *
  ...
  // NOTE: Currently the steer method locks this SteeringPilot into one proprietary LEGO robot
  // design.
  ...
  // Note: Perhaps it should return the actual radius/arc it achieves, in case can't do the one it
  // is called to do.
  // Although this might really screw things up for the algorithm. Shouldn't necessarily attempt arc
  // it wasn't asked to perform.
  // Perhaps it should check if radius is < minRadius, then throw exception or return failed if it
  // can't do it.
  /** Positive radius = left turn Negative radius = right turn */
  ...
  public void arc(double turnRadius, double arcAngle) throws IllegalArgumentException {
    if (turnRadius == 0)
      throw new IllegalArgumentException(
          "SteeringPilot can't do zero radius turns."); // Can't turn in one spot
  ...
  public void travelArc(double turnRadius, double distance, boolean immediateReturn)
      throws IllegalArgumentException {

    // Hack here because JVM causes extra decimals for Math.abs function?
    double diff = this.getMinRadius() - Math.abs(turnRadius);
    if (diff > 0.1)
      throw new IllegalArgumentException("Turn radius can't be less than " + this.getMinRadius());
  ...
  // TODO: This method should indicate it is not live speed. Such as getSpeedSetting(),
  // setSpeedSetting()
  ...
  public void setLinearSpeed(double speed) {
    // TODO This should set the motor speed for the drive motor, perhaps also calculates based on
    // wheel diameter?
  ...
  public void stop() {
    ...
    //		if(!isMoving()) return false; // Should return no movement? Or moveEvent? Null might be
    // appropriate.
  ...
  public Move getMovement() {
    // TODO This is probably supposed to provide the movement that has occurred since starting? (No
    // Javadocs for this method makes it hard to figure out how to implement this method.)
    return null;
  }
