...
/**
 * The DifferentialPilot class is a software abstraction of the Pilot mechanism of a NXT robot. It
 * contains methods to control robot movements: travel forward or backward in a straight line or a
 * circular path or rotate to a new direction.<br>
 * This class will only work with two independently controlled motors to steer differentially, so it
 * can rotate within its own footprint (i.e. turn on one spot). It registers as a {@link
 * lejos.robotics.RegulatedMotorListener} with each of its motors. An object of this class assumes
 * that it has exclusive control of its motors. If any other object makes calls to its motors, the
 * results are unpredictable. <br>
 * This class can be used with robots that have reversed motor design: the robot moves in the
 * direction opposite to the the direction of motor rotation. .<br>
 * It automatically updates a {@link lejos.robotics.localization.OdometryPoseProvider} which has
 * called the <code>addMoveListener</code> method on this object.<br>
 * Some methods optionally return immediately so the thread that called it can do things while the
 * robot is moving, such as monitor sensors and call {@link #stop()}.<br>
 ...
 * </code> is called, and, if a blocking method is executing, that method exits. The units of
 * measure for travel distance, speed and acceleration are the units used in specifying the wheel
 * diameter and track width in the constructor. <br>
 * In all the methods that cause the robot to change its heading (the angle relative to the X axis
 * in which the robot is facing) the angle parameter specifies the change in heading. A positive
 * angle causes a turn to the left (anti-clockwise) to increase the heading, and a negative angle
 * causes a turn to the right (clockwise). <br>
 ...
 * </pre></code>
 *
 * <p>Note: A DifferentialPilot robot can simulate a SteeringPilot robot by calling
 * DifferentialPilot.setMinRadius() and setting the value to something greater than zero (perhaps 15
 * cm).
 *
...
public class DifferentialPilot implements LineFollowingMoveController {
  /**
   * Allocates a DifferentialPilot object, and sets the physical parameters of the NXT robot.<br>
   * Assumes Motor.forward() causes the robot to move forward.
   *
   * @param wheelDiameter Diameter of the tire, in any convenient units (diameter in mm is usually
   *     printed on the tire).
   * @param trackWidth Distance between center of right tire and center of left tire, in same units
   *     as wheelDiameter.
   ...
   * Allocates a DifferentialPilot object, and sets the physical parameters of the NXT robot.<br>
   *
   * @param leftWheelDiameter Diameter of the left wheel, in any convenient units (diameter in mm is
   *     usually printed on the tire).
   * @param rightWheelDiameter Diameter of the right wheel. You can actually fit intentionally
   *     wheels with different size to your robot. If you fitted wheels with the same size, but your
   *     robot is not going straight, try swapping the wheels and see if it deviates into the other
   *     direction. That would indicate a small difference in wheel size. Adjust wheel size
   *     accordingly. The minimum change in wheel size which will actually have an effect is given
   *     by minChange = A*wheelDiameter*wheelDiameter/(1-(A*wheelDiameter) where A =
   *     PI/(moveSpeed*360). Thus for a moveSpeed of 25 cm/second and a wheelDiameter of 5,5 cm the
   *     minChange is about 0,01058 cm. The reason for this is, that different while sizes will
   *     result in different motor speed. And that is given as an integer in degree per second.
   * @param trackWidth Distance between center of right tire and center of left tire, in same units
   *     as wheelDiameter.
   * @param leftMotor The left Motor (e.g., Motor.C).
   * @param rightMotor The right Motor (e.g., Motor.A).
   ...
   */
  public DifferentialPilot(
      final double leftWheelDiameter,
      final double rightWheelDiameter,
      final double trackWidth,
      final RegulatedMotor leftMotor,
      final RegulatedMotor rightMotor,
      final boolean reverse) {
  ...
  public void setLinearSpeed(final double travelSpeed) {
    if (!isMoving()) {
      ...
      setSpeed(
          (int) Math.round(travelSpeed * _leftDegPerDistance),
  ...
  public void setAngularSpeed(double rotateSpeed) {
    ...
    setSpeed(
        (int) Math.round(rotateSpeed * _leftTurnRatio),
  ...
  public void forward() {
    ...
    movementStart();
    setSpeed(
        Math.round(_robotTravelSpeed * _leftDegPerDistance),
   ...
   *
   * @param angle The wanted angle of rotation in degrees. Positive angle rotate left
   *     (anti-clockwise), negative right.
   */
  ...
  public void rotate(final double angle, final boolean immediateReturn) {
    ...
    setMotorAccel(_acceleration);
    setSpeed(
        Math.round(_robotRotateSpeed * _leftTurnRatio),
   ...
   * A positive distance causes forward motion, a negative distance moves backward. If a drift
   * correction has been specified in the constructor it will be applied to the left motor. calls
   * travel(distance, false)
   *
   * @param distance The distance to move. Unit of measure for distance must be same as
   *     wheelDiameter and trackWidth.
   */
   ...
   * Moves the NXT robot a specific distance in an (hopefully) straight line.<br>
   * A positive distance causes forward motion, a negative distance moves backward. If a drift
   * correction has been specified in the constructor it will be applied to the left motor.
   *
   * @param distance The distance to move. Unit of measure for distance must be same as
   *     wheelDiameter and trackWidth.
   * @param immediateReturn If true this method returns immediately.
   ...
   * use as the parameter for steer() negative argument means center of turn is on right, so angle
   * of turn is negative
   *
   * @param radius
  ...
  /**
   * Returns the radius of the turn made by steer(turnRate) Used in for planned distance at start of
   * arc and steer moves.
   *
   * @param turnRate
  ...
  /**
   * Starts the robot moving forward along a curved path. This method is similar to the {@link
   * #arcForward(double radius )} method except it uses the <code> turnRate</code> parameter do
   * determine the curvature of the path and therefore has the ability to drive straight. This makes
   * it useful for line following applications.
   *
   * <p>The <code>turnRate</code> specifies the sharpness of the turn. Use values between -200 and
   * +200.<br>
   * A positive value means that center of the turn is on the left. If the robot is traveling toward
   * the top of the page the arc looks like this: <b>)</b>. <br>
   * A negative value means that center of the turn is on the right so the arc looks this: <b>(</b>.
   * <br>
   * . In this class, this parameter determines the ratio of inner wheel speed to outer wheel speed
   * <b>as a percent</b>.<br>
   * <I>Formula:</I> <code>ratio = 100 - abs(turnRate)</code>.<br>
   * When the ratio is negative, the outer and inner wheels rotate in opposite directions. Examples
   ...
   *       turn left
   *   <LI><code>steer(200)</code> -> the inner wheel turns at the same speed as the outer wheel - a
   *       zero radius turn.
   * </UL>
   *
   * <p>Note: If you have specified a drift correction in the constructor it will not be applied in
   * this method.
   *
   * @param turnRate If positive, the left side of the robot is on the inside of the turn. If
   *     negative, the left side is on the outside.
   */
  ...
  /**
   * Starts the robot moving backward along a curved path. This method is essentially the same as
  ...
  /**
   * Moves the robot along a curved path through a specified turn angle. This method is similar to
   * the {@link #arc(double radius , double angle)} method except it uses a ratio of motor speeds to
   * determine the curvature of the path and therefore has the ability to drive straight. This makes
   ...
   * The <code>turnRate</code> specifies the sharpness of the turn. Use values between -200 and
   * +200.<br>
   * For details about how this parameter works.See {@link #steer(double turnRate) }
   *
   * <p>The robot will stop when its heading has changed by the amount of the <code>angle</code>
   * parameter.<br>
   * If <code>angle</code> is positive, the robot will move in the direction that increases its
   * heading (it turns left).<br>
   * If <code>angle</code> is negative, the robot will move in the directin that decreases its
   * heading (turns right).<br>
   * If <code>angle</code> is zero, the robot will not move and the method returns immediately.
   *
   * <p>The sign of the turn rate and the sign of the angle together determine if the robot will
   * move forward or backward. Assuming the robot is heading toward the top of the page. Then a
   * positive turn rate means the arc looks like this: <b> )</b> . If the angle is positive, the
   * robot moves forward to increase its heading angle. If negative, it moves backward to decrease
   * the heading. <br>
   * But if the turn rate is negative, the arc looks like this: <b> ( </b> .So a positive angle
   * (increase in heading) means the robot moves backwards, while a negative angle means the robot
   * moves forward to decrease its heading.
   *
   * <p>Note: If you have specified a drift correction in the constructor it will not be applied in
   * this method.
   *
   * @param turnRate If positive, the left side of the robot is on the inside of the turn. If
   *     negative, the left side is on the outside.
   * @param angle The angle through which the robot will rotate. If negative, the robot will move in
   *     the directin that decreases its heading.
   */
  ...
  /**
   * Moves the robot along a curved path for a specified angle of rotation. This method is similar
   * to the {@link #arc(double radius, double angle, boolean immediateReturn)} method except it uses
   * the <code> turnRate()</code> parameter to determine the curvature of the path and therefore has
   * the ability to drive straight. This makes it useful for line following applications. This
   * method has the ability to return immediately by using the <code>immediateReturn</code>
   ...
   * If <code>angle</code> is positive, the robot will move in the direction that increases its
   * heading (it turns left).<br>
   * If <code>angle</code> is negative, the robot will move in the direction that decreases its
   * heading (turns right).<br>
   ...
   * @param turnRate If positive, the left side of the robot is on the inside of the turn. If
   *     negative, the left side is on the outside.
   * @param angle The angle through which the robot will rotate. If negative, robot traces the
   *     turning circle backwards.
  ...
  /**
   * helper method used by steer(float) and steer(float,float,boolean) sets _outsideSpeed,
   * _insideSpeed, _steerRatio set motor acceleration to help continuous steer and arc moves
   *
   * @param turnRate .
   */
  ...
  /**
   * called at start of a movement to inform the listeners that a movement has started. Must be
   * called before the motors start to move to capture the correct positional information
   */
  protected void movementStart() {
    Move move =
        new Move(
            ...
            isMoving());
    reset();
    for (MoveListener ml : _listeners) ml.moveStarted(move, this);
    reset();
  }
  ...
  private void movementStop() {
    ...
    move =
        new Move(
            ...
            _robotRotateSpeed,
            isMoving());
    for (MoveListener ml : _listeners) ml.moveStopped(move, this);
    synchronized (_monitor) {
      _moveActive = false;
      _monitor.notifyAll();
    }
  ...
  /**
   * Set the radius of the minimum turning circle. Note: A DifferentialPilot robot can simulate a
   * SteeringPilot robot by calling DifferentialPilot.setMinRadius() and setting the value to
   * something greater than zero (example: 15 cm).
   *
  ...
  public float getAngleIncrement() {
    return /* _parity * */ (((getRightCount() - _rightTC) / _rightTurnRatio)
            - ((getLeftCount() - _leftTC) / _leftTurnRatio))
        / 2.0f;
  }
  ...
  private class Monitor extends Thread {
    ...
    public synchronized void run() {
      while (more) {
        if (_moveActive) {
          if (isStalled()) DifferentialPilot.this.stop();
  ...
  /**
   * Left motor revolutions for 360 degree rotation of robot (motors running in opposite
   * directions). Calculated from wheel diameter and track width. Used by rotate() and steer()
   * methods.
   */
  private final float _leftTurnRatio;

  /**
   * Right motor revolutions for 360 degree rotation of robot (motors running in opposite
   * directions). Calculated from wheel diameter and track width. Used by rotate() and steer()
   * methods.
   */
  ...
  /**
   * Speed of robot for moving in wheel diameter units per seconds. Set by setSpeed(),
   * setTravelSpeed()
   */
