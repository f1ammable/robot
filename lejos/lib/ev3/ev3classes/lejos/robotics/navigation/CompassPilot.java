 ...
 *
 * @deprecated This class will disappear in NXJ version 1.0. Compass should be added to a
 *     PoseProvider.
 * @see lejos.robotics.localization.PoseProvider#getPose()
 */
// TODO: Note @deprecated message above, I'm not sure PoseProvider is exactly right place to point
// users to yet.
...
public class CompassPilot extends DifferentialPilot {
  ...
  /**
   * Returns the change in robot heading since the last call of reset() normalized to be within -180
   * and _180 degrees
   */
  ...
  /**
   * Rotates the robot 360 degrees while calibrating the compass resets compass zero to heading at
   * end of calibration
   */
   ...
   * value moves it backwards. The robot steers to maintain its compass heading.
   *
   ...
   * A positive distance causes forward motion; negative distance moves backward. Robot steers to
   * maintain its compass heading;
   *
   ...
   *
   * @param immediateReturn - if true, method returns immediately. Robot stops when specified angle
   *     is reached or when stop() is called
   */
   ...
   * radius turn.
   *
   * @param angle degrees. Positive angle rotates to the left (clockwise); negative to the right.
   *     <br>
  ...
  /**
   * Stops the robot soon after the method is executed. (It takes time for the motors to slow to a
   * halt)
   */
  public void stop() {
    super.stop();
  ...
  /**
   * inner class to regulate heading during travel move Proportional control of steering ; error is
   * an average of heading change from tacho counts and from compass change
   *
      ...
      while (true) {
        ...
        { // travel started
          ...
          do // travel in progress
          {
            ...
            _estimatedHeading =
                normalize(
                    0.5f * normalize(compass.getDegreesCartesian()) + 0.5f * _estimatedHeading);
