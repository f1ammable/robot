 ...
 * Both differential and holonomic robots can be represented by the WheeledChassis class.
 *
 * <p><b>How to create a WheeledChassis object</b><br>
 * The constructor of the DifferentialChassis class accepts an array of Wheel objects. Each of the
 * wheel objects describes one of the motorized wheels on the chassis. A Description of a wheel
 * consists of its diameter, its position, its motor and the gear train between wheel and motor.
 * Wheel objects can be created using a modeler class. A modeler for traditional wheel can be
 * obtained using the {@link #modelWheel} method. A modeler for a holonomic wheel can be obtained
 * using the {@link #modelHolonomicWheel} method. .
 *
 * <p>This example creates a WheeledChassis for a differential robot.
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>This example creates a WheeledChassis for a holonomic robot.
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Please note that a Chassis can have an unlimited number of motorized wheels but a differential
 * robot needs at least two wheels and a holonomic robot needs at least three wheels.
 *
 * <p>See also the {@link Chassis} interface.*
 *
...
public class WheeledChassis implements Chassis {
  ...
  /**
   * The program adds a dummy wheel to a differential chassis. This dummy wheel makes it possible to
   * use linear algebra on three elements (x, y, angle) for the differential chassis. The y value is
   * always zero for a differential chassis.
   */
  ...
  public WheeledChassis(Wheel[] wheels, int dim) {
    ...
    if (nWheels < dim)
      throw new IllegalArgumentException(
          String.format("The chassis must have at least %d motorized wheels", dim));
    ...
    try {
      ...
      throw new RuntimeException(
          "Invalid wheel setup, this robot is not controlable. Check position of the wheels.");
    }

    // create absolute versions of forward and reverse matrices (used for speed and acceleration
    // that both are always positive).
  ...
  public void waitComplete() {
    for (RegulatedMotor wheel : motor) wheel.waitComplete();
  }

  @Override
  public boolean isStalled() {
    for (RegulatedMotor wheel : motor) {
      if (wheel.isStalled()) return true;
    }
    return false;
  }
  ...
  public void travelCartesian(double xSpeed, double ySpeed, double angularSpeed) {
    setVelocity(
        Math.sqrt(xSpeed * xSpeed + ySpeed * ySpeed), Math.atan2(ySpeed, xSpeed), angularSpeed);
  }
  ...
  public synchronized void setVelocity(double linearSpeed, double direction, double angularSpeed) {
    ...
    Matrix motorSpeed =
        forward.times(toCartesianMatrix(linearSpeed, Math.toRadians(direction), angularSpeed));
    Matrix motorAcceleration =
        forwardAbs.times(
            copyAbsolute(
                toCartesianMatrix(
                    linearAcceleration, Math.toRadians(direction), angularAcceleration)));
  ...
  public void arc(double radius, double angle) {
    ...
    if (Double.isInfinite(angle)) {
      // Decrease one of both speed components so that they have the calculated ratio and call
      // travel method
      if (ratio > 1)
        setVelocity(Math.signum(angle) * linearSpeed, 0, Math.signum(radius) * linearSpeed / ratio);
      else
        setVelocity(
            Math.signum(angle) * angularSpeed * ratio, 0, Math.signum(radius) * angularSpeed);
    } else if (radius == 0) {
      rotate(angle);
      return;
    } else if (Double.isInfinite(radius)) {
      if (angle < 0) travel(Double.POSITIVE_INFINITY);
      else travel(Double.NEGATIVE_INFINITY);
    } else {
      // Matrix holding linear and angular distance matching the specified arc
      Matrix displacement =
          toMatrix(
              Math.signum(angle) * 2 * Math.PI * Math.abs(radius) * Math.abs(angle) / 360,
              0,
              Math.signum(radius) * angle);

      Matrix tSpeed;
      Matrix tAcceleration;
      // Decrease one of both speed and acceleration components so that they have the calculated
      // ratio
      if (ratio > 1) {
   ...
   */
  protected synchronized void setMotors(
      Matrix motorDelta, Matrix motorSpeed, Matrix motorAcceleration) {
    master.startSynchronization();
    for (int i = 0; i < nWheels; i++) {
  ...
  public double getMaxLinearSpeed() {
    ...
    return Math.sqrt(
        wheelSpeed.get(0, 0) * wheelSpeed.get(0, 0) + wheelSpeed.get(1, 0) * wheelSpeed.get(1, 0));
  }
  ...
  public Move getDisplacement(Move move) {
    ...
    double distance =
        Math.sqrt(delta.get(0, 0) * delta.get(0, 0) + delta.get(1, 0) * delta.get(1, 0));
   ...
   *
   * <ul>
   *   <li>polarPosition() specifies the location of the wheel
   *   <li>gearRatio() specifies the gear ratio of the gear train between motor and wheel
   *   <li>invert() inverts the direction of the motor. Equivalent to a negative gearing
   * </ul>
   *
   * <p>
   *
  ...
  public static class HolonomicModeler implements Wheel {
     ...
     *
     * @param motor The regulated motor that drives the wheel
     * @param diameter The diameter of the wheel (Lego wheels have the diameter printed on the side)
     */
     ...
     *
     * <p>Use this method only when the wheel axis points to the center of the robot.
     *
     ...
     * <p>
     *
     * @param x X-position of the center of the wheel in respect to the robots origin
     * @param y Y-position of the center of the wheel in respect to the robots origin
     * @param angle Angle between the driving direction of the wheel and the robots x-axis
     * @return the modeler
     */
     ...
     *
     * @param val The ratio between wheel speed and motor speed. A ratio greater than 1 means the
     *     wheel turns faster than the motor.
     * @return the modeler
     */
    ...
    public Matrix getFactors() {
      // TODO: correct angular component when the wheel axis doesn't go through the origin of the
      // robot
      ...
      factors.set(
          0,
          0,
          Math.cos(Math.toRadians(pose.getHeading()))
              * (360)
              / (diameter * Math.PI * gearRatio * (invert ? -1 : 1)));
      factors.set(
          0,
          1,
          Math.sin(Math.toRadians(pose.getHeading()))
              * (360)
              / (diameter * Math.PI * gearRatio * (invert ? -1 : 1)));
      factors.set(
          0, 2, ((2 * pose.getLocation().length()) / (diameter * gearRatio * (invert ? -1 : 1))));
      return factors;
    }
   ...
   *
   * <ul>
   *   <li>offset() specifes the location of the wheel along the y-axis
   *   <li>gearRatio() specifes the gear ratio of the gear train between motor and wheel
   *   <li>invert() inverts the direction of the motor. Equivalent to a negative gearing
   * </ul>
   *
   * <p>
  ...
  public static class Modeler implements Wheel {
     ...
     *
     * @param val The distance between the robots yPose-axis and the center of the wheel
     * @return the modeler
     */
  ...
  /**
   * The odometer keeps track of the robot pose based on odometry using the encoders of the
   * regulated motors of the wheels.
   *
   * @author Aswin Bouwmeester
   */
  ...
  protected double getMax(Matrix a) {
    ...
    for (int m = 0; m < a.getRowDimension(); m++) {
      for (int n = 0; n < a.getColumnDimension(); n++) {
        if (!Double.isNaN(a.get(m, n))) max = Math.max(Math.abs(a.get(m, n)), max);
      }
    }
    return max;
  }
  ...
  /**
   * Make a copy of the source matrix, each of its element being the absolute value of the elements
