 ...
 *
 * @author Lawrie Griffiths <br>
 *     <br>
 *     WARNING: THIS CLASS IS SHARED BETWEEN THE classes AND pccomms PROJECTS. DO NOT EDIT THE
 *     VERSION IN pccomms AS IT WILL BE OVERWRITTEN WHEN THE PROJECT IS BUILT.
 */
public class Line extends Line2D.Float {
  ...
  /**
   * Returns the minimum distance between two line segments--this line segment and another. If they
   * intersect the distance is 0. Lines can be parallel or skewed (non-parallel).
   *
   * @param seg The other line segment.
   * @return The distance between the two line segments.
   */
  public double segDist(Line seg) {
    if (this.intersectsLine(seg)) return 0;
    double a =
        Line2D.ptSegDist(
            this.getX1(), this.getY1(), this.getX2(), this.getY2(), seg.getX1(), seg.getY1());
    double b =
        Line2D.ptSegDist(
            this.getX1(), this.getY1(), this.getX2(), this.getY2(), seg.getX2(), seg.getY2());
    double c =
        Line2D.ptSegDist(
            seg.getX1(), seg.getY1(), seg.getX2(), seg.getY2(), this.getX1(), this.getY1());
    double d =
        Line2D.ptSegDist(
            seg.getX1(), seg.getY1(), seg.getX2(), seg.getY2(), this.getX2(), this.getY2());
  ...
  /**
   * Make this line longer by an amount delta at each end. Used by DijkstraPathFinder to use the
   * same LineMap as the a RangeScanner in MCL navigation.
   *
