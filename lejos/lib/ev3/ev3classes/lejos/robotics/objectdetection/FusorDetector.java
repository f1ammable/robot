 ...
 * listener, or if you want to control them at the same time (such as disabling them all at once)
 * you can use this class.
 *
 * <p>This class maintains its own thread for checking the FeatureDetectors.
 *
 * @author BB
 */
public class FusorDetector implements FeatureDetector, FeatureListener {
  ...
  public void addDetector(FeatureDetector detector) {
    ...
    if (detector.getDelay() > this.delay) this.delay = detector.getDelay();
  ...
  public Feature scan() {
    ...
    for (FeatureDetector d : detectors) {
      ...
      if (df != null) {
        ...
        for (int i = 0; i < temp.size(); i++) rr.add(temp.get(i));
      }
    }
  ...
  /**
   * This method must deal with different delays from different sensors. e.g. Touch is 50 ms, while
   * ultrasonic is 250 ms. But must amalgamate readings that are captured over same period.
   * SOLUTION: Use listeners. Get max scan time (e.g. 250 ms) and wait that long for each listener
   * to report in before reporting all at once.
   */
  public void featureDetected(Feature feature, FeatureDetector detector) {
    // 1. Need to know who is reporting this otherwise can accumulate 5 touch sensor readings (50 ms
    // each)
    // while range sensor produces one (250 ms). Need to ID who is reporting via comparison against
    // set of detectors.
    int index = detectors.indexOf(detector);

    // 2. Store latest from each object in an array (arraylist size of detectors) corresponding to
    // that particular
    // FeatureDetector.
    ...
    // 3. Thread waits "delay" for them to report. Accumulate each one as described in 2. When time
    // is up,
    // make one object accumulated from the array. (see NotifyThread)

    // 4. After delay, thread notifies all listeners IF features isn't equal to null. Then makes new
    // readings object.
    // (see NotifyThread)
  }
    ...
    public void run() {
      while (true) {
        ...
        if (enabled) {
          ...
          if (rrs.size() > 0) notifyListeners(new RangeFeature(rrs));

          // 4. Now clear out the detectors so they aren't resent next loop.
          readings.clear();

          // 5. And add dummies to it to make it proper size.
          for (int i = 0; i < detectors.size(); i++) readings.add(new RangeReadings(0));
        }
  ...
  /**
   * This method enables/disables automatic scanning and listener reporting for this object and all
   * FeatureDetectors used in this FusorDetector object.
   */
