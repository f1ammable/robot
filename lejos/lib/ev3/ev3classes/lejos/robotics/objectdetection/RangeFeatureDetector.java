 ...
 * This class is unable to identify the feature and merely reports the range and angle to the
 * object.
 *
 * <p>You can also have the scan identify the object (such as a camera using facial recognition to
 * identify a person). One possibility to implement this is to extend RangeFeatureDetector and add a
 * camera to the constructor. When the FeatureDetector range scanner detects an object, take a
 * picture and look for a face in the image. This is then reported as an extended class of
 * RangeFeature called PersonFeature, which has a getPerson() method containing information on the
 * object that was detected.
 *
 * <p>To create a more complex FeatureDetector, extend this class and override the {@link
 * FeatureDetector#scan()} method. It is possible to add more complex functionality in this method,
 * such as only returning a "hit" if the scanner detects an object in the same location twice in a
 * row. This type of filtering could also take place in the FeatureListener method, although then
 * single {@link FeatureDetector#scan()} calls will not contain the filtering.
 *
...
public class RangeFeatureDetector extends FeatureDetectorAdapter {
  ...
  /**
   * This constructor allows you to specify the sensor, the maximum distance to report a detection,
   * and the delay between scanning the sensor. It assumes the range sensor is pointed straight
   * ahead, so heading is always 0 for the returned RangeReading.
   *
  ...
  /**
   * This constructor allows you to specify the sensor, the maximum distance to report a detection,
   * the delay between scanning the sensor, and the angle the sensor is pointed.
   *
   * @param rf The range finder sensor. e.g. UltrasonicSensor
   * @param maxDistance The upper limit of distance it will report. e.g. 40 cm.
   * @param delay The interval range finder checks for objects. e.g. 250 ms.
   * @param angle The angle, in degrees, the range sensor is pointed. (0 = forward, +ve = left, -ve
   *     = right)
   */
  ...
  public Feature scan() {
    ...
    // TODO: Note: If it is slower to retrieve multiple rather than single scan. Have option for
    // single only in constructor?
    float[] ranges = range_finder.getRanges();
    RangeReadings rrs = new RangeReadings(0); // TODO: Should it omit anything outside max_dist?
    if (ranges.length <= 0)
      return null; // Check to make sure it retrieved some readings. Seems to return nothing
                   // sometimes.
