...
/**
 * This class represents three beacons in a triangle. Uses triangulation to calculate the position
 * of the robot.
 *
 * <p>Note: The algorithm in calcPose() is taken from "Generalized Geometric Triangulation Algorithm
 * for Mobile Robot Absolute Self-Localization" from University of Minho, Portugal, 2003.<br>
 ...
 * http://repositorium.sdum.uminho.pt/bitstream/1822/6057/1/An Improved Version of the Generalized
 * Geometric Triangulation Algorithm.pdf
 *
...
public class BeaconTriangle {
  ...
  /**
   * Triangulates the pose of a robot given three angles to the three beacons. The angle to a beacon
   * must correspond to the proper beacon as given in the BeaconTriangle constructor. For example,
   * the angle a1 in the method calcPose() must be the angle to beacon1, as given in the
   * constructor.
   *
  ...
  public Pose calcPose(double a1, double a2, double a3) {
    ...
    // line 1: if less than 3 beacon angles, abort. Unneeded here due to method signature. Will
    ...
    double tau =
        atan(
            (sin(Y12) * (L12 * sin(Y31) - L31 * sin(gamma)))
                / (L31 * sin(Y12) * cos(gamma) - L12 * cos(Y12) * sin(Y31)));
