...
/**
 * Represents a particle for the particle filtering algorithm. The state of the particle is the
 * pose, which represents a possible pose of the robot.
 *
 * <p>The weight for a particle is set by taking a set of theoretical range readings using a map of
 * the environment, and comparing these ranges with those taken by the robot. The weight represents
 * the relative probability that the robot has this pose. Weights are from 0 to 1.
 *
...
public class MCLParticle {
  ...
  /**
   * Apply the robot's move to the particle with a bit of random noise. Only works for rotate or
   * travel movements.
   *
  ...
  public void applyMove(Move move, float distanceNoiseFactor, float angleNoiseFactor) {
    ...
    pose.setHeading(
        (float)
            (pose.getHeading() + move.getAngleTurned() + (angleNoiseFactor * rand.nextGaussian())));
