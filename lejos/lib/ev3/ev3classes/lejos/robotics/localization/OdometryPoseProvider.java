...
/**
 * A PoseProvider keeps track of the robot {@link lejos.robotics.navigation.Pose}. It does this
 * using odometry (dead reckoning) data contained in a {@link lejos.robotics.navigation.Move}, which
 * is supplied by a {@link lejos.robotics.navigation.MoveProvider}. When the PoseProivder is
 * constructed, it registers as listener with its MoveProvider,
 */
public class OdometryPoseProvider implements PoseProvider, MoveListener, SampleProvider {
  ...
  /**
   * returns a new pose that represents the current location and heading of the robot. If called
   * while the robot is moving, the PoseProvider will get updated odometry data from its
   * MoveProvider
   *
   * @return pose
   */
