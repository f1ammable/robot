 ...
 * <b>The starting position of the robot must be such that it roughly points in the direction of
 * beacon 2, with beacon 1 to the right of the robot, and beacon 3 to the left.</b> This ensures
 * that when it performs the first scan, the beacons are scanned in order 1, 2, 3. After the first
 * scan, the robot can move about on its own and will be able to scan them in any order.
 *
 * <p>After it does a scan, you will either hear a beep or a buzz. A beep means the calculation
 * worked. A buzz means something went wrong with the calculation. This could be the beacon ordering
 * is wrong, or it didn't scan three beacons in, or the calculated beacon pose deviated
 * significantly from the estimated odometry pose. In this case, the results are thrown out, the
 * odometry pose is used, and it will try another scan after the next move. If you hear a lot of
 * buzzes in a row, it probably means it will not recover.
 *
 * <p>The class uses an OdometryPoseProvider internally for three reasons:
 * <li>1. To serve as a backup pose provider if the BeaconLocator detects less than 3 beacons, or
 *     greater than 3 (meaning a false reflection occurred).
 * <li>2. It is time consuming to scan beacons after every move. Instead, this will scan about every
 *     5 moves. In between these scans, it uses the OdometryPoseProvider to navigate.
 * <li>3. It is essential to scan the beacons in order. This allows it to determine the identity of
 *     each beacon so they are properly fed to the equations. The OdometryPoseProvider keeps track
 *     of heading so it knows which beacons were scanned.
 *
 *     <p>The downside of using an OdometryPoseProvider to assist with detecting pose is that the
 *     robot needs to be placed with an approximate heading prior to first scan. Also, if the robot
 *     is moved at any time (lifted and placed with a different heading), it probably will no longer
 *     be able to identify the beacons and become hopelessly lost. As long as the wheels are always
 *     on the ground, this should not happen.
 *
 * @author BB
 */
public class BeaconPoseProvider implements PoseProvider, MoveListener {
  ...
  // 2. TODO: Ability to not even tell it the coordinates of the beacons. Instead it starts off with
  // the assumption that the
  // robot is at 0,0,0. Then it does the particle set with MCL, applies a move, then scan, then
  // weeds out the point cloud.
  // Keeps going until it has coordinates of three beacons figured out. Optimally this would be
  // included in BeaconTriangle
  // in most or all code.
  // Also add methods for saving and loading beacon coordinates to flash memory, then could call
  // this after it has located
  // them, and load them in before next trial.

  // 3. TODO: Could also do a sanity check on beacon pose calculation to see if x, y is within say
  // 100 cm of odometry
  //  pose estimate. If not, then probably a false beacon was scanned and a true beacon was missed
  // (rare) which resulted in it
  ...
  // TODO: Alt constructor that does not attempt to sort the beacons using the odometry (no
  // MoveProvider).
  // This constructor will be useful for BeaconLocators that can identify the beacon order/id, such
  // as an NXTCamBeaconLocator.

  /**
   * Creates a PoseProvider using beacons. The BeaconLocator must be capable of scanning the angles
   * to 3 beacons. The BeaconTriangle contains the coordinates of the three beacons. The
   * MoveProvider is simply a Pilot, such as DifferentialPilot, which is used to keep track of
   * odometry.
   *
   * <p>By default, this class takes a scan every 5 moves of the robot.
   *
   ...
   */
  public BeaconPoseProvider(
      BeaconLocator bl, BeaconTriangle bt, MoveProvider mp, int scanInterval) {
    this.bt = bt;
    this.bl = bl;
  ...
  /**
   * Converts the absolute Cartesian heading into the relative angle values the robot sees when
   * scanning.
   *
  ...
  public Pose getPose() {
    if (moves >= scanInterval) {
      ...
      if (beaconAngles.size() == 3) {
        ...
        if (hasScanned) { // Can only do this after one successful scan. Otherwise opp.setPose() has
                          // not been set to proper coordinates.
   ...
   * This method assumes beacons will always contain the beacon angles that were scanned in
   * counter-clockwise. Strategy: Pick widest theoretical angle between beacons. There will always
   * be one at least 120 degs. Then determine which beacon is clockwise of this angle.
   *
  ...
  private static boolean sortBeacons(ArrayList<Double> beacons, ArrayList<Double> angles) {
    // Could take a1, a2, a3 instead. Then do sanity check when done to make sure the difference
    // between a1 and 1, a2 and 2,
    // a3 and 3 are all about the same, such as all about 20 degrees from one another--or at least
    ...
    // Sanity check? If largest angle equals another, then this is a bad situation. Abort and use
    // Odometry.
    // What if there are two similar largest angles? Could variability screw this up? If there are
    // two similar ones,
    // then what it could do is start to look at which is closest to the expected angle. Too
    // dangerous.

    // Another sanity check? Check if order of both sets of angles goes greatest, least, median or
    // whatever. However,
    // this could be dangerous because if two are very similar, a bit of real-world variability
    // could cause it to throw
    // out good results. Too dangerous.

    // OK, so now it should pick the next beacon angle counter-clockwise of middleAngle. This will
    // then be designated
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (indexedAngle < middleAngle)
        indexedAngle +=
            360; // since measuring angle counter clockwise, smaller angles actually add 360
    ...
    // System.out.println("Beacon index counterclockwise of middle angle " +
    // beaconAngleCounterClockwiseIndex);
    // System.out.println("Beacon angle counterclockwise of middle angle " +
    // beacons.get(beaconAngleCounterClockwiseIndex));
    ...
    for (int aIndex = aAngleCounterClockwiseIndex; anglesOrdered < 3; aIndex++, bIndex++) {
      ...
      // If a31, then 1. If a23 then 3. If a12 then 2. But assigning the actual scanned beacon angle
      // that is after the halfway mark.
      tempAngles[aIndex] = beacons.get(bIndex);
      anglesOrdered++;
    }

    beacons.clear();
    for (int i = 0; i < tempAngles.length; i++) beacons.add(tempAngles[i]);
  ...
  public void setPose(Pose aPose) {
    // TODO When this is called, it sets the current robot location as 0,0 or whatever they set it
    // as.
