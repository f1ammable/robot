...
public class MCLParticleSet implements Transmittable {
  ...
  /**
   * Generates a set of particles within the map that have a minimum weight as as calculated from
   * the particle pose, the range readings and the map.
   *
   * @param map
  ...
  /**
   * Generates a circular cloud of particles centered on initialPose with random normal radius and
   * angle, and random normal heading.
   *
   * @param map the map
   ...
   */
  public MCLParticleSet(
      RangeMap map, int numParticles, Pose initialPose, float radiusNoise, float headingNoise) {
    this.map = map;
    this.numParticles = numParticles;
    border = 0;
    boundingRect = map.getBoundingRect();
   ...
   *
   * <p>Note that the new set has multiple instances of the particles with higher weights.
   *
  ...
  /**
   * Find the index of the particle closest to a given co-ordinates. This is used for diagnostic
   * purposes.
   *
  ...
  public int findClosest(float x, float y) {
    ...
    for (int i = 0; i < numParticles; i++) {
      ...
      float distance =
          (float)
              Math.sqrt(
                  (double) ((pose.getX() - x) * (pose.getX() - x))
  ...
  /**
   * Find the closest particle to specified coordinates and dump its details to a data output
   * stream.
   *
   ...
   * @throws IOException
   */
  public void dumpClosest(RangeReadings rr, DataOutputStream dos, float x, float y)
      throws IOException {
