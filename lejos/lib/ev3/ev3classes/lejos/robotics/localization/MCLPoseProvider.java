...
/**
 * Maintains an estimate of the robot pose using sensor data. It uses Monte Carlo Localization (See
 * section 8.3 of "Probabilistic Robotics" by Thrun et al. <br>
 * Uses a {@link MCLParticleSet} to represent the probability distribution of the estimated pose. It
 * uses a {@link lejos.robotics.navigation.MoveProvider} to supply odometry data whenever a movement
 * is completed, from which the {@link lejos.robotics.navigation.Pose} of each particle is updated.
 * It then uses a {@link lejos.robotics.RangeScanner} to provide {@link
 * lejos.robotics.RangeReadings} which are used, together with the {@link
 * lejos.robotics.mapping.RangeMap} to calculate the probability weight of each {@link MCLParticle}
 * .
 *
...
public class MCLPoseProvider implements PoseProvider, MoveListener, Transmittable {
   ...
   */
  public MCLPoseProvider(
      MoveProvider mp, RangeScanner scanner, RangeMap map, int numParticles, int border) {
  ...
  /**
   * Generates an initial particle set using the range readings. The particles have a significant
   * probability weight given the readings.
   *
   * @param readings
  ...
  public void setInitialPose(RangeReadings readings, float sigma) {
    ...
    particles =
        new MCLParticleSet(map, numParticles, border, readings, 2 * sigma * sigma, minWeight);
    updated = true;
  }
  ...
  /**
   * Generate a new particle set, uniformly distributed within the map, and uniformly distributed
   * heading.
   */
  ...
  /**
   * Required by MoveListener interface. The pose of each particle is updated using the odometry
   * data of the Move object.
   *
  ...
  /**
   * Calls range scanner to get range readings, calculates the probabilities of each particle from
   * the range readings and the map and calls resample(()
   *
  ...
  public boolean update(RangeReadings readings) {
    ...
    if (debug)
      System.out.println(" max Weight " + particles.getMaxWeight() + " Good pose " + goodPose);
   ...
   */
  public Pose getPose() {
    if (debug) System.out.println("Mcl call update; updated? " + updated + " busy " + busy);
    if (!updated) {
      while (busy) {
  ...
  public Rectangle2D getErrorRect() {
    return new RectangleInt32((int) minX, (int) minY, (int) (maxX - minX), (int) (maxY - minY));
  }
  ...
  /**
   * returns true if particle weights are being updated. The robot should not move while this is
   * happening otherwise the prediction from odometry data may introduce errors into the updating.
   *
