...
/**
 * Generates a grid of nodes. Spacing between the grid nodes and clearance around map geometry can
 * be specified. This set can be generated once at the beginning of a user program, and the same
 * node set can be used for all subsequent navigation.
 *
 * @author BB
 */
public class FourWayGridMesh implements NavigationMesh {
  ...
  /**
   * Instantiates a grid mesh of nodes which won't interconnect between any map geometry. Will also
   * keep away the set parameter from map geometry. Grid spacing is adjustable via the constructor.
   *
  ...
  /**
   * Change the size of each grid square. NOTE: When grid space value is changed, this class does
   * not regenerate the navigation mesh until regenerate() is explicitly called.
   *
  ...
  /**
   * Changes the safety zone between all nodes/connections and map geometry. This leaves a margin of
   * error between potential object collisions and the robot. NOTE: When clearance value is changed,
  ...
  /**
   * Feeds this class a new map. NOTE: When Map is changed, this class does not regenerate the
   * navigation mesh until regenerate() is explicitly called.
   *
  ...
  public void regenerate() {
    ...
    for (float y = starty; y < endy; y += gridspace) {
      ...
      for (float x = startx; x < endx; x += gridspace) {
        ...
        // TODO: Why not use addNode for each subsequent node?! Because it tries to connect it to
        // others.
      }
    }
    ...
    // TODO: At this point I could optionally remove nodes that are too close to geometry. Pretty
    // quick. Currently
  ...
  public boolean connect(Node node1, Node node2) {
    ...
    if (map != null) {
      ...
      for (int i = 0; i < lines.length; i++) {
        if (lines[i].segDist(connection) < clearance) return false;
      }
    }
  ...
  /**
   * Adds a node to this set and connects it with a number of neighboring nodes. If it is unable to
   * find any neighbors it will return 0. This might occur because the node is outside of the
   * bounded area of the map. Note: The most FourWayGridMesh can connect a node to is four. If you
   * select a number larger than four, a maximum of four neighbors will be connected with the node.
   *
