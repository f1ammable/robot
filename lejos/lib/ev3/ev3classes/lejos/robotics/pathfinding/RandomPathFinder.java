...
/**
 * PathFinder that takes a map and a dummy set of range readings. It finds a path that is in short
 * moves, has no obstacles in the way and where valid range readings can be taken from each
 * waypoint.
 *
 * <p>The algorithm is not deterministic so each time it is called a new route will be found.
 *
...
public class RandomPathFinder implements PathFinder {
  ...
  /**
   * Set the clearance around the edge of the map. This does not really work as clearance shiould be
   * around all walls and obstacles.
   *
  ...
  public Path findRoute(Pose start, Waypoint destination) throws DestinationUnreachableException {
    ...
    for (; ; ) {
      ...
      for (i = 0; i < maxIterations; i++) {
        ...
        if (pose.distanceTo(destination) - testPose.distanceTo(destination) < minGain) continue;
        ...
        if (map.range(testPose) < testPose.distanceTo(pose.getLocation())) continue;
  ...
  private Pose generatePose() {
    ...
    Rectangle innerRect =
        new Rectangle(
            boundingRect.x + clearance,
            boundingRect.y + clearance,
            boundingRect.width - clearance * 2,
            boundingRect.height - clearance * 2);
