...
/**
 * This class represents a Node which can be connected to other neighboring nodes. Node sets can be
 * searched using search algorithms. Typically the search algorithm only requires one starting node
 * and one goal node. It assumes these nodes are linked by intermediate nodes.
 *
 * @author BB
...
public class Node {
  ...
  /**
   * The predecessor node used by A* search algorithm to mark off the previous node in the search
   * tree.
   */
  ...
  /**
   * Adds a neighboring node to this node, connecting them together. Note: You must make sure to add
   * this node to the neighbor, and also add the neighbor to this node. This method doesn't do both.
   *
   * @param neighbor The neighboring node to connect with.
   * @return Returns false if the neighbor already existed, or if you try to add this node to itself
   *     as a neighbor.
   */
  ...
  /**
   * Removes a node from this node as neighbors, effectively disconnecting them. Note: You have to
   * remove this node from the neighbor, and also remove the neighbor from this node. This method
   * doesn't do both.
   *
  ...
  /**
   * Method used by A* to calculate search score. The H score is the estimated distance to the goal
   * node from this node. It can either be distance "as the crow flies" or in the case of a grid
   * navigation mesh, the minimum number of grid spaces to get to the goal node (x squares
   * horizontally + y squares vertically from goal). NOTE: There is no getH_score() because the A*
   * algorithm only needs to set this value, not retrieve it.
   *
   * @param h
   */
  ...
  /**
   * Method used by A* to calculate search score. The G score is the cumulative distance from the
  ...
  /**
   * Method used by A* to calculate search score. You can't set FScore because it is derived
   * internally by adding the gscore and hscore.
   */
