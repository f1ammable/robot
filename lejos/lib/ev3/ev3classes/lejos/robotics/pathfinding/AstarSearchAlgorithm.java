...
// TODO: This works, but this code keeps the Node properties right in the Node object. The same Node
// set
// (aka Navigation Mesh) might conceivably (probably) be used repeatedly for many different
// searches. So things
// like setPredecessor() and setG_Score() should be temporary, not part of Node object?

/**
 * This is an implementation of the A* search algorithm. Typically this object would be instantiated
 * and then used in a NodePathFinder constructor, along with a set of connected nodes.
 *
...
public class AstarSearchAlgorithm implements SearchAlgorithm {
  ...
  public Path findPath(Node start, Node goal) {
    ...
    ArrayList<Node> closedset =
        new ArrayList<Node>(); // The set of nodes already evaluated. Empty at start.
    ...
    // ArrayList <Node> path = new ArrayList<Node>(); // came_from := the empty map // The map of
    // navigated nodes.
    start.setG_Score(
        0); // Distance from start along optimal path. Zero by definition since at start. g(start)
    // start.setH_Score((float)Point2D.distance(start.x, start.y, goal.x, goal.y)); // ORIGINAL
    // h(start)
    ...
    while (!openset.isEmpty()) {
      ...
      // TODO: Any mem saving/speedup not using iter? Could use array. Saves about 33 ms total, not
      // really worth it yet?
      ...
      while (yIter.hasNext()) { // for each y in neighbor_nodes(x)
        ...
        // float tentative_g_score = x.getG_Score() + (float)Point2D.distance(x.x, x.y, y.x, y.y);
        // // ORIGINAL g_score[x] + dist_between(x,y)
        ...
        // y.setH_Score((float)Point2D.distance(y.x, y.y, goal.x, goal.y)); // ORIGINAL
        // heuristic_estimate_of_distance(y, goal)
        // Update(closedset,y)  This might mean update the values of y in closedset and openset?
        // Unneeded I assume.
        // Update(openset,y)
      } // while yIter.hasNext()
    } // while main loop
  ...
  /**
   * Finds the node within a set of neighbors with the least cost (potentially shortest distance to
   * goal).
   *
  ...
  private static Node getLowestCost(Collection<Node> nodeSet) {
    ...
    while (nodeIterator.hasNext()) {
      ...
      if (cur.getF_Score() < best.getF_Score()) best = cur;
    }
    return best;
  }
   ...
   */
  private static final void reconstructPath(
      Node current_node, Node start, Collection<Waypoint> path) {
    if (current_node != start) reconstructPath(current_node.getPredecessor(), start, path);
