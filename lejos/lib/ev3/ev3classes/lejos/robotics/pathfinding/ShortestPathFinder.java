...
/**
 * This class calculates the shortest path from a starting point to a finish point. while avoiding
 * obstacles that are represented as a set of straight lines. The path passes through the end points
 * of some of these lines, which is where the changes of direction occur. Since the robot is not
 * point, the lines representing the obstacles should be lengthened so the actual robot will miss
 * the actual obstacles. Use the lengthenLines() method to do this. Uses modification of the A*
 * algorithm,which is a a variant of the Dijkstra shortest path algorithm. This variant adds nodes
 * needed. It uses the Node inner class for its internal representation of points.
 *
...
public class ShortestPathFinder implements PathFinder {
  ...
  /**
   * Finds the shortest path from start to finish using the map (or collection of lines) in the
   * constructor.
   *
  ...
  /**
   * Finds the shortest path from start to finish using the map ( collection of lines) in the
   * constructor.
   *
   * @param start the initial robot pose
   * @param finish the final robot location
  ...
  /**
   * Finds the shortest path between start and finish Points while avoiding the obstacles
   * represented by lines in the map
   *
   * @param start : the beginning of the path
   * @param finish : the destination
   * @param theMap that contains the obstacles
   * @return an array list of waypoints. If no path exists, returns null and throws an exception.
   */
  private Path findPath(Point start, Point finish, ArrayList<Line> theMap)
    ...
    while (!_reached.contains(_destination) && !failed) {
      ...
      float
          distance; // find node closest to source; check all combinations of reached and candidate
                    // nodes
      for (Node reached : _reached) {
        for (Node candidate : _candidate) {
          ...
          {
            distance = reached._sourceDistance + reached.getDistance(candidate);
      ...
      if (_debug) System.out.println("bestCandidate " + bestCandidate + " minDist " + minDistance);
      // is the line from bestReached to bestCandidate blocked?  The segmentBlocked method adds
      // nodes
      if (!segmentBlocked(bestReached, bestCandidate)) {
        ...
        if (_debug)
          System.out.println(
              "Moved from candidate to reached "
                  + bestCandidate
                  + " source dist "
                  + bestCandidate.getSourceDistance()
                  + " of "
                  + _candidate.size());
      } else { // line from bestReached to bestCandidate is blocked
        if (_debug)
          System.out.println("bestCandidate " + bestCandidate + " blocked from " + bestReached);
        bestReached.block(bestCandidate);
        bestCandidate.block(bestReached);
      }
      failed = minDistance >= BIG;
    } // end while
    if (failed) throw new DestinationUnreachableException();
    if (_debug) {
  ...
  /**
   * Helper method for findPath(). Determines if the straight line segment crosses a line on the
   * map. Side effect: creates nodes at the end of the blocking line and adds them to the _candidate
   * set
   *
  ...
  private boolean segmentBlocked(final Node from, final Node theDest) {
    ...
    for (Line l : _map) // test every line in the map to see if it blocks the segment
    {
      ...
      if (intersection != null
          && !from.atEndOfLine(l)
          && !to.atEndOfLine(
              l)) { // segment is not blocked if the intersection point is not a line end.
        line = l;
        blocked = true;
        break;
    ...
    // set
    {
      ...
      if (!inReachedSet(n1) && !inCandidateSet(n1)) {
        ...
        if (_debug)
          System.out.println(
              "Candidate add n1 " + n1 + " Source Distance " + n1.getSourceDistance());
      }
      ...
      if (!inReachedSet(n2) && !inCandidateSet(n2)) {
        ...
        if (_debug)
          System.out.println(
              "Candidate add n2 " + n2 + " Source Distance " + n2.getSourceDistance());
      }
    }
    return blocked;
  }
  ...
  public void setMap(LineMap theMap) {
    ...
    for (int i = 0; i < lines.length; i++) _map.add(lines[i]);
  }
  ...
  /**
   * the set of nodes that are candidates for being in the shortest path, but whose distance from
   * the start node is not yet known.
   */
  private ArrayList<Node> _candidate = new ArrayList<Node>();

  /**
   * the set of nodes that are candidates for being in the shortest path, and whose distance from
  ...
  public class Node {
    ...
    /**
     * Return the straight line distance from this node to aNode or a big number if the straight
     * line is known to be blocked
     *
     * @param aNode
     * @return the distance
     */
    private float getDistance(Node aNode) {
      if (isBlocked(aNode)) return BIG;
