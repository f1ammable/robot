...
/**
 * This path finder class uses one of the common search algorithms (e.g. A*) and a navigation mesh
 * (e.g. grid) and uses them to find a path around map geometry.
 *
 * @author BB
 */
public class NodePathFinder implements PathFinder {
  ...
  /**
   * Instantiates a NodePathFinder object using a specified algorithm. Ideally this class would work
   * with self propagating nodes, but it currently is not able to.
   *
   * @param alg
   */
  ...
  /**
   * Instantiates a NodePathFinder object using a specified algorithm. The supplied mesh is used to
   * add and remove nodes (start and goal) when requesting a path.
   *
    ...
    if (listeners != null) {
      for (WaypointListener l : listeners) {
        Iterator<Waypoint> iterator =
            solution.iterator(); // TODO: If solution null, this can crash here.
