 ...
 * the actual obstacles. Use the lengthenLines() method to do this. Uses the A* algorithm, a variant
 * of the Dijkstra shortest path algorithm It uses the Node inner class for its internal
...
public class DijkstraPathFinder implements PathFinder {
  ...
  private Path findPath(Point start, Point finish, ArrayList<Line> theMap)
    ...
    while (_candidate.size() > 0 && !failed) {
      ...
      } else { // dest is not known to be blocked  from  best  reached node
        if (segmentBlocked(
            from,
            dest)) { // this method call possibly created and added new nodes to the _candidate set
  ...
  protected boolean segmentBlocked(final Node from, final Node theDest) {
    ...
    for (Line l : _map) // test ever line in the map to see if it blocks the segment
    {
      ...
      if (intersection != null
          && !from.atEndOfLine(l)
          && !to.atEndOfLine(
              l)) { // segment is legal if it starts or ends at an end point of the line
        line = l;
        blocked = true;
   ...
   * returns the node in the Reached set, whose distance from the start node plus its straight line
   * distance to the destination is the minimum.
   *
