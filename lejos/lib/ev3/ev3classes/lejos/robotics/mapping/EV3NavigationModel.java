 ...
 *
 * <p>All local navigation objects, including pilots, navigators, path finders, feature detectors,
 * and range scanners can be added to the model.
 *
 * <p>Where possible, the model registers itself as an event listener and when the event occurs,
 * updates the model and sends the event and the updates to the PC.
 *
 * <p>A receiver thread receives events from the PC, updates the local model, and uses the
 * navigation objects to implement the event if it involves robot behaviour.
 *
 * <p>There are set methods to set various navigation parameters.
 *
 * @author Lawrie Griffiths
 */
public class EV3NavigationModel extends NavigationModel
  ...
  protected ArrayList<FeatureDetector> detectors =
      new ArrayList<FeatureDetector>(); // Multiple feature detectors allowed
  ...
  class Receiver implements Runnable {
            ...
            switch (navEvent) {
              ...
              case ARC: // Request to travel an arc og given radius and angle
                ...
                if (pilot != null && pilot instanceof ArcMoveController)
                  ((ArcMoveController) pilot).arc(radius, angle, false);
                break;
              case ROTATE_TO: // Request to rotate to a given angle
                angle = dis.readFloat();
                if (pp != null && pilot != null && pilot instanceof RotateMoveController)
                  ((RotateMoveController) pilot).rotate(angleTo(angle), false);
                break;
    ...
    private void randomMove() {
      if (pilot != null && pilot instanceof RotateMoveController) {
        ...
        if (forwardRange < 0 || distance + clearance < forwardRange) pilot.travel(distance, false);
