...
/**
 * This class loads map data from a Shapefile and produces a LineMap object, which can be used by
 * the leJOS navigation package.
 *
 * <p>There are many map editors which can use the Shapefile format (OpenEV, Global Mapper). Once
 * you have created a map, export it as Shapefile. This will produce three files ending in .shp .shx
 * and .dbf. The only file used by this class is .shp.
 *
 * <p>NOTE: Shapefiles can only contain one type of shape data (polygon or polyline, not both). A
 * single file can't mix polylines with polygons.
 *
 * <p>This class' code can parse points and multipoints. However, a LineMap object can't deal with
 * points (only lines) so points and multipoints are discarded.
 *
 * @author BB
 */
public class ShapefileLoader {
  ...
  public LineMap readLineMap() throws IOException {
    ...
    // System.out.println("Length: " + fileLength); // TODO: Docs say length is in 16-bit words.
    // Unsure if this is strictly correct. Seems higher than what hex editor shows.
    ...
    Rectangle rect =
        new Rectangle((float) xMin, (float) yMin, (float) (xMax - xMin), (float) (yMax - yMin));
    ...
    try { // TODO: This is a cheesy way to detect EOF condition. Not very good coding.
      while (2 > 1) { // TODO: Temp code to keep it looping. Should really detect EOF condition.
        ...
        int recordLen =
            data_is
                .readInt(); // TODO: in 16-bit words. Might cause bug if number of shapes gets
                            // bigger than 16-bit short?
        ...
        switch (recShapeType) {
          ...
          case POLYGON:
            // Polygons can contain multiple polygons, such as a donut with outer ring and inner
            // ring for hole.
            // Max bounding rect: 4 doubles in a row. TODO: Discard bounding rect. values and skip
            // instead.
            ...
            for (int i = 0; i < numPoints - 1; i++) {
              // Could check here if onto new polygon (i = next index). If so, do something with
              // line formation.
              ...
              Line myLine =
                  new Line((float) firstX, (float) firstY, (float) secondX, (float) secondY);
              lines.add(myLine);
              firstX = secondX;
              firstY = secondY;
            }
