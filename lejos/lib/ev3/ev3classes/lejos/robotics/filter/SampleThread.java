...
public class SampleThread extends AbstractFilter {
   ...
   *
   * @param source A SampleProvider
   * @param sampleRate The sample rate expressed in Hertz (Samples / second)
   */
  ...
  public synchronized void fetchSample(float[] dst, int off) {
    for (int axis = 0; axis < sampleSize; axis++) dst[axis + off] = buffer[axis];
    newSampleAvailable = false;
  }

  /**
   * Separate thread to continuously update the buffer with most recent sensor data at fixed
   * interval.
   *
  ...
  private class Runner extends Thread {
    ...
    public void run() {
      ...
      while (true) {
        ...
        if (currentTime < nextTime) Delay.msDelay(nextTime - currentTime);
      }
    }
  }
