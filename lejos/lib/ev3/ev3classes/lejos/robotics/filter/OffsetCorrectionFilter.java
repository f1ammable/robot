...
/**
 * The OffsetCorrectionFilter is used to correct sensors that have an unknown offset error.<br>
 * The offset error is calculated by this class and then substracted from the sample to give a
 * corrected sample. The filter works by calculating a running mean of a sample. this mean value is
 * compared to the reference value, the difference between the two is the offset error. Not all
 * samples are used to calculate the running mean. Only those samples that seem to indicate a stable
...
public class OffsetCorrectionFilter extends AbstractFilter {
   ...
   * All samples are corrected to a reference value of zero using a window of 200 samples for
   * calculating the mean
   *
   * @param source source for sample
   */
  ...
  /**
   * Constructor for the offset correction filter using default window of 200 samples. <br>
   * All samples are corrected to the specified reference values
   *
   * @param source source for sample
   * @param reference An array with reference values. The array length should match the sample size.
   */
   ...
   * @param source Source for sample
   * @param reference An array with reference values. The array length should match the sample size.
   * @param bufferSize Number of samples to use for calculating offset error
   */
  ...
  private boolean withinLimits(int i) {
    if (actual[i] == Float.NaN) return false;
    if (buffer[i].size() < 15) return true;
    float interval = 2 * this.getStandardDeviation(i);
    if (actual[i] < mean[i] - interval) return false;
    if (actual[i] > mean[i] + interval) return false;
    return true;
  }
  ...
  public float[] getStandardDeviation() {
    ...
    for (int i = 0; i < sampleSize; i++) {
      if (buffer[i].size() < 2) std[i] = Float.NaN;
      else std[i] = getStandardDeviation(i);
    }
    return std;
  }
}
