...
/**
 * Displays a list of items. The select() method allows the user to scroll the list using the right
 * and left keys to scroll forward and backward through the list. The location of the list , and an
 * optional title can be specified.
 *
...
public class TextMenu {
  ...
  /**
   * This constuctor allows the specfication of a title (of up to 16 characters) and the location of
   * the item list <br>
   * The title is displayed in the row above the item list.
   *
   * @param items - string array containing the menu items. No items beyond the first null will be
   *     displayed.
   */
  ...
  public void setTitle(String title) {
    ...
    _height = 8 - _topRow;
    if (_height > _length) _height = _length;
  }
  ...
  /**
   * Allows the user to scroll through the items, using the right and left buttons (forward and
   * back) The Enter key closes the menu <br>
   * and returns the index of the selected item. <br>
   * The menu display wraps items that scroll off the top will reappear on the bottom and vice
   * versa.
   *
   * <p>The selectedIndex is set to the first menu item.
   *
   ...
   * versa.
   *
   * <p>This version of select allows the selected index to be set when the menu is first displayed.
   *
