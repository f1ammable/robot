...
/**
 * Tetrix DC motor abstraction with encoder support that implements <code>RegulatedMotor</code>. The
 * Tetrix motor must have an encoder installed and connected to the controller for the methods in
 * this class to work. If an encoder is not installed, use the <code>{@link TetrixMotor}</code>
 * class instead.
 *
 * <p>Use <code>{@link TetrixMotorController#getRegulatedMotor}</code> to retrieve a <code>
 * TetrixRegulatedMotor</code> instance.
 *
...
public class TetrixRegulatedMotor extends TetrixEncoderMotor implements RegulatedMotor {
  ...
  /**
   * OVERRIDDEN TO NOT ALLOW CHANGE OF REGULATED STATE as the <code>TetrixRegulatedMotor</code>
   * class must use regulation. Motors are always in regulated mode when using the <code>
   * TetrixRegulatedMotor</code> class.
   *
  ...
  /**
   * Sets desired motor speed, in degrees per second. Since the TETRIX Motor Controller only has
   * power adjustment, the power value is derived from the passed <code>speed</code> value as: <br>
   *
   * <pre>  power = Math.round((speed - 0.5553f) * 0.102247398f);</pre>
   *
   * and as such, the actual speed value will not be exact.
   *
   * <p>The maximum reliably sustainable velocity for the TETRIX DC Gear motor P/N 739023 (which was
   * used as the test case for creating this class) is 154 RPM => 924 degs/sec.
   *
  ...
  /**
   * Return the speed value calculated from the actual power value as: <br>
   *
