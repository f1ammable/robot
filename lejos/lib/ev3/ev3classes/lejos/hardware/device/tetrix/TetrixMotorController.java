...
/**
 * HiTechnic Tetrix Motor Controller abstraction. Provides <code>TetrixMotor</code> and <code>
 * TetrixEncoderMotor</code> instances which are used to control the Tetrix motors.
 *
 * <p>Use <code>{@link TetrixControllerFactory#newMotorController}</code> to retrieve a <code>
 * TetrixMotorController</code> instance.
 *
...
public class TetrixMotorController extends I2CSensor {
  ...
  private static final int REG_ENCODERSREAD =
      0x4C; // used by tachmonitor to read both encoders at once
  ...
  /**
   * Instantiate a HiTechnic TETRIX Motor Controller connected to the given <code>port</code> and
   ...
   * @throws IllegalStateException if a Motor Controller was not found with given <code>port</code>
  ...
  public TetrixMotorController(I2CPort port, int daisyChainPosition) {
    ...
    // This thread will keep the controller active. Without I2C activity within 2.5 seconds, it
    // times out.
    // We could use the NTO bit of mode (MODEBIT_NTO) to keep the controller from timing
    // out but the motors would still run if the EV3 faulted, was shutdown, etc. which could be
    // unsafe with big,
            ...
            new Runnable() {
              public void run() {
                ...
                for (; ; ) {
                  ...
                  // let the thread die if we are constantly getting tachocounts as this will keep
                  // the contoller active instead
  ...
  // When a rotate is issued, an instance monitors the BUSY bit and sets STATE_STOPPED when the
  // command completes.
  // Also does any required RegulatedMotorListener STOP and notifies any wait() in
  // waitRotateComplete().
   ...
   *     Tetrix motor instance other than <code>TetrixMotor</code>.
   * @see lejos.hardware.device.tetrix.TetrixMotor
   * @see #getEncoderMotor
  ...
  /**
   * Get a <code>TetrixEncoderMotor</code> instance that is associated with the <code>motorID</code>
   * . The motor must have an encoder installed for a <code>TetrixEncoderMotor</code> instance to
   * work correctly.
   *
   * @param motorID The motor ID number. This is indicated on the HiTechnic Motor Controller and is
   *     represented using <code> {@link #MOTOR_1}</code> or <code> {@link #MOTOR_2}</code>.
   ...
   * @throws IllegalArgumentException if invalid <code>motorID</code>
   * @throws UnsupportedOperationException if <code>motorID</code> has already been used for a
   *     Tetrix motor instance other than <code>TetrixEncoderMotor</code>.
   * @see lejos.hardware.device.tetrix.TetrixEncoderMotor
   * @see #getBasicMotor
   * @see #getRegulatedMotor
   */
  ...
  /**
   * Get a <code>TetrixRegulatedMotor</code> instance that is associated with the <code>motorID
   * </code>. The motor must have an encoder installed for a <code>TetrixRegulatedMotor</code>
   ...
   * @throws IllegalArgumentException if invalid <code>motorID</code>
   * @throws UnsupportedOperationException if <code>motorID</code> has already been used for a
   *     Tetrix motor instance other than <code>TetrixRegulatedMotor</code>.
  ...
  private void setMode(int channel, boolean resetEncoder) {
    ...
    // if we set both the MODEBIT_SEL_SPEED and MODEBIT_SEL_POSITION, it equals
    // MODEBIT_SEL_RST_ENCODER and
    // cancels out the ROTATE command
    if (motorParams[MOTPARAM_REGULATED][channel] == MOTPARAM_OP_TRUE
  ...
  private void motorGo(int channel, int command) {
    ...
    motorState[channel] =
        command
            + 1; // STATE_RUNNING_FWD, STATE_RUNNING_BKWD assuming command
                 // IN(CMD_FORWARD,CMD_BACKWARD)
  ...
  private void initController() {
    ...
    for (int i = 0; i < CHANNELS; i++) {
      ...
      motorParams[MOTPARAM_REGULATED][i] =
          MOTPARAM_OP_FALSE; // 0=false=power control, 1=speed control
