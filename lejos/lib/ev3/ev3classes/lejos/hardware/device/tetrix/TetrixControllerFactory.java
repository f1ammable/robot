...
/**
 * HiTechnic Tetrix Motor and Servo Controller factory class used to provide Motor and Servo
 * controller abstractions. These are then used to obtain motor and servo instances respectively.
 * These abstraction classes are <code>TetrixMotorController</code> and <code>TetrixServoController
 * </code>.
 *
 * <p>Motor and servo controllers are enumerated starting at the controller connected to one of the
 * NXT's sensor ports and then working outwards along the daisy chain. 4 controllers can be
 * daisy-chained, with a mixture of servo and/or motor controllers. No other sensors can be
...
public class TetrixControllerFactory {
  ...
  private class Finder extends I2CSensor {
     ...
     * @return the index the product ID was found. -1 if not found or outside
     *     MAX_CHAINED_CONTROLLERS bounds
     */
    private int findProduct(int i, String product) {
      if (i < 0 || i >= MAX_CHAINED_CONTROLLERS) return -1;
      for (; i < MAX_CHAINED_CONTROLLERS; i++) {
        address = (i + 1) * 2;
        if (getVendorID().equalsIgnoreCase(TETRIX_VENDOR_ID)
            && getProductID().equalsIgnoreCase(product)) return i;
      }
      return -1;
    }
  }
  ...
  /**
   * Get the next available Tetrix Motor controller. Servo controllers in the daisy-chain (if any)
   * are skipped in the search.
   *
   * <p>Successive controllers in a daisy-chain logically go "outwards" from the controller closest
   * to the NXT as #1 to #4 for each controller in the chain. Once a specific, logical controller
   * has been retrieved using this method, it cannot be retrieved again.
   *
   * <p>A combination of Servo and Motor controllers can be daisy-chained.
   *
   * @return The next available <code>TetrixMotorController</code> instance.
   * @throws IllegalStateException If no more motor controllers can be returned. If there are no
   *     motor controllers in the daisy-chain, this exception is also thrown.
  ...
  public TetrixMotorController newMotorController() {
    ...
    TetrixMotorController mc =
        new TetrixMotorController(finder.getPort(), (this.currentMotorIndex + 1) * 2);
    this.currentMotorIndex++;
    return mc;
  }

  /**
   * Get the next available Tetrix Servo controller. Motor controllers in the daisy-chain (if any)
   * are skipped in the search.
   *
   * <p>Successive controllers in a daisy-chain logically go "outwards" from the controller closest
   * to the NXT as #1 to #4 for each controller in the chain. Once a specific, logical controller
   ...
   * @throws IllegalStateException If no more servo controllers can be returned. If there are no
   *     servo controllers in the daisy-chain, this exception is also thrown.
  ...
  public TetrixServoController newServoController() {
    ...
    TetrixServoController sc =
        new TetrixServoController(finder.getPort(), (this.currentServoIndex + 1) * 2);
