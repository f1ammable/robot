...
/**
 * Tetrix DC motor abstraction with encoder support. The Tetrix motor must have an encoder installed
 * and connected to the controller for the methods in this class to work. If an encoder is not
 * installed, use the <code>{@link TetrixMotor}</code> class instead.
 *
 * <p>Use <code>{@link TetrixMotorController#getEncoderMotor}</code> to retrieve a <code>
 * TetrixEncoderMotor</code> instance.
 *
...
public class TetrixEncoderMotor extends TetrixMotor implements Encoder {
  ...
  /**
   * Reset the the tachometer count. Calling this method will stop any current motor action. This is
   * imposed by the HiTechic Motor Controller firmware.
   */
  ...
  /**
   * Disable or Enable internal motor controller speed regulation. Setting this to <code>true</code>
   * will cause the motor controller firmware to adjust the motor power to compensate for changing
   * loads in order to maintain a constant motor speed. Default at instantiation is <code>false
   * </code>.
   *
