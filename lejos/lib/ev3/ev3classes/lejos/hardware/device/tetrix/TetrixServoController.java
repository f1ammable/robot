...
/**
 * HiTechnic Servo Controller abstraction. Provides <code>TetrixServo</code> instances which are
 * used to control the Tetrix servos.
 *
 * <p>Servos are driven by a PWM signal with varying pulse widths controlling the rotational
 * position of the servo actuator. The pulse nominally ranges from 1.0 ms to 2.0 ms with 1.5 ms
 * always being center of range. Pulse widths outside this range can be used for "overtravel"
 * -moving the servo beyond its normal range.
 *
 * <p>As an example, for a servo with a 90 deg. travel range, a pulse width of 1.5 ms (1500
 * microseconds) will typically set the servo to its "neutral" position or 45 degrees, a pulse of
 * 1.25 ms could set it to 0 degrees and a pulse of 1.75 ms to 90 degrees. The physical limits and
 * timings of the servo hardware varies between brands and models, but a general 90 degree servo's
 * angular motion will travel somewhere in the range of 90 deg. - 120 deg. and the neutral position
 * is almost always at 1.5 ms. This is the "standard pulse servo mode" used by all hobby analog
 * servos.
 *
 * <p>The HiTechic Servo Controller allows setting of the PWM output from 0.75 - 2.25ms. Note that
 * some servos may hit their internal mechanical limits at each end of this range causing them to
 * consume excessive current and <b>potentially be damaged</b>.
 *
 * <p>Use <code>{@link TetrixControllerFactory#newServoController}</code> to retrieve a <code>
 * TetrixServoController</code> instance.
 *
...
public class TetrixServoController extends I2CSensor {
  ...
  private static final float PULSE_RESOLUTION =
      255f / (CONTROLLER_PULSE_RANGE_HIGH - CONTROLLER_PULSE_RANGE_LOW);
  ...
  /**
   * Instantiate for a HiTechnic TETRIX Servo Controller connected to the given <code>port</code>
   ...
   * @throws IllegalStateException if a Servo Controller was not found with given <code>port</code>
   *     and <code>daisyChainPosition</code>
   */
  public TetrixServoController(I2CPort port, int daisyChainPosition) {
    ...
    initController();
    // This thread will keep the controller active. Without I2C activity within 10 seconds, it times
    // out.
  ...
  /**
   * Get the <code>TetrixServo</code> instance that is associated with the passed <code>servoID
   * </code>.
   *
   * @param servoID The motor ID number <code>SERVO_1</code> to <code>SERVO_6</code>. This is
   *     indicated on the HiTechnic Servo Controller.
   ...
   */
  synchronized void setPulseRange(int channel, int microsecLOW, int microsecHIGH, int travelRange)
  ...
  synchronized void setPulseWidth(int channel, int pulseWidth) throws IllegalArgumentException {
    ...
    workingByte =
        (byte) (Math.round((pulseWidth - CONTROLLER_PULSE_RANGE_LOW) * PULSE_RESOLUTION) & 0xff);
    setPulse(channel, workingByte);
  }

  synchronized int getPulseWidth(int channel) {
    return (int)
        ((float) servoParams[SRVOPARAM_PULSEBYTE][channel] / PULSE_RESOLUTION
            + CONTROLLER_PULSE_RANGE_LOW);
  }

  synchronized float getAngle(int channel) {
    float servoResolution =
        (servoParams[SRVOPARAM_PULSEWIDTH_RANGE_HIGH][channel]
                - (float) servoParams[SRVOPARAM_PULSEWIDTH_RANGE_LOW][channel])
            / servoParams[SRVOPARAM_ROTATION_RANGE][channel];
    float angle =
        (servoParams[SRVOPARAM_PULSEBYTE][channel]
                - (float) servoParams[SRVOPARAM_PULSEWIDTH_RANGE_LOW][channel] * PULSE_RESOLUTION
                + (float) CONTROLLER_PULSE_RANGE_LOW * PULSE_RESOLUTION)
            / (servoResolution * PULSE_RESOLUTION);
    return angle;
  }
  ...
  synchronized void setAngle(int channel, float angle) {
    ...
    // add offset between servo low range and contoller low range to determine actual target pulse
    // width
    ...
    int theByte =
        (int)
            ((servoResolution * angle
                    + (servoParams[SRVOPARAM_PULSEWIDTH_RANGE_LOW][channel]
                        - CONTROLLER_PULSE_RANGE_LOW))
                * PULSE_RESOLUTION);
  ...
  /**
   * Returns whether or not there are servos on this controller that are moving. This method always
   * returns <code>false</code> when the step time is set to 0 (disabled).
   *
  ...
  /**
   * Set all servos connected to this controller to float mode. This means they are powered down and
   * will not attempt to hold their current position. The next call to any motion command for any
   * servo will re-enable power to all servo channels.
   */
  ...
  /**
   * Sets the step time used for all servos on this controller. This sets the step time for the
   * servo channel which has the furthest to move. Other servo channels which are not at their
   * designated positions yet will run at a slower rate to ensure they reach their destination
   * positions at the same time. The controller defaults to 0 on power-up.
   *
   * <p>The step time can be considered a delay before progressing to the next step. For example, if
   * a servo is positioned at 1500 microseconds pulse width, and you give it a new position command
   * of 2000 microseconds, it will normally go as fast as it can to get to the new position. If you
   * want it to go to the new position but not at the maximum output, you can set the step to a
   * value from 0 to 15.
   *
   * <p>One of the main things it could be useful for, is if you have two servos with different
   * loads, and you want them to be as much in sync as possible. You can set the speed to slow the
   * controller from changing the servo signals instantly.
   *
   * <p>The <code>isMoving()</code> method always returns <code>false</code> if the step time is set
   * to zero.
   *
