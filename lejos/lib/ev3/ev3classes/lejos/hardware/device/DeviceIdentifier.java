...
public class DeviceIdentifier extends Device implements EV3SensorConstants {
  ...
  protected static List<AbstractMap.SimpleEntry<Integer, String>> EV3AnalogMap =
  ...
  public void close() {
    if (configPort != null) configPort.close();
    super.close();
  }
  ...
  protected void waitValid() {
    if (configPort == null) openConfigPort();
    long minDelay = (openTime + MIN_TIME) - System.currentTimeMillis();
    if (minDelay > 0) Delay.msDelay(minDelay);
  ...
  /**
   * Get the type classification for the port. If a sensor is attached to the port this will
   * identify the connection type (CONN_NONE, CONN_INPUT_UART etc.). See the class
   * EV3SensorConstants for the actual values
   *
   ...
   * Note that only very basic sensor identification information may be available for some sensor
   * types. It may be necessary to actually open the sensor to allow it to be identified in further
   * detail.
   *
  ...
  protected String getI2CSignature(boolean full) {
    ...
    try {
      ...
      for (int i = 2; i < 255; i += 2) {
        ...
        if (product.length() != 0) {
          if (i > 2) address = String.valueOf(i);
          break;
        }
      }
    ...
    if (product.length() == 0) product = "unknown";
  ...
  /**
   * return the signature of the attached device. This signature can be used to identify the actual
   * device. Note that identification may require that the device is opened.
   *
