 ...
 *
 * <p>The dLight is a RGB color LED connected to a sensor port. Up to four dLights can be daisy
 * chained on a single port. The individual dLights are identified within this class by a sequence
 * number (1-4). All dLights can be addressed simultaniously as a group by using sequence number 0.
 *
 * <p>The DLights class supports two different color schemes for instructing the dLights; RGB and
 * HSL. It supports just RGB for querying the dLights
 *
...
public class DLights {
   ...
   *
   * <p>Nb. It also enables HIGH-SPEED I2C on this port.
   *
   * @param port The I2CPort that the dLights are attached to.
   */
  public DLights(I2CPort port) {
    for (int i = 0; i < 5; i++) dLights[i] = new LED(port, address[i]);
  }
   ...
   *
   * <p>Nb. For ordinary use there is no need to use the I2C driver for a single dLight that this
   * method returns. The method and the DLight instance it returns are nly useful if one needs to
   * access the dLight directly over I2C.
   *
   * @param lightNo The sequence number of the dLight (1-4). Please note that the group number 0 is
   *     invalid for this method.
   * @return the I2C driver for a single dLight
   */
  public DLight getDLight(int lightNo) {
    if (1 > lightNo || lightNo > 4) throw new IllegalArgumentException();
  ...
  public void enable(int lightNo) {
    if (0 > lightNo || lightNo > 4) throw new IllegalArgumentException();
    dLights[lightNo].enable();
  }

  /**
   * Disables the dLight. The light will be off, no matter what values for color are sent to the
   * dLight.
   *
   * <p>Values for color and blinking pattern are not overwritten and will still be in effect after
   * enabling the dLight again.
   *
   * @param lightNo The sequence number of the dLight (0-4).
   */
  ...
  /**
   * Changes the color of the LED according to specified RGB color. Each of the color values should
   * be between 0 (fully off) and 255 (fully on).
   *
   * @param lightNo The sequence number of the dLight (0-4).
   * @param red
   * @param green
  ...
  /**
   * Specifies the blinking pattern of the LED. Blinnking mode should be enabled for the pattern to
   * be in effect.
   *
   * @param lightNo The sequence number of the dLight (0-4).
  ...
  public void setBlinkingPattern(int lightNo, float seconds, int percentageOn) {
    if (0 > lightNo
        ...
        || seconds < 0.04f
        || percentageOn < 0
        || percentageOn > 100) throw new IllegalArgumentException();
    dLights[lightNo].setBlinkingPattern(seconds, percentageOn);
  }

  /**
   * Enables blinking pattern. The blinking pattern should be set with the SetBlinkingPattern
   * method.
   *
   * @param lightNo The sequence number of the dLight (0-4).
   */
  ...
  /**
   * Returns the RGB color of the LED Each of the color values is between 0 (fully off) and 255
   * (fully on).
   *
   * @param lightNo The sequence number of the dLight (1-4). Please note that the group number 0 is
   *     invalid for this method.
  ...
  /**
   * Sets the PWM value of the external LED driver of the dLight. Each dLight has two leads broken
   * out that can be used to drive an external LED.
   *
   * @param lightNo The sequence number of the dLight (0-4).
  ...
  /**
   * Gets the PWM value of the external LED driver of the dLight. Each dLight has two leads broken
   * out that can be used to drive an external LED.
   *
   * @param lightNo The sequence number of the dLight (1-4). Please note that the group number 0 is
   *     invalid for this method.
  ...
  /**
   * This is the I2C driver for a single (or group of) dLights. If this class represents a group
   * then all dLights respond to it's methods.
   *
   * @author Aswin
   */
  private class LED extends I2CSensor implements DLight {
    ...
    public void setColor(int[] rgb) {
      for (int i = 0; i < 3; i++) buf[i] = (byte) (rgb[i] % 256);
      sendData(COLOR + AUTOINCREMENT, buf, 3);
    }
