...
/**
 * Abstraction to drive a regulated encoder motor with the NXTMMX motor multiplexer. The NXTMMX
 * motor multiplexer device allows you to connect two additional motors to your robot using a sensor
 * port. Multiple NXTMMXs can be chained together.
 *
 * <p>Use the <code>NXTMMX.getRegulatedMotor()</code> factory method to retrieve an instance of this
 * class.
 *
 * @see NXTMMX
...
public class MMXRegulatedMotor extends MMXMotor implements RegulatedMotor {
  ...
  void doListenerState(int listenerState) {
    synchronized (lockObj) {
      ...
      if (listenerState == LISTENERSTATE_STOP) {
        ...
        //				public void run() {
        //					waitComplete();
        //					listener.rotationStopped(MMXRegulatedMotor.this, getTachoCount(), false,
        // System.currentTimeMillis());
        //				}}).start();
        listener.rotationStopped(
            MMXRegulatedMotor.this, getTachoCount(), false, System.currentTimeMillis());
      } else {
  ...
  /**
   * Return the current rotational speed calculated from the encoder position every 100 ms. This
   * will likely differ from what was specified in <code>setSpeed</code>.
   *
  ...
  /**
   * Rotate by the requested number of degrees with option for wait until completion or immediate
   * return where the motor completes its rotation asynchronously.
   *
   * @param degrees number of degrees to rotate relative to the current position.
   * @param immediateReturn if <code>true</code>, do not wait for the move to complete. <code>false
   *     </code> will block until the rotation completes.
   */
  ...
  /**
   * Rotate to the target angle with option for wait until completion or immediate return where the
   * motor completes its rotation asynchronously.
   *
   * @param limitAngle Angle [in degrees] to rotate to.
   * @param immediateReturn if <code>true</code>, do not wait for the move to complete. <code>false
   *     </code> will block until the rotation completes.
   ...
   *
   * <p>The NXTMMX does not provide speed control per se (just power) so we approximate the power
   * value used based on the requested degress/sec (dps) passed in <code>speed</code>. This means if
   * you request 400 dps, the actual dps value may not reflect that. Setting speed during a rotate
   * method will have no effect on the running rotate but will on the next rotate method call.
   *
   * <p>experimental data gives: dps=8.1551*power+32.253 (unloaded @ 8.83V)
   *
   * <p><b>Note:</b>The NXTMMX doesn't seem to want to drive the standard NXT motor below ~40 dps.
   *
  ...
  /**
   * Sets speed ramping is enabled/disabled for this motor. The <code>RegulatedMotor</code>
   * interface specifies this in degrees/sec/sec but the NXTMMX does not allow the rate to be
   * changed, just if the motor uses smooth acceleration or not so we use the <code>acceleration
   * </code> parameter to specify ramping state.
   *
   * <p>Default at instantiation is ramping enabled.
   *
