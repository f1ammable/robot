 ...
 *
 * <p>Create an instance of this class and use the factory methods to provide a <code>
 * MMXRegulatedMotor</code> or <code>MMXMotor</code> instance.
 *
...
public class NXTMMX extends I2CSensor {
  ...
  // common registers
  static final int REG_ENCODERSREAD =
      REGISTER_MAP[REG_IDX_ENCODER_CURRENT][
          MOTOR_M1]; // used by tachmonitor to read both encoders at once
  ...
  private static final int MOTPARAM_ENCODER_HOLD =
      3; // 1=lock to position after rotation completion, 0=don't
  private static final int MOTPARAM_REGULATE =
      4; // 1=set CMDBIT_SPEED_MODE on command issue, 0=don't
  ...
  // When a rotate is issued, an instance monitors the status bits and sets STATE_STOPPED when the
  // command completes.
    ...
    public void run() {
      ...
      if (motorState[channel] != STATE_STOPPED) {
        ...
        if (motorType[channel] == MOTTYPE_REGULATED) {
          ((MMXRegulatedMotor) motors[channel])
              .doListenerState(MMXRegulatedMotor.LISTENERSTATE_STOP);
        }
      }
   ...
   * WAITS are done in the MMXRegulatedMotor class
   *
    ...
    switch (command) {
      ...
      case CMD_BACKWARD:
        ...
        if (motorType[channel] == MOTTYPE_REGULATED) {
          ...
          ((MMXRegulatedMotor) motors[channel])
              .doListenerState(MMXRegulatedMotor.LISTENERSTATE_START);
        }
      ...
      case CMD_ROTATE_TO:
        if (motorType[channel] == MOTTYPE_REGULATED) {
          if (motorState[channel] != STATE_STOPPED) {
            ...
            //                		sendData(REG_MMXCOMMAND,
            // MMXCOMMAND_MAP[MMXCOMMAND_IDX_BRAKE][channel]);
  ...
  /**
   * sets motor command mask to the NXTMMX motor command register with appropriate bit masks set on
   * passed command value.
   *
  ...
  private void commandMotor(int channel, int maskValue) {
    ...
    if (motorParams[MOTPARAM_REGULATE][channel] == MOTPARAM_OP_TRUE)
      workingByte |= CMDBIT_SPEED_MODE;
