...
/**
 * Interface for the Mindsensors Touch Multiplexer. This device allows up to three touch sensors to
 * be attached to a single NXT sensor port.
 *
 * @author Andy
 */
public class TouchMUX extends AnalogSensor {
  ...
  private class MuxTouchSensor implements Touch {
    ...
    /**
     * Create an object to represent one touch sensor attached to the multiplexer.
     *
   ...
   * Returns ID_T1 if T1 is pressed, ID_T2 if T2 is pressed and ID_T3 id T3 is pressed. If more then
   * one sensor is pressed the return will be an or of the values.
   *
  ...
  public int readSensors() {
    ...
    val = 1023 - val
     */
    if (L1 <= val && val < H1) ret = ID_T1;
    else if (L2 <= val && val < H2) ret = ID_T2;
    else if (L3 <= val && val < H3) ret = ID_T3;
    else if (L12 <= val && val < H12) ret = ID_T1 | ID_T2;
    else if (L13 <= val && val < H13) ret = ID_T1 | ID_T3;
    else if (L23 <= val && val < H23) ret = ID_T2 | ID_T3;
    else if (L123 <= val && val < H123) ret = ID_T1 | ID_T2 | ID_T3;
    else ret = 0;
    return ret;
  }
}
