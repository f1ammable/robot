...
/**
 * This class has been defined to manage the device LSC, Lattebox Servo Controller which manage
 * until 10 RC Servos / DC Motors
 *
...
public class LSC extends I2CSensor {
   ...
   * @throws ArrayIndexOutOfBoundsException
   */
  public void addServo(int location, String name, int min_angle, int max_angle)
      throws ArrayIndexOutOfBoundsException {
    if (arrServo.size() <= MAXIMUM_SERVOS_DCMOTORS) {
      LServo s =
          new LServo(this.portConnected, location, name, this.SPI_PORT, min_angle, max_angle);
      arrServo.add(s);
    } else {
      // throw new ArrayIndexOutOfBoundsException(ERROR_SERVO_DEFINITION);
      throw new ArrayIndexOutOfBoundsException();
