...
/**
 * This class allows you to use a Sony Playstation 2 controller to control your robot in conjunction
 * with the Mindsensors.com PSP-Nx interface. The controller has 2 analog joysticks and 16 buttons.
...
public class PSPNXController extends I2CSensor {
  ...
  /* Device Registers */
  /** BUTTON_1 and _2 combine to provide status for 16 buttons (8 button states per byte) */
  ...
  public void powerUp(boolean activate) {
    if (activate) setMode(ENERGIZED);
    else setMode(DE_ENERGIZED);
  }
  ...
  public void setDigitalMode(boolean activate) {
    if (activate) setMode(SET_DIGITAL_MODE);
    else setMode(SET_ANALOG_MODE);
  }
  ...
  public void setADPAMode(boolean activate) {
     ...
     */
    if (activate) setMode(SET_ADPA_MODE_ON);
    else setMode(SET_ADPA_MODE_OFF);
  }
}
