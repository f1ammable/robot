...
public class MindSensorsNumPad extends I2CSensor {
  ...
  /**
   * Constructor for objects of the NumericPad of Mindsensors. It assumes that you didn't change the
   * I²C address of the device.
   *
   * @param port the port number (SensorPort.S1 ... SensorPort.S4) to which the numeric pad is
   *     connected.
   */
  ...
  private void sendGroup(int reg, String data) {
    int len = data.length();
    byte[] b = new byte[len];
    for (int i = 0; i < len; i++) b[i] = (byte) data.charAt(i);
    this.sendData(reg, b, 0, len);
  }

  /**
   * Returns a Bitmask. Bits 0 to 9 correspond to the Number Keys 0 to 9. Bit 10 corresponds to the
   * star and Bit 11 to the hash key.
   *
   ...
   * returned as a bitmask. The lower twenve bits (bits 0 to 11) indicate, which buttons have been
   * pressed. Bits 16 to 27 indicate which buttons have been released.
   *
   * @return the bitmask
   */
  ...
  public int waitForAnyEvent(int timeout) {
    ...
    while (true) {
      ...
      if (newDown == ERROR) newDown = oldDown;
      ...
      if (toWait > POLL_INTERVAL) toWait = POLL_INTERVAL;
  ...
  public int waitForAnyPress(int timeout) {
    ...
    while (true) {
      ...
      curButtons = newDown;
      int pressed = newDown & (~oldDown);
      if (pressed != 0) return rawToOrdered(pressed);
  ...
  /**
   * The lower 12 bits (Bits 0 to 11) are converted to a string containing 0 to 9, *, and #
   * depending on whether which of bits are set. The returned string contains these character in
   * that order.
   */
  public static String maskToString(int x) {
    StringBuilder sb = new StringBuilder(12);
    for (int i = 0; i < 10; i++) if ((x & (1 << i)) != 0) sb.append((char) ('0' + i));
    if ((x & ID_STAR) != 0) sb.append('*');
    if ((x & ID_HASH) != 0) sb.append('#');
    return sb.toString();
  }
}
