...
public class Sound implements Sounds {
   ...
   * This function plays a more musical sounding note compared to playTone(). It uses a set of
   * supplied "instrument" parameters to define the shape of the note's envelope.
   *
   * <p>Instruments are defined in the Sounds interface, which is inherited by this class. For
   * example, the piano instrument array looks like this: <code>
   * public final static int[] PIANO = new int[]{4, 25, 500, 7000, 5};</code>
   *
   * <p>The parameter len is the total time of the note to be played val 0 is the attack time the
   * time for the volume to go from near zero to 100% val 1 is the decay time during this period the
   * volume decreases by val[2]/100 to the sustain level. the sustain period is (len - val[0] -
   * val[1] - val[4]) during this period the volume decreases by val[3]/100. the final decay period
   * is val[4] and the volume reduces to zero. All of the times are units of 2mS (except for len
   * which is in mS).
   *
