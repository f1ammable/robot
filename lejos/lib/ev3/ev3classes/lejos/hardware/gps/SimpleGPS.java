...
/**
 * This class manages data received from a GPS Device. SimpleGPS Class manages the following NMEA
 * Sentences which supply location, heading, and speed data:
...
public class SimpleGPS extends Thread {
  ...
  // Listener-notifier
  private static ArrayList<GPSListener> listeners = new ArrayList<GPSListener>();
   ...
   * this number at: http://en.wikipedia.org/wiki/List_of_GPS_satellite_launches Note: This number
   * might be similar or identical to SVN.
   *
   ...
   * together in the sky, the geometry is said to be weak and the DOP value is high; when far apart,
   * the geometry is strong and the DOP value is low. Thus a low DOP value represents a better GPS
   * positional accuracy due to the wider angular separation between the satellites used to
   * calculate a GPS unit's position. Other factors that can increase the effective DOP are
   * obstructions such as nearby mountains or buildings.
   *
   * @return The PDOP (PDOP * 6 meters = the error to expect in meters) -1 means PDOP is unavailable
   *     from the GPS.
   */
  ...
  /**
   * Get the Horizontal Dilution of Precision (HDOP). When visible GPS satellites are close together
   * in the sky, the geometry is said to be weak and the DOP value is high; when far apart, the
   * geometry is strong and the DOP value is low. Thus a low DOP value represents a better GPS
   * positional accuracy due to the wider angular separation between the satellites used to
   * calculate a GPS unit's position. Other factors that can increase the effective DOP are
   * obstructions such as nearby mountains or buildings.
   *
   * @return the HDOP (HDOP * 6 meters = the error to expect in meters) -1 means HDOP is unavailable
   *     from the GPS.
   */
  ...
  /**
   * Get the Vertical Dilution of Precision (VDOP). When visible GPS satellites are close together
   * in the sky, the geometry is said to be weak and the DOP value is high; when far apart, the
   * geometry is strong and the DOP value is low. Thus a low DOP value represents a better GPS
   * positional accuracy due to the wider angular separation between the satellites used to
   * calculate a GPS unit's position. Other factors that can increase the effective DOP are
   * obstructions such as nearby mountains or buildings.
   *
   * @return the VDOP (VDOP * 6 meters = the error to expect in meters) -1 means VDOP is unavailable
   *     from the GPS.
   */
  ...
  /**
   * Method used to close connection. There is no real need to call this method. Included in case
   * programmer wants absolutely clean exit.
   */
  ...
  /**
   * Keeps reading sentences from GPS receiver stream and extracting data. This is a daemon thread
   * so when program ends it won't keep running.
   */
  public void run() {
    while (!close) {
      ...
      int checksum1 = 0;
      for (int i = 1; i < p; i++) checksum1 ^= s.charAt(i);
