...
public class BrickFinder {
  ...
  /**
   * Internal class to implement a name server that can respond to queries generated by discover and
   * find methods. User code will not normally need to run an instance of this as the leJOS menu
   * will usually do so.
   *
   * @author andy
   */
      ...
      try {
        ...
        while (true) {
          ...
          if (args.length == 5 && args[0].equalsIgnoreCase(FIND_CMD)) {
            ...
            if ((!forward || hops == MAX_HOPS)
                && (args[1].equalsIgnoreCase("*") || args[1].equalsIgnoreCase(hostname))) {
              byte[] sendData = hostname.getBytes();
              // System.out.println("Send response to " + replyAddr);
              // Send a response
              DatagramPacket sendPacket =
                  new DatagramPacket(sendData, sendData.length, replyAddr, replyPort);
              try {
  ...
  private static void broadcastFindRequest(
      DatagramSocket socket, String name, InetAddress replyAddr, int replyPort, int hop) {
    try {
      ...
      while (interfaces.hasMoreElements()) {
        ...
        for (InterfaceAddress interfaceAddress : networkInterface.getInterfaceAddresses()) {
          ...
          if (replyAddr == null)
            message +=
                " " + interfaceAddress.getAddress().getHostAddress() + " " + socket.getLocalPort();
          else message += " " + replyAddr.getHostAddress() + " " + replyPort;
          message += " " + hop;
          ...
          try {
            // System.out.println("Send to " + broadcast.getHostAddress() + " port " +
            // DISCOVERY_PORT );
            DatagramPacket sendPacket =
                new DatagramPacket(sendData, sendData.length, broadcast, DISCOVERY_PORT);
            socket.send(sendPacket);
          } catch (Exception e) {
            System.err.println(
                "Exception sending to : " + networkInterface.getDisplayName() + " : " + e);
          }
        }
      }
  ...
  /**
   * Search for a named EV3. Return a table of the addresses that can be used to contact the device.
   * An empty table is returned if no EV3s are found.
   *
   * @param name
  ...
  /**
   * Start the discovery server running. There should be a single discovery server running on each
   * EV3. This provides responses to remote discover and find requests. Normally this server will be
   * run by the leJOS menu and so user code does not need to start a copy.
   *
   * @param forward true if requests should be forwarded to other devices
   */
