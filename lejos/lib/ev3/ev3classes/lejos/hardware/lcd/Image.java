...
/**
 * Provides support for in memory images. The format of the bitmap is in standard leJOS format (so
 * aligned for use on EV3 LCD display). There is one bit per pixel. The pixels are packed into bytes
 * with each byte spanning 8 scan lines. The least significant bit of each byte is the pixel for the
 * top most scan line, the most significant bit is the 8th scan line. Values of 1 represent black. 0
 * white. This class implements a sub set of the standard lcdui Image class. Only mutable images are
 * supported and the ARGB methods are not available.
 *
 * <p>TODO: This file needs to be updated to match the EV3 image format.
 *
...
public class Image implements Serializable {
  ...
  /**
   * Create an image using an already existing byte array. The byte array is used to store the image
   * data. The array may already be initialized with image data. <br>
   ...
   * </table>
   *
   * <p>For example: After a file with content
   *
   * <table border="1">
   * <tr>
   * <th>width (int)</th>
   ...
   * </table>
   *
   * was read, this method will return an object which is equivalent to <div
   * style="margin-left:4em;"><code>new Image(3, 5, new byte[] {(byte)0x00, (byte)0x02, (byte)0x1f})
   * </code></div>
   *
  ...
  public static Image createImage(InputStream s) throws IOException {
    ...
    if (p != LNI0_HEADER) throw new IOException("File format error!");
  ...
  /**
   * Return the byte array used to hold the image data. <br>
