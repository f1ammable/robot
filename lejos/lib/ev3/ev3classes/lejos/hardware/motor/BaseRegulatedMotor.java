...
/**
 * Abstraction for a Regulated motor motor. The basic control methods are: <code>
 * forward, backward, reverseDirection, stop</code> and <code>flt</code>. To set each motor's
 * velocity, use {@link #setSpeed(int) <code>setSpeed </code> }. The maximum velocity of the motor
 * is limited by the battery voltage and load. With no load, the maximum degrees per second is about
 * 100 times the voltage (for the large EV3 motor). <br>
 * The velocity is regulated by comparing the tacho count with velocity times elapsed time, and
 * adjusting motor power to keep these closely matched. Changes in velocity will be made at the rate
 * specified via the <code> setAcceleration(int acceleration)</code> method. The methods <code>
 * rotate(int angle) </code> and <code>rotateTo(int ange)</code> use the tachometer to control the
 * position at which the motor stops, usually within 1 degree or 2.<br>
 * <br>
 * <b> Listeners.</b> An object implementing the {@link lejos.robotics.RegulatedMotorListener <code>
 * RegulatedMotorListener </code> } interface may register with this class. It will be informed each
 * time the motor starts or stops. <br>
 * <b>Stall detection</b> If a stall is detected or if for some other reason the speed regulation
 * fails, the motor will stop, and <code>isStalled()</code > returns <b>true</b>. <br>
 * Motors will hold their position when stopped. If this is not what you require use the flt()
 * method instead of stop(). <br>
 *
 * <p>Example:
 *
 * <p><code><pre>
 ...
 * </pre></code> TODO: Fix the name
 *
 * @author Roger Glassey/Andy Shaw
 */
public abstract class BaseRegulatedMotor extends Device implements RegulatedMotor {
  // Following should be set to the max SPEED (in deg/sec) of the motor when free running and
  // powered by 9V
  ...
  public BaseRegulatedMotor(
    ...
    if (regulator == null) reg = port.getRegulator();
    else reg = regulator;
  ...
  public BaseRegulatedMotor(
      ...
      int maxSpeed) {
    this(
        port.open(TachoMotorPort.class),
        regulator,
        typ,
        moveP,
        moveI,
  ...
  /**
   * Removes this motor from the motor regulation system. After this call the motor will be in float
   * mode and will have stopped. Note calling any of the high level move operations (forward, rotate
   * etc.), will automatically enable regulation.
   *
  ...
  /**
   * Returns the current position that the motor regulator is trying to maintain. Normally this will
   * be the actual position of the motor and will be the same as the value returned by
   * getTachoCount(). However in some circumstances (activeMotors that are in the process of
   * stalling, or activeMotors that have been forced out of position), the two values may differ.
  ...
  /**
   * Set the motor into float mode. This will stop the motor without braking and the position of the
   * motor will not be maintained.
   */
  public void flt() {
    reg.newMove(0, acceleration, NO_LIMIT, false, true);
  }
  ...
  /**
   * Causes motor to stop, pretty much instantaneously. In other words, the motor doesn't just stop;
   * it will resist any further motion. Cancels any rotate() orders in progress
   */
  public void stop() {
  ...
  /**
   * This method returns <b>true </b> if the motor is attempting to rotate. The return value may not
   * correspond to the actual motor movement.<br>
   * For example, If the motor is stalled, isMoving() will return <b> true. </b><br>
   * After flt() is called, this method will return <b>false</b> even though the motor axle may
   * continue to rotate by inertia. If the motor is stalled, isMoving() will return <b> true. </b> .
   * A stall can be detected by calling {@link #isStalled()};
   *
   ...
   * The default value is 6000; Smaller values will make speeding up. or stopping at the end of a
   * rotate() task, smoother;
   *
   * @param acceleration
   */
  ...
  /**
   * Reset the tachometer associated with this motor. Note calling this method will cause any
   * current move operation to be halted.
   */
  ...
  /**
   * @deprecated The regulator will always try to hold position unless the motor is set into float
   *     mode using flt().
  ...
  /**
   * Set the parameters for detecting a stalled motor. A motor will be recognised as stalled if the
   * movement error (the amount the motor lags the regulated position) is greater than error for a
   * period longer than time.
   *
  ...
  /**
   * Specify a set of motors that should be kept in synchronization with this one. The
   * synchronization mechanism simply ensures that operations between a startSynchronization call
   * and an endSynchronization call will all be executed at the same time (when the
   * endSynchronization method is called). This is all that is needed to ensure that motors will
   * operate in a synchronized fashion. The start/end methods can also be used to ensure that reads
   * of the motor state will also be consistent.
   *
  ...
  public void synchronizeWith(RegulatedMotor[] syncList) {
    ...
    for (int i = 0; i < syncList.length; i++) rl[i] = ((BaseRegulatedMotor) syncList[i]).reg;
