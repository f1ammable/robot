...
/**
 * Java based regulator regulate velocity; also stop motor at desired rotation angle. This class
 * uses a very simple movement model based on simple linear acceleration. This model is used to
 * generate ideal target positions which are then used to generate error terms between the actual
 * and target position this error term is then used to drive a PID style motor controller to
 * regulate the power supplied to the motor.
 *
 * <p>If new command are issued while a move is in progress, the new command is blended with the
 * current one to provide smooth movement.
 *
 * <p>If the requested speed is not possible then the controller will simply drop move cycles until
 * the motor catches up with the ideal position. If too many consecutive dropped moves are required
 * then the motor is viewed to have stalled and the move is terminated.
 *
 * <p>Once the motor stops, the final position is held using the same PID control mechanism (with
 * slightly different parameters), as that used for movement.
 *
 * <p>TODO: Should we add synchronization to this?
 */
public class JavaMotorRegulator implements MotorRegulator {
  ...
  protected synchronized void updateState(int velocity, boolean hold, boolean stalled) {
    if (listener != null) {
      ...
      else listener.rotationStarted(motor, getTachoCount(), false, System.currentTimeMillis());
    }
  }
  ...
  @Override
  public void setControlParamaters(
      int typ,
      float moveP,
      float moveI,
      float moveD,
  ...
  /**
   * Helper method. Start a sub move operation. A sub move consists of acceleration/deceleration to
   * a set velocity and then holding that velocity up to an optional limit point. If a limit point
   * is set this method will be called again to initiate a controlled deceleration to that point
   *
   * @param speed
   * @param acceleration
   * @param limit
  ...
  private synchronized void startSubMove(float speed, float acceleration, int limit, boolean hold) {
    ...
    baseTime = now;
    if (!moving && Math.abs(limit - curCnt) < 1.0) curTargetVelocity = 0;
    else curTargetVelocity = (limit - curCnt >= 0 ? speed : -speed);
  ...
  /**
   * Initiate a new move and optionally wait for it to complete. If some other move is currently
   * executing then ensure that this move is terminated correctly and then start the new move
   * operation.
   *
   * @param speed
   * @param acceleration
   ...
   * @param waitComplete
   */
  public synchronized void newMove(
      float speed, int acceleration, int limit, boolean hold, boolean waitComplete) {
    if (!active) {
      cont.addMotor(this);
      active = true;
    }
    ...
    if (speed == 0) startSubMove(0, acceleration, NO_LIMIT, hold);
    else if (!moving) {
  ...
  public synchronized void adjustSpeed(float newSpeed) {
    ...
    if (pending) this.newSpeed = newSpeed;
  }
  ...
  public synchronized void adjustAcceleration(int newAcc) {
    ...
    if (pending) newAcceleration = newAcc;
  }
  ...
  synchronized void regulateMotor(long delta) {
    ...
    if (moving) {
      if (elapsed < accTime) {
        ...
        if (curTargetVelocity == 0
            && (pending
                || (Math.abs(error) < 2 && elapsed > accTime + 100)
                || elapsed > accTime + 500)) {
          endMove(false);
        }
      }
      ...
      if (checkLimit) {
        ...
        if (curStopAcc / acc < 1.0) startSubMove(0, acc, NO_LIMIT, curHold);
      }
    } else if (curHold) {
  ...
  /**
   * helper method for velocity regulation. calculates power from error using double smoothing and
   * PID like control
   *
   * @param error
   */
  private void calcPower(float error, float P, float I, float D, float time) {
    ...
    if (basePower > TachoMotorPort.MAX_POWER) basePower = TachoMotorPort.MAX_POWER;
    else if (basePower < -TachoMotorPort.MAX_POWER) basePower = -TachoMotorPort.MAX_POWER;
    // newPower = (float) (power*0.75 + newPower*0.25);
    power =
        (newPower > TachoMotorPort.MAX_POWER
            ? TachoMotorPort.MAX_POWER
            : newPower < -TachoMotorPort.MAX_POWER
                ? -TachoMotorPort.MAX_POWER
                : Math.round(newPower));
  ...
  /**
   * This class provides a single thread that drives all of the motor regulation process. Only
   * active motors will be regulated. To try and keep motors as closely synchronized as possible
   * tach counts for all motors are gathered as close as possible to the same time. Similarly new
   * power levels for each motor are also set at the same time.
   */
  protected static class Controller extends Thread {
    ...
    synchronized void removeMotor(JavaMotorRegulator m) {
      ...
      for (int i = 0; i < activeMotors.length; i++)
        if (activeMotors[i] != m) newMotors[j++] = activeMotors[i];
      activeMotors = newMotors;
    }
    ...
    synchronized void shutdown() {
      ...
      running = false;
      for (JavaMotorRegulator m : activeMotors) m.tachoPort.controlMotor(0, TachoMotorPort.FLOAT);
    ...
    public void run() {
      ...
      while (running) {
        ...
        synchronized (this) {
          ...
          for (JavaMotorRegulator m : motors) m.regulateMotor(delta);
          for (JavaMotorRegulator m : motors) m.tachoPort.controlMotor(m.power, m.mode);
        }
