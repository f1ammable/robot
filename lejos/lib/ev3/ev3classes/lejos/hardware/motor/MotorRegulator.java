...
public interface MotorRegulator {
  ...
  /**
   * Reset the tachometer base value, after this call the tachometer will return zero for the
   * current position. Note that any in progress movements will be aborted.
   */
  ...
  /**
   * Return the current velocity (in degrees/second) that the motor is currently running at. Note
   * that this value may be supplied from the internal control model not from actually measuring the
   * rotation speed. If the regulator is functioning correctly this will closely match the actual
   * velocity
   *
   * @return velocity
   */
  ...
  /**
   * Set the stall detection parameters. The motor will be declared as stalled if the error in the
   * motor position exceeds the specified value for longer than the given time.
   *
   * @param error
   * @param time
   */
