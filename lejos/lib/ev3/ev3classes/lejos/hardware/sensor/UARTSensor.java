...
public class UARTSensor extends BaseSensor {
  ...
  public UARTSensor(UARTPort port, int mode) {
    this.port = port;
    if (!port.setMode(mode)) throw new DeviceException("Unable to initialize device");
    currentMode = mode;
  }
  ...
  /**
   * Switch to the selected mode (if not already in that mode) and delay for the specified period to
   * allow the sensor to settle in the new mode. <br>
   * A mode of -1 resets the sensor. TODO: There really should be a better way to work out when the
   * switch is complete, if you don't wait though you end up reading data from the previous mode.
   *
  ...
  protected void switchMode(int newMode, long switchDelay) {
    if (currentMode != newMode) {
      if (newMode == -1) port.resetSensor();
      else if (!port.setMode(newMode)) throw new IllegalArgumentException("Invalid sensor mode");
      currentMode = newMode;
