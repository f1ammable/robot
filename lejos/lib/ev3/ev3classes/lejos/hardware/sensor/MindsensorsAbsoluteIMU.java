 ...
 * Sensor interface for the Mindsensors AbsoluteIMU family of sensors. The AbsoluteIMU sensors
 * combine gyro, accelerometer and compass sensors in various combinations in a single housing. This
 * interface works with all AbsoluteIMU models, but not all modes will work with any particular
 * model.
 *
 * <p>
 *
 ...
 * The gyro sensor of the AbsoluteIMU uses a filter to remove noise from the samples. The filter can
 * be configured using the {@link #setGyroFilter } method. <br>
 * The compass sensor of the AbsoluteIMU can be calibrated to compensate for magnetical disturbances
 * on the robot (soft iron calibration) using the {@link #startCalibration} and {@link
 * #stopCalibration} methods.
 *
 * <p>To calibrate Compass, mount it on your robot where it will be used and issue startCalibration
 * method and then rotate AbsoluteIMU slowly along all three axes. (The Compass in AbsoluteIMU is a
 * 3 axis compass, and hence needs to be turned along all three axes, and if it's mounted on your
 * robot, the whole robot needs to rotate). Rotate one axis at a time, turn once in clock-wise
 * direction completing at-least 360 degrees, and then turn it in anti-clock-wise direction, then go
 * to next axis. Upon finishing turning along all axes, issue stopCalibration method.
 *
 * <p>See <a href=
 * "http://mindsensors.com/index.php?module=documents&JAS_DocumentManager_op=downloadFile&JAS_File_id=1369"
 * >Mindsensors IMU user guide"> Sensor Product page </a> See <a
 * href="http://sourceforge.net/p/lejos/wiki/Sensor%20Framework/">The leJOS sensor framework</a> See
 * {@link lejos.robotics.SampleProvider leJOS conventions for SampleProviders}
 *
 * <p>
 *
 * @author Andy
...
public class MindsensorsAbsoluteIMU extends I2CSensor implements SensorModes, Calibrate {
  ...
  protected class ShortSensorMode implements SensorMode {
    ...
    /**
     * Internal class to provide the sensor data. Handles obtaining the base data and converting it
     * to suitable SI units.
     *
    ...
    protected ShortSensorMode(String name, int reg, int sampleSize, float convert, float scale) {
      ...
      for (int i = 0; i < sampleSize; i++) this.convert[i] = convert;
      this.name = name;
      this.sampleSize = sampleSize;
      this.baseReg = reg;
      this.scale = new float[sampleSize];
    ...
    protected void setScale(float scale) {
      for (int i = 0; i < convert.length; i++) this.scale[i] = convert[i] * scale;
    }
  ...
  protected void init() {
    ...
    this.setModes(new SensorMode[] {magMode, compassMode, angleMode, accelMode, gyroMode});
    setRange(LOW);
    setGyroFilter(4);
  }
  ...
  /**
   * Return a SensorMode object that will provide tilt compensated compass data . The sample
   * contains one element containing the bearing of the sensor relative to north expressed in
   * degrees. East being at 90 degrees.
   *
  ...
  /**
   * Return a SensorMode object that will provide angle data. The sample contains one element
   * containing the bearing of the sensor relative to north expressed in degrees using a right hand
   * coordinate system in the range of (-180 to 180). West being at 90 degrees, east being at -90
   * degrees.
   *
   * @return a SensorMode object
   */
  ...
  /**
   * Return a SensorMode object that will acceleration data for the X, Y and Z axis. The data is
   * returned in units of m/s/s.
   *
   * @return a SensorMode object
   */
  ...
  /**
   * Return a SensorMode object that will return Magnetic data for the X, Y and Z axis The data is
   * returned in Guass
   *
   * @return a SensorMode object
   */
  ...
  /**
   * Return a SensorMode object that will angular velocity data for the X, Y and Z axis. The data is
   * returned in units of degrees/s.
   *
   * @return a SensorMode object
   */
  ...
  /**
   * Set the sensitivity used by the sensor. This setting impacts the maximum range of the returned
   * value and the resolution of the reading.<br>
   ...
   * The Gyro readings are filtered with n’th order finite impulse response filter, (where n ranges
   * from 0 to 7) value 0 will apply no filter, resulting in faster reading, but noisier
   * values.value 7 will apply stronger filter resulting in slower read (about 10 milli-seconds
   * slower) but less noise.<br>
  ...
  /**
   * To calibrate Compass, mount it on your robot where it will be used and issue startCalibration
   * method and then rotate AbsoluteIMU slowly along all three axes. (The Compass in AbsoluteIMU is
   * a 3 axis compass, and hence needs to be turned along all three axes, and if it's mounted on
   * your robot, the whole robot needs to rotate). Rotate one axis at a time, turn once in
   * clock-wise direction completing at-least 360 degrees, and then turn it in anti-clock-wise
   * direction, then go to next axis. Upon finishing turning along all axes, issue stopCalibration
   * method.
   */
  @Override
