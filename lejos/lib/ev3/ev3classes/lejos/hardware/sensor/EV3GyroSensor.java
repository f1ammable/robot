 ...
 * The digital EV3 Gyro Sensor measures the sensors rotational motion and changes in its
 * orientation.
 *
 * <p>
 *
 ...
 * Use {@link #reset()} to recalibrate the sensor and to reset accumulated angle to zero. Keep the
 * sensor motionless during a reset. The sensor shuld also be motionless during initialization.
 *
 * <p>See <a href="http://www.ev-3.net/en/archives/849">Sensor Product page </a> See <a
...
public class EV3GyroSensor extends UARTSensor {
   ...
   * The sample contains one elements representing the orientation (in Degrees) of the sensor in
   * respect to its start position.
   *
   ...
   *
   * <p><b>Size and content of the sample</b><br>
   * The sample contains one elements representing the angular velocity (in Degrees / second) of the
   * sensor.
   *
   * <p><b>Configuration</b><br>
   ...
   *
   * <p><b>Size and content of the sample</b><br>
   * The sample contains two elements. The first element contains angular velocity (in degrees /
   * second). The second element contain angle (in degrees).
   *
   * <p><b>Configuration</b><br>
   * The sensor can be recalibrated using the reset method of the sensor.
   *
   ...
   */
  public void reset() {
    // Reset mode (4) is not used here as it behaves eratically. Instead the reset is done
    // implicitly by going to mode 1.
