 ...
 *
 * <p>Extend this class to implement new I2C sensors.
 *
 * <p>TODO: We should probably change this class to use the Product ID etc. obtained by the Lego
 * kernel module during initialisation.
 *
...
public class I2CSensor extends BaseSensor implements SensorConstants {
  ...
  public void setRetryCount(int newCount) {
    if (newCount <= 0) throw new IllegalArgumentException("Invalid retry count");
    retryCount = newCount;
  }
  ...
  public synchronized void sendData(int register, byte[] buf, int offset, int len) {
    if (len >= ioBuf.length) throw new IllegalArgumentException("Invalid buffer length");
    ioBuf[0] = (byte) register;
    // avoid NPE in case length==0 and data==null
    if (buf != null) System.arraycopy(buf, offset, ioBuf, 1, len);
    I2CException error = null;
    for (int i = 0; i < retryCount; i++) {
      try {
  ...
  /**
   * Read the sensor's version string. This method reads up to 8 bytes and returns the characters
   * before the zero termination byte. Examples: "V1.0", ...
   *
  ...
  /**
   * Read the sensor's vendor identifier. This method reads up to 8 bytes and returns the characters
   * before the zero termination byte. Examples: "LEGO", "HiTechnc", ...
   *
  ...
  /**
   * Read the sensor's product identifier. This method reads up to 8 bytes and returns the
   * characters before the zero termination byte. Examples: "Sonar", ...
   *
  ...
  /**
   * Read a string from the device. This functions reads the specified number of bytes and returns
   * the characters before the zero termination byte.
   *
   * @param reg
  ...
  protected String fetchString(byte reg, int len) {
    ...
    for (i = 0; i < len && buf[i] != 0; i++) charBuff[i] = (char) (buf[i] & 0xFF);

    return new String(charBuff, 0, i).trim();
  }

  /**
   * Set the address of the port Addresses use the standard Lego/NXT format and are in the range
   * 0x2-0xfe. The low bit must always be zero. Some data sheets (and older versions of leJOS) may
   * use i2c 7 bit format (0x1-0x7f) in which case this address must be shifted left one place to be
   * used with this function.
   *
   * @param addr 0x02 to 0xfe
   * @deprecated If the device has a changeable address, then constructor of the class should have
   *     an address parameter. If not, please report a bug.
   */
  @Deprecated
