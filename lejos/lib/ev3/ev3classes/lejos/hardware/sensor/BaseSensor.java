...
public class BaseSensor extends Device implements SensorModes {
  ...
  public ArrayList<String> getAvailableModes() {
    if (modeList == null) {
      ...
      if (modes != null) for (SensorMode m : modes) modeList.add(m.getName());
    }
    return modeList;
  }
  ...
  public SensorMode getMode(int mode) {
    if (mode < 0) throw new IllegalArgumentException("Invalid mode " + mode);
    if (modes == null || mode >= modes.length) return null;
    return modes[mode];
  }
  ...
  public SensorMode getMode(String modeName) {
    ...
    for (String s : getAvailableModes()) {
      if (s.equals(modeName)) return modes[i];
      i++;
    }
