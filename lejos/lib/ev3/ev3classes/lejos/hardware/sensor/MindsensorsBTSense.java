...
public class MindsensorsBTSense {
  ...
  /**
   * A - Accelerometer G - Gravity N - Linear Acceleration L - Light M - Magnetic Field C - Compass
   * P - Proximity S - Sound D - Date T - Time G - Gyroscope E - Atmospheric pressure H - Humidity U
   * - Temperature W - Network location O - GPS Location
   */
   ...
   */
  private void processSensor(char sensorType, byte[] packet, int offset)
    ...
    switch (sensorType) {
      ...
      case 'O':
        ...
        if (debug)
          System.out.println(
              "GPS Lat: " + hd + ":" + hm + ":" + hs + " Long: " + wd + ":" + wm + ":" + ws);
        break;
   ...
   */
  private int getInt(byte[] packet, int offset, int len)
      throws NumberFormatException, UnsupportedEncodingException {
    byte[] b = new byte[len];
