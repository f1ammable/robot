 ...
 * The sensor cannot be configured directly. The internal update frequency of the sensor is adjusted
 * automaticly to match the I2C port speed.
 *
 * <p>See <a href="http://dexterindustries.com/files/dIMU_Datasheets.zip">Sensor datasheet </a> See
 * <a href="http://www.dexterindustries.com/dIMU.html">Sensor Product page </a> See <a
...
public class DexterIMUSensor extends BaseSensor implements SensorModes {
  ...
  public DexterIMUSensor(I2CPort port) {
    ...
    setModes(
        new SensorMode[] {
          gyro.getMode(0), new DexterIMUAccelerationSensor(port, Accel_I2C_address), gyro.getMode(1)
        });
  }
  ...
  public DexterIMUSensor(Port port) {
    ...
    setModes(
        new SensorMode[] {
          gyro.getMode(0),
          new DexterIMUAccelerationSensor(gyro.port, Accel_I2C_address),
          gyro.getMode(1)
        });
    releaseOnClose(gyro);
  }
   ...
   * The sample contains three elements. Each element gives the linear acceleration (in
   * metres/second^2) of the sensor over a single axis. The order of the axes in the sample is X, Y
   * and Z.
   *
   * <p><b>Configuration</b><br>
   * The sensor is configured for a dynamic range of -2G tot 2G and an internal update rate of 125
   * Hertz. Currently there are no configurable settings.
   *
   * @return A sampleProvider See {@link lejos.robotics.SampleProvider leJOS conventions for
   *     SampleProviders} See <a
   *     href="http://www.freescale.com/files/sensors/doc/data_sheet/MMA7455L.pdf">Freescale
   *     MMA7455L datasheet </a>
   */
   ...
   *
   * <p><b>Size and content of the sample</b><br>
   * The sample contains three elements. Each element gives the angular speed (in degrees/second) of
   * the sensor over a single axis. The order of the axes in the sample is X, Y and Z.
   *
   * <p><b>Configuration</b><br>
   * The sensor is configured for a dynamic range from -2000 to 2000 degrees/second. The internal
   * sample rate is 100 Hertz by default and 400 Hertz when the sensor port is configured for high
   * speed. Currently there are no configurable settings.
   *
   * @return A sampleProvider See {@link lejos.robotics.SampleProvider leJOS conventions for
   *     SampleProviders} See <a
   ...
   *
   * <p><b>Size and content of the sample</b><br>
   * The sample contains one elements providing the internal temperature (in gegrees Celcius) of the
   * gyro sensors. Please note that the internal temperature of the sensor will exceed the
   * temperature of the environment when the sensor is in use.
   *
   * <p><b>Configuration</b><br>
   * The temperature range of the sensor is -40 to 85 degrees Celcius. The update rate is 1 Hertz.
  ...
  private class DexterIMUGyroSensor extends I2CSensor {
    ...
    private boolean isNewDataAvailable() {
      ...
      if ((buf[0] & 0x08) == 0x08) return true;
      return false;
    }
    ...
    /**
     * Provides access to the temperature data on the Gyro sensor Method added for consistency with
     * other SensoModes
     *
     * @return
     */
  ...
  /**
   * Class to access the Acceleration sensor from Dexter Industries IMU the acceleration sensor is
   * the Freescale MMA7455
   *
   * @author Aswin
   */
