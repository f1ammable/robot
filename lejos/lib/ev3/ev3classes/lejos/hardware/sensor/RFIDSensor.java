...
/**
 * Support for the <a href="http://www.codatex.com">Codatex RFID Sensor</a>. This device requires
 * delays between various commands for them to function correctly, it also enters a sleep mode and
 * requires to be woken up. The methods in this class fall into two categories. Basic commands These
 * pretty much match one to one with the device command set. They do not incorporate any delays, or
 * wake up code. They can be used by user programs but if they are then appropriate delays etc. must
 * be used. They are provided to allow more sophisticated user programs access to the low level
 * device.
 *
 * <p>High level commands These provide higher level access to the device and are often implemented
 * via several i2c commands. They do include delays and wake up logic.
 *
 * @author andy
 */
public class RFIDSensor extends I2CSensor {
  ...
  /**
   * Helper function, return current time, used as basis of required inter command delays.
   *
   * @return
   */
  private long now() {
  ...
  private void waitUntil(long target) {
    long d = target - now();
    if (d > 0) Delay.msDelay(d);
  }

  /**
   * The sensor will go into a power save mode after a short time. This means that we will need to
   * wake it up before issuing commands. Includes the required Delay.msDelay.
   */
  public void wakeUp() {
  ...
  /**
   * We over-ride the default implementation to ensure that the device is awake before we talk to
   * it.
   *
  ...
  /**
   * Start the firmware on the RFID device. NOTES: It seems that you need to issue this command (or
   * some other firmware command), prior to attempting to read the version number etc. Does not wake
   * up the device or contain any delays.
   */
  ...
  /**
   * Obtain the serial number of the RFID Sensor. NOTES: To obtain the serial number the device must
   * be in boot loader mode. This function will switch into this mode and then return to normal mode
   * on completion.
   *
  ...
  /**
   * Read a transponder id. Reads the transponder using either continuous or single shot mode. If
   * using single shot mode the tag must me available now. If using continuous mode then after the
   * first call, the tag can be presented at any time and a subsequent read will return the results.
   *
