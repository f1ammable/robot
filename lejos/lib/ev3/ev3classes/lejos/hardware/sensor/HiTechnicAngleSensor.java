 ...
 * The zero position of the sensor can be set and stored in the sensors memory using the
 * calibrateAngle method. The accumulated angle can be set to zero using the resetAccumulatedAngle
 * method. The accumulated angle is not stored in memory.
 *
 * <p>See <a href="http://www.hitechnic.com/cgi-bin/commerce.cgi?preadd=action&key=NAA1030">Sensor
...
public class HiTechnicAngleSensor extends I2CSensor {
  ...
  protected void init() {
    setModes(
        new SensorMode[] {new AngleMode(), new AccumulatedAngleMode(), new AngularVelocityMode()});
  }
  ...
  /**
   * Calibrate the zero position of angle. Zero position is saved in EEPROM on sensor. Thread sleeps
   * for 50ms while that is done.
   */
   ...
   * The sample contains one elements containing the angle of the sensor in the range of 0 to 359
   * degrees.
   */
