 ...
 * The NXT Ultrasonic sensor generates sound waves and reads their echoes to detect and measure
 ...
 * </table>
 *
 * <p>See <a href="http://www.lego.com/en-us/mindstorms/downloads/software/nxt-hdk/">Mindstorms NXT
 * HDK/SDK </a> See <a href="http://sourceforge.net/p/lejos/wiki/Sensor%20Framework/">The leJOS
 * sensor framework</a> See {@link lejos.robotics.SampleProvider leJOS conventions for
 * SampleProviders}
 *
 * <p>
 *
...
public class NXTUltrasonicSensor extends I2CSensor {
  // Supported modes
  // According to the datasheet there are other modes. These modes do not function on the sensor and
  // are not implemented;
   ...
   * the surrounding continuously and reports the distance to the nearest object in sight. The
   * sensor reports the distance to the nearest object in meters. The theoretical range of the
   * sensor is 0,04 to 2.54 meter. If there is no object detected within this range the sensor
   * reports Float.POSITIVE_INFINITY. <br>
   * Samples can only be provided every 30 ms. If samples are fetched more often the SampleProvider
   * will pause until the 30ms have passed.
   *
   * @return A SamplePrivider
   */
  ...
  /**
   * Gives a SampleProvider representing the sensor in ping mode. In this mode the sensor only scans
   * the surrounding upon request, ie by calling fetchSample method. The sensor reports the
   * distance, expressed in meters, to the eight nearest objects in sight. The theoretical range of
   * the sensor is 0,04 to 2.54 meter. If there are less then eight objects within the sensors range
   * the sensor will reports Float.POSITIVE_INFINITY. Fetching a sample in ping mode takes about 70
   * ms.
   *
   ...
   *
   * @param mode
   * @return True, if the mode was changed. <br>
      ...
      if (setMode(MODE_CONTINUOUS)) {
        dataAvailableTime =
            System.currentTimeMillis()
                + DELAY_DATA_CONTINUOUS; // when the sensor was in another mode, the wait period
                                         // just start. Otherwise it was set the last time this
                                         // method was called;
      }
