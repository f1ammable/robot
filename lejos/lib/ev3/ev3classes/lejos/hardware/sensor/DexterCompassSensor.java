 ...
 * Range can be set Using getRanges() and setRange methods. Internal update frequency of the sensor
 * can be set using getRates and setRate methods.
 *
 * <p>See <a href="http://www.adafruit.com/datasheets/HMC5883L_3-Axis_Digital_Compass_IC.pdf">Sensor
 * datasheet </a> See <a href="http://www.dexterindustries.com/dCompass.html">Sensor Product page
 * </a> See <a href="http://sourceforge.net/p/lejos/wiki/Sensor%20Framework/">The leJOS sensor
 * framework</a> See {@link lejos.robotics.SampleProvider leJOS conventions for SampleProviders}
 *
 * <p>
 *
...
public class DexterCompassSensor extends I2CSensor implements SensorModes {
  ...
  private static final float[] RATES = {0.75f, 1.5f, 3, 7.5f, 15, 30, 75};
  private static final int[] RANGEMULTIPLIER = {1370, 1090, 820, 660, 440, 390, 330, 230};
  private static final float[] RANGES = {0.88f, 1.3f, 1.9f, 2.5f, 4, 4.7f, 5.6f, 8.1f};
   ...
   * The sample contains 3 elements. Each element gives the strength of the magnetic field (in
   * Gueass). Axis order is X, Y, Z.
   *
   * <p><b>Configuration</b><br>
   * By default the sensor is configured for a range of 5.6 Gauss and an update frequency of 30
   * Hertz. <br>
   ...
   *     href="http://www.adafruit.com/datasheets/HMC5883L_3-Axis_Digital_Compass_IC.pdf">Sensor
   *     datasheet </a>
   */
  ...
  /** Sets the configuration registers of the sensor according to the current settings */
  ...
  /**
   * @return Returns the measurement mode of the sensor (normal, positive bias or negative bias).
   *     <p>positive and negative bias mode should only be used for testing the sensor.
   */
  ...
  /**
   * @return The operating mode of the sensor (single measurement, continuous or Idle)
   */
  ...
  /**
   * @param measurementMode Sets the measurement mode of the sensor.
   */
   ...
   *
   * @param operatingMode Continuous is normal mode of operation
   *     <p>SingleMeasurement can be used to conserve energy or to increase maximum measurement rate
   *     <p>Idle is to stop the sensor and conserve energy
   */
   ...
   *
   * @return An array of boolean values. A true indicates the sensor axis is working properly.
   */
  public boolean[] test() {
    ...
    for (int axis = 0; axis < 3; axis++)
      if (dummy[axis] > 243 && dummy[axis] < 575) ret[axis] = true;
      else ret[axis] = false;
  ...
  public void setSampleRate(float rate) {
    for (int i = 0; i < RATES.length; i++) if (RATES[i] == rate) rate = i;
    configureSensor();
  }
  ...
  public void setRange(float range) {
    for (int i = 0; i < RANGES.length; i++) if (RANGES[i] == range) range = i;
    configureSensor();
  }
  ...
  private class MagneticMode implements SensorMode {
     ...
     *
     * <p>The array order is X, Y, Z
     *
     * <p>When the sensor is idle zeros will be returned.
     */
    public void fetchSample(float[] sample, int offset) {
      ...
      switch (operatingMode) {
        ...
        case (CONTINUOUS):
          ...
          break;
        default:
          for (int axis = 0; axis < 3; axis++) sample[axis + offset] = Float.NaN;
          break;
      }
    }
