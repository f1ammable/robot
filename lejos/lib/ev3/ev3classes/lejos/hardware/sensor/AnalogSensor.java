...
public class AnalogSensor extends BaseSensor implements SensorConstants {
  ...
  /**
   * Switch to the selected type (if not already in that type) and delay for the specified period to
   * allow the sensor to settle in the new state. <br>
   * NOTE: This method is intended for use with NXT sensor drivers that use a sensor type to specify
   * the operating mode.
   *
  ...
  protected void switchType(int newType, long switchDelay) {
    if (currentType != newType) {
      if (!port.setType(newType)) throw new IllegalArgumentException("Invalid sensor mode");
      currentType = newType;
