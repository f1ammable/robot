 ...
 * </pre>
 *
 * <b>Notions:</b> The API is designed around two notions: states (up / down) and events (press /
 * release). It is said that a button is pressed (press event), if its state changes from up to
 * down. Similarly, it is said that a button is released (release event), if its states changed from
 * down to up.
 *
 * <p><b>Thread Safety</b>: All methods that return buttons states can be used safely from multiple
 * threads, even while a call to one of the waitFor* methods active. However, it is not safe to
 * invoke waitFor* methods in parallel from different threads. This includes the waitFor* methods of
 * different buttons. For example Button.ENTER.waitForPress() must not be invoked in parallel to
 * Button.ESCAPE.waitForPress() or the static Button.waitForAnyEvent(). In case this is needed, it
 * is strongly recommended that you write your own Thread, which waits for button events and
 * dispatches the events to anyone who's interested.
 */
public class Button {
  ...
  /**
   * This method discards any events. In contrast to {@link #readButtons()}, this method doesn't
   * beep if a button is pressed.
   */
  ...
  /**
   * Waits for some button to be pressed or released. Which buttons have been released or pressed is
   * returned as a bitmask. The lower eight bits (bits 0 to 7) indicate, which buttons have been
   * pressed. Bits 8 to 15 indicate which buttons have been released.
   *
   * @return the bitmask
   * @see #ID_ENTER
   * @see #ID_LEFT
  ...
  /**
   * Waits for some button to be pressed. If a button is already pressed, it must be released and
   * pressed again.
   *
   * @param timeout The maximum number of milliseconds to wait
   * @return the ID of the button that has been pressed or in rare cases a bitmask of button IDs, 0
   ...
   *
   * @return An integer with possibly some bits set: {@link #ID_ENTER} (ENTER button pressed) {@link
   *     #ID_LEFT} (LEFT button pressed), {@link #ID_RIGHT} (RIGHT button pressed), {@link
   *     #ID_ESCAPE} (ESCAPE button pressed). If all buttons are released, this method returns 0.
   */
  ...
  /**
   * Set the frequency used for a particular key. Setting this to 0 disables the click. Note that
   * key may also be a corded set of keys.
   *
