 ...
 *
 * <p>TODO: Sort out a better way to do this, or least clean up the magic numbers.
 */
public class EV3MotorPort extends EV3IOPort implements TachoMotorPort {
  ...
  protected static final EV3MotorRegulatorKernelModule[] syncSlave =
      new EV3MotorRegulatorKernelModule[0];

  /**
   * Implementation of a PID based motor regulator that uses a kernel module for the core regulation
   * operations. This mechanism is accessed via the EV3MotorPort class.
   */
  public class EV3MotorRegulatorKernelModule extends Thread implements MotorRegulator {
    ...
    public EV3MotorRegulatorKernelModule(TachoMotorPort p) {
      if (p != EV3MotorPort.this) throw new IllegalArgumentException("Invlaid port specified");
     ...
     * @param deadBand
     */
    public synchronized void setControlParams(
        int typ,
        float moveP,
        float moveI,
        float moveD,
    ...
    /** Check to see if the current command is complete and if needed call any listeners. */
    ...
    protected synchronized void startNewMove() {
      ...
      if (started) throw new IllegalStateException("Motor must be stopped");
      started = true;
      if (listener != null) {
    ...
    protected void subMove(
        ...
        int ts,
        boolean hold) {
      // System.out.println("t1 " + t1 + " t2 " + t2 + " t3 " + t3 + " c1 " + c1 + " c2 " + c2 + "
      // c3 " + c3 + " v1 " + v1 + " v2 " + v2 + " a1 " + a1 + " a3 " + a3);
      ...
      if ((v1 != 0 || v2 != 0) && ts == 0) startNewMove();
    }

    /**
     * Helper method generate a move by splitting it into three phases, initial acceleration,
     * constant velocity, and final deceleration. We allow for the case were it is not possible to
     * reach the required constant velocity and hence the move becomes triangular rather than
     * trapezoid.
     *
    ...
    protected void genMove(
      ...
      float a1 = acc;
      float a3 = acc;
      // System.out.println("pos " + curPos + " curVel " + curVel + " limit " + limit + " len " +
      // len + " speed " + speed + " hold " + hold);
      if (speed == 0.0) {
        // Stop case
      ...
      if (vmax2 <= v2) {
        ...
        if (v < 0) v = -(float) Math.sqrt(vmax2);
        else v = (float) Math.sqrt(vmax2);
    ...
    public void waitComplete() {
      for (EV3MotorRegulatorKernelModule r : syncActive) {
        while (r.isMoving()) Delay.msDelay(1);
      }
      for (EV3MotorRegulatorKernelModule r : syncActive) r.checkComplete();
    }
      ...
      for (EV3MotorRegulatorKernelModule r : syncActive) {
        if (r.newMove)
          r.genMove(
              ...
              r.curPosition,
              r.curCnt,
              (r.curState >= ST_START ? r.curTime : 0),
              r.curSpeed,
              r.curAcc,
              r.curLimit,
              r.curHold);
    ...
    public void newMove(
      ...
      synchronized (this) {
        limitAngle = limit;
        if (Math.abs(limit) != NO_LIMIT) limit += zeroTachoCnt;
        updateRegulatorInformation();
        // Ignore repeated commands
        if (curState != ST_STALL
            && !waitComplete
            && (speed == curSpeed)
            && (curAcc == acceleration)
            && (curLimit == limit)
      ...
      if (waitComplete) waitComplete();
    }

    /**
     * The kernel module updates the shared memory serial number every time a new command is issued.
     * We can use this to wait for the shared mem to be updated. We must do this to ensure that we
     * do not see a move as complete when in fact it may not have even started yet!
     *
     * @return
     */
    ...
    /** Grabs the current state of the regulator and stores in class member variables */
    ...
    public synchronized void synchronizeWith(MotorRegulator[] syncList) {
      ...
      for (MotorRegulator r : syncList) {
        ...
        if (r == this) throw new IllegalArgumentException("Can't synchronize with self");
      }
      ...
      int i = 1;
      for (MotorRegulator r : syncList) sl[i++] = (EV3MotorRegulatorKernelModule) r;
      sl[0] = this;
      this.syncWith = sl;
    }
    ...
    public synchronized void startSynchronization() {
      synchronized (pwm) {
        // set slaves to sync
        for (int i = 1; i < syncWith.length; i++) syncWith[i].syncActive = syncSlave;
    ...
    public synchronized void endSynchronization(boolean immRet) {
      synchronized (pwm) {
        ...
        for (int i = 1; i < syncWith.length; i++) syncWith[i].syncActive = syncWith[i].syncThis;
      }
      if (!immRet) waitComplete();
  ...
  public synchronized MotorRegulator getRegulator() {
    if (regulator == null) regulator = new EV3MotorRegulatorKernelModule(this);
