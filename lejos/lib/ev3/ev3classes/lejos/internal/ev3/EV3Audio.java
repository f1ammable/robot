...
public class EV3Audio implements Audio {
  ...
  public void systemSound(int aCode) {
    if (aCode == BEEP) playTone(600, 200);
  ...
  static void playFreq(int aFrequency, int aDuration, int aVolume) {
    if (aVolume >= 0) aVolume = (aVolume * masterVolume) / 100;
    else aVolume = -aVolume;
    byte[] cmd = new byte[6];
    cmd[0] = OP_TONE;
    cmd[1] = (byte) aVolume;
  ...
  public synchronized void startPCMPlayback(int sampleSize, int sampleRate, int vol) {
    if (sampleSize != 8 && sampleSize != 16)
      throw new UnsupportedOperationException(
          "Only 8bit and 16bit samples supported size is " + sampleSize);
    ...
    if (vol >= 0) vol = (vol * masterVolume) / 100;
    else vol = -vol;
  ...
  public void writePCMSamples(byte[] data, int offset, int dataLength) {
    ...
    if (PCMSampleSize == 8) {
      ...
      while (offset < dataLength) {
        ...
        if (len > PCM_BUFFER_SIZE / 2) len = PCM_BUFFER_SIZE / 2;
        int outOffset = 1;
        for (int i = 0; i < len; i++) {
  ...
  public int playSample(File file, int vol) {
    ...
    if (file.length() < RIFF_HDR_SIZE) return -9;
    ...
    try {
      ...
      if (d.readInt() != RIFF_WAVE_SIG) return -2;
      if (d.readInt() != RIFF_FMT_SIG) return -3;
      // Now check that the format is PCM, Mono 8 bits. Note that these
      // values are stored little endian.
      int sz = readLSBInt(d);
      if (d.readShort() != RIFF_FMT_PCM) return -4;
      if (d.readShort() != RIFF_FMT_1CHAN) return -5;
      int sampleRate = readLSBInt(d);
      if (sampleRate > 48000) return -7;
      d.readInt();
      d.readShort();
      short sampleSize = d.readShort();
      if (sampleSize != RIFF_FMT_16BITS && sampleSize != RIFF_FMT_8BITS) return -6;
      playing = true;
      // Skip any data in this chunk after the 16 bytes above
      sz -= 16;
      while (sz-- > 0) d.readByte();
      ...
      for (; ; ) {
        ...
        if (chunk == RIFF_DATA_SIG) {
          ...
          if (sampleSize == RIFF_FMT_16BITS) {
            ...
            while (dataLen > 0
                && (read =
                        d.read(
                            PCMBuffer,
                            1,
                            (PCMBuffer.length - 1 < dataLen ? PCMBuffer.length - 1 : dataLen)))
                    > 0) {
            ...
            while (dataLen > 0
                && (read = d.read(data, 0, (data.length < dataLen ? data.length : dataLen))) > 0) {
   ...
   * @return The number of milliseconds the sample will play for or < 0 if there is an error.
   * @throws FileNotFoundException
   */
  ...
  private int waitUntil(int t) {
    int t2;
    while ((t2 = (int) System.currentTimeMillis()) < t) Thread.yield();
    return t2;
  }

  /**
   * Play a note with attack, decay, sustain and release shape. This function allows the playing of
   * a more musical sounding note. It uses a set of supplied "instrument" parameters to define the
   * shape of the notes envelope.
   *
  ...
  /**
   * Load the current system settings associated with this class. Called automatically to initialize
   * the class. May be called if it is required to reload any settings.
   */
