...
/**
 * lcdui.graphics implementation for the EV3 LCD screen and in memory images. This class provides a
 * sun-set of the standard JavaME graphics class. In particular it does not implement clipping.
 * There are also several additional non-standard methods to allow easier use of images, alow access
 * to inverse text, and allow the use of raster operations. Some of the standard limitations (like
 * not allowing copyArea of the screen), are not enforced.
 *
...
public class EV3GraphicsLCD extends EV3LCD implements GraphicsLCD {
  ...
  /**
   * Create a graphics object that can be used to access the supplied memory buffer.
   *
  ...
  /**
   * Return the width of the associated drawing surface. <br>
   * <b>Note</b>: This is a non standard method.
   *
  ...
  /**
   * Return the height of the associated drawing surface. <br>
   * <b>Note</b>: This is a non standard method.
   *
  ...
  /**
   * Draws the specified String using the current font and color. x and y give the location of the
   * anchor point. Additional method to allow for the easy use of inverted text. In this case the
   * area below the string is drawn in the current color, before drawing the text in the "inverted"
   * color. <br>
   * <b>Note</b>: This is a non standard method.
   *
  ...
  public void drawString(String str, int x, int y, int anchor, boolean inverted) {
    ...
    if ((anchor & LEFT) == 0) {
      ...
      if ((anchor & RIGHT) != 0) x -= strWidth;
      else if ((anchor & HCENTER) != 0) x -= (strWidth / 2);
    }

    if ((anchor & TOP) == 0) {
      if ((anchor & BASELINE) != 0) y -= font.getBaselinePosition();
      else if ((anchor & BOTTOM) != 0) y -= font.getHeight();
    }
  ...
  /**
   * Draw the specified image to the graphics surface, using the supplied rop. <br>
   * <b>Note</b>: This is a non standard method. Added because without it, it is very hard to
   * invert/manipulate an image, or screen region
   *
  ...
  /**
   * Draw the specified region of the source image to the graphics surface after applying the
   * requested transformation, use the supplied rop. <br>
   * NOTE: When calculating the anchor point this method assumes that a transformed version of the
   * source width/height should be used.
   *
   ...
   * On the EV3 LCD and value that is not black will be treated as white.
   *
   * @param color
  ...
  /**
   * Set the current drawing color. The value is in the format 0x00RRGGBB. NOTE. Currently only
   * black and white is supported. any non black color is treated as white!
   *
   * @param rgb new color.
   */
   ...
   * @param blue the blue
   * @throws IllegalArgumentException if any of the color components are outside of range <code>
   *     0-255</code>
   * @see #getColor
   */
  ...
  /**
   * Draw the specified region of the supplied image to the graphics surface. NOTE: Transforms are
  ...
  private void drawLine(int x0, int y0, int x1, int y1, int style) {
    ...
    if (style == SOLID && (dx == 0 || dy == 0)) {
      ...
      bitBlt(
          ...
          dx + 1,
          dy + 1,
          (rgbColor == BLACK ? ROP_SET : ROP_CLEAR));
      return;
    }
    if (dy < 0) {
      dy = -dy;
    ...
    if (dx > dy) {
      ...
      while (x0 != x1) {
        ...
        if ((style == SOLID) || !skip) setPixel(x0, y0);
        skip = !skip;
      }
    } else {
  ...
  private void drawArc(
      int x, int y, int width, int height, int startAngle, int arcAngle, int style, boolean fill) {
  ...
  private void drawTheCircle(
      int radius, int style, boolean fill, int xscale, int yscale, int x0, int y0) {
  ...
  public void drawRoundRect(int x, int y, int width, int height, int arcWidth, int arcHeight) {
    ...
    while (yyy >= 0 && xxx <= a) {
      ...
      if (xxx != 0 || yyy != 0) setPixel(xc - xxx - translateX, yc - yyy - translateY); // Q2
      if (xxx != 0 && yyy != 0) {
