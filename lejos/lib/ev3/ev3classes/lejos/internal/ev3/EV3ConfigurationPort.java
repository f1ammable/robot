...
public class EV3ConfigurationPort extends EV3IOPort implements ConfigurationPort {
  ...
  /**
   * {@inheritDoc} Note that it can take up to two seconds for the identification data to be
   * available after the port has been opened.
   */
  @Override
   ...
   * identify the connection type (UART/IIC/Dumb/Output etc.)
   *
   * @return The type of the port.
   */
  public int getPortType() {
    if (typ == EV3Port.MOTOR_PORT) return EV3AnalogPort.getMotorPortType(port);
    else return EV3AnalogPort.getPortType(port);
  }
  ...
  public int getDeviceType() {
    if (typ == EV3Port.MOTOR_PORT) return EV3AnalogPort.getMotorType(port);
    else return EV3AnalogPort.getAnalogSensorType(port);
  }

  /**
   * Set the basic operating mode of the various sensor pins to allow correct operation of the
   * attached sensor.
   *
