...
public class EV3LCD implements CommonLCD {
  ...
  protected static final int SCREEN_MEM_WIDTH =
      (SCREEN_WIDTH + 7) / 8; // width of leJOS screen buffer in bytes
  protected static final int LCD_BUFFER_LENGTH = SCREEN_MEM_WIDTH * SCREEN_HEIGHT;
      ...
      while (ycnt-- > 0) {
        ...
        while (cnt-- > 0) {
          if (noDst) dst[outIndex] = (byte) ((src[inIndex] & ca2) ^ cx2);
            ...
            dst[outIndex] =
                (byte) ((dst[outIndex] & ((inVal & ca1) ^ cx1)) ^ ((inVal & ca2) ^ cx2));
          }
          outIndex += xinc;
          inIndex += xinc;
        }
