...
public class EV3Wrapper implements UncaughtExceptionHandler {
  ...
  private static void switchToLayer(String name) {
    ...
    if (name.equals("*")) {
      for (LCDLayer l : layers) l.setVisible(true);
      return;
    }

    // turn off all layers
    for (LCDLayer l : layers) l.setVisible(false);
    // find the layer and enable it
    for (LCDLayer l : layers) if (l.getName().equals(name)) l.setVisible(true);
  }
      ...
      while (true) {
        if (keys.getButtons() == (Keys.ID_LEFT | Keys.ID_RIGHT)) {
          ...
          if (curLayer >= layers.length) {
            ...
            curLayer = -1;
            layerName = "*";
          } else layerName = layers[curLayer].getName();
          // Display the layer name for a short period
          systemLCD.clear();
          systemLCD.drawString(
              layerName, (systemLCD.getTextWidth() - layerName.length() + 1) / 2, 1);
