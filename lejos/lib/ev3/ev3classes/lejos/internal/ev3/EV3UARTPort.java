...
/**
 * Provide access to EV3 sensor ports operating in UART mode.
 *
 * <p>A mechanism is provided to allow access to the RAW UART device allowing simple read/write
 * operations. This RAW mode disables the normal Lego protocol used to communicate with Lego UART
 * sensors
 *
 * <p>NOTE: This code is not pretty! The interface uses a number of structures mapped into memory
 * from the device. I am not aware of any clean way to implement this interface in Java. So for now
 * multiple pointers to bytes/ints array etc. are used this means that the actual offsets of the
 * start of the C arrays needs to be obtained and these (along with various sizes) are currently
 * hard-coded as "OFF" values below. I'm sure there must be a better way! Also note that there seem
 * to be a large number of potential race conditions in the device initialisation stage hence the
...
public class EV3UARTPort extends EV3IOPort implements UARTPort {
  ...
  public static class UARTCTL extends Structure {
    ...
    protected List getFieldOrder() {
      // TODO Auto-generated method stub
      return Arrays.asList(new String[] {"TypeData", "Port", "Mode"});
    }
  }
  ...
  public static class UARTCONFIG extends Structure {
    ...
    protected List getFieldOrder() {
      // TODO Auto-generated method stub
      return Arrays.asList(new String[] {"Port", "BitRate"});
    }
  }
  ...
  protected boolean getModeInfo(int mode, UARTCTL uc) {
    ...
    uc.Mode = (byte) mode;
    uc.write();
    // System.out.println("size is " + uc.size() + " TYPES " + uc.TypeData.size() + " ptr " +
    // uc.getPointer().SIZE);
  ...
  /**
   * Clear the flag that indicates the mode info has been cached. This allows us to read the same
   * infomration again later without having to reset the device.
   *
   * @param mode mode number to read
   * @param uc control structure to read the data into
   * @return
  ...
  protected boolean readModeInfo() {
    ...
    for (int i = 0; i < UART_MAX_MODES; i++) {
      ...
      if (getModeInfo(i, uc)) {
        ...
        modeCnt++;
      } else modeInfo[i] = null;
    }
    // System.out.println("Got " + modeCnt + " entries time " + (System.currentTimeMillis() -
    // base));
    return modeCnt > 0;
  }
  ...
  protected int initSensor(int mode) {
    ...
    // System.out.println("Init complete retry " + retryCnt + " time " + (System.currentTimeMillis()
    // - base));
    if ((status & UART_DATA_READY) != 0 && (status & UART_PORT_CHANGED) == 0) return 1;
    else return 0;
  }
  ...
  public boolean setMode(int mode) {
    ...
    // are we initialised ?
    if (modeCnt <= 0) return initialiseSensor(mode);
    if (modeInfo[mode] == null) return false;
  ...
  /**
   * The RAW data is held in a circular buffer with 32 bytes of data per entry and 300 entries per
   * port. This method calculates the byte offset of the latest data value read into the buffer.
   *
  ...
  /**
   * Wait for a new data point to be added to the data set. Return true if new data is available,
   * false if not
   *
   * @param timeout
  ...
  /**
   * Check the sensor status, and if possible recover any from any error. If everything fails throw
   * an exception
   */
  protected void checkSensor() {
    if ((getStatus() & UART_PORT_CHANGED) != 0) {
      ...
      if (!initialiseSensor(getMode())) throw new DeviceException("Sensor changed unable to reset");
    }
  }
  ...
  public void getBytes(byte[] vals, int offset, int len) {
    ...
    for (int i = 0; i < len; i++) vals[i + offset] = raw.get(loc + i);
  }
  ...
  public void getShorts(short[] vals, int offset, int len) {
    checkSensor();
    int loc = calcRawOffset();
    for (int i = 0; i < len; i++) vals[i + offset] = raw.getShort(loc + i * 2);
  }
   ...
   * <p>
   *
   * <p>TODO: Make other mode data available.
   *
  ...
  public String getModeName(int mode) {
    if (modeInfo[mode] != null) {
      ...
      int len = name.length;
      for (int i = 0; i < name.length; i++) if (name[i] == 0) len = i;
  ...
  public String toString() {
    ...
    switch (info.Format) {
      case 0:
        if (info.RawMin >= 0) val = getByte() & 0xff;
        else val = getByte();
        break;
      case 1:
        if (info.RawMin >= 0) val = getShort() & 0xffff;
        else val = getShort();
        break;
   ...
   *
   * <p>Note: The port must have been set into RAW mode to use this method.
   *
  ...
  public int rawRead(byte[] buffer, int offset, int len) {
    if (rawInput == null) rawInput = new byte[RAW_BUFFER_SIZE + 2];
    rawInput[0] = (byte) port;
    if (len > RAW_BUFFER_SIZE) len = RAW_BUFFER_SIZE;
    rawInput[1] = (byte) len;
  ...
  /**
   * Attempt to write a series of bytes to the uart port. This call is non-blocking if there is no
   * space in the write buffer a count of 0 is returned.
   *
   * <p>Note: The port must have been set into RAW mode before attempting to use the method.
   *
  ...
  public int rawWrite(byte[] buffer, int offset, int len) {
    if (rawInput == null) rawOutput = new byte[RAW_BUFFER_SIZE + 2];
