 ...
 * Each layer can be used as the frame buffer for text/graphics output. Layers can be made visible
 * or invisible. If more than one layer is visible the layers will be combined and will overlay each
 * other on the display. Layers are named and this name can be used to access the layer.
 *
 * @author andy
 */
public class EV3LCDManager {
  public static final int DEFAULT_REFRESH_PERIOD = 250;
  protected static final int HW_MEM_WIDTH =
      ((EV3LCD.SCREEN_WIDTH + 31) / 32) * 4; // width of HW Buffer in bytes
  protected static final int LCD_HW_BUFFER_LENGTH = HW_MEM_WIDTH * EV3LCD.SCREEN_HEIGHT;
  ...
  public static class LCDLayer implements Closeable {
    ...
    public synchronized void close() {
      if (openCnt <= 0) throw new IllegalStateException("layer is not open");
      if (openCnt == 1) {
        setVisible(false);
        displayBuf = null;
      }
    ...
    public void run() {
      for (; ; ) {
        ...
        if (now >= refreshTime) {
          ...
          else refreshTime = now + DEFAULT_REFRESH_PERIOD;
        }
  ...
  protected LCDLayer findLayer(String name) {
    for (LCDLayer l : layers) if (l.getName().equals(name)) return l;
    return null;
  }
  ...
  public synchronized LCDLayer getLayer(String name) {
    ...
    if (l == null) throw new IllegalArgumentException("Layer does not exist " + name);
    return l;
  }
  ...
  public synchronized LCDLayer newLayer(String name) {
    if (findLayer(name) != null) throw new IllegalArgumentException("Layer already exists " + name);
  ...
  protected synchronized void buildVisibleList() {
    ...
    int visCnt = 0;
    for (LCDLayer l : layers) if (l.visible && l.displayBuf != null) visCnt++;
      ...
      for (int i = 1; i < vis.length; i++)
        for (int row = 0; row < EV3LCD.SCREEN_HEIGHT; row++)
          for (int col = 0; col < EV3LCD.SCREEN_MEM_WIDTH; col++)
            hwBuffer[row * HW_MEM_WIDTH + col] |=
                vis[i].displayBuf[row * EV3LCD.SCREEN_MEM_WIDTH + col];
      lcd.write(0, hwBuffer, 0, hwBuffer.length);
    } else {
      // nothing to display, clear the screen
      for (int i = 0; i < hwBuffer.length; i++) hwBuffer[i] = 0;
    }
    lcd.write(0, hwBuffer, 0, hwBuffer.length);
  ...
  protected void closeAll() {
    for (LCDLayer l : layers) while (l.isOpen()) l.close();
  }
