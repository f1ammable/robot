...
public class EV3AnalogPort extends EV3IOPort implements AnalogPort {
  ...
  protected void getRawValues() {
    ...
    for (int i = 0; i < rawValues.length; i++) rawValues[i] = shortVals.getShort(first + i * 2);
  }

  /**
   * This method accepts a set of raw values (in full color mode) and processes them using the
   * calibration data to return standard RGB values between 0 and 255
   *
  ...
  private void calibrate(short[] vals) {
    ...
    if (blankVal < calLimits[1]) calTab = 2;
    else if (blankVal < calLimits[0]) calTab = 1;
    else calTab = 0;
    ...
    if (blankVal > MINBLANKVAL) blankVal -= MINBLANKVAL;
    else blankVal = 0;
    blankVal = (blankVal * 100) / (((SENSORMAX - MINBLANKVAL) * 100) / ADMAX);
    if (blankVal > SENSORRESOLUTION) blankVal = SENSORRESOLUTION;
    vals[BLANK_INDEX] = (short) ((blankVal * calData[calTab][BLANK_INDEX]) >>> 16);
  }

  /**
   * Return a single processed value. If in single color mode this returns a single reading as a
   * percentage. If in full color mode it returns a Lego color value that identifies the color of
   * the object in view.
   *
  ...
  protected short getColor() {
    ...
    if (red > blue && red > green) {
      // red dominant color
      if (red < 65 || (blank < 40 && red < 110)) return BLACK;
      if (((blue >> 2) + (blue >> 3) + blue < green) && ((green << 1) > red)) return YELLOW;
      if ((green << 1) - (green >> 2) < red) return RED;
      if (blue < 70 || green < 70 || (blank < 140 && red < 140)) return BLACK;
      return WHITE;
    } else if (green > blue) {
      // green dominant color
      if (green < 40 || (blank < 30 && green < 70)) return BLACK;
      if ((blue << 1) < red) return YELLOW;
      if ((red + (red >> 2)) < green || (blue + (blue >> 2)) < green) return GREEN;
      if (red < 70 || blue < 70 || (blank < 140 && green < 140)) return BLACK;
      return WHITE;
    } else {
      // blue dominant color
      if (blue < 48 || (blank < 25 && blue < 85)) return BLACK;
      if ((((red * 48) >> 5) < blue && ((green * 48) >> 5) < blue)
          || ((red * 58) >> 5) < blue
          || ((green * 58) >> 5) < blue) return BLUE;
      if (red < 60 || green < 60 || (blank < 110 && blue < 120)) return BLACK;
      if ((red + (red >> 3)) < blue || (green + (green >> 3)) < blue) return BLUE;
      return WHITE;
    }
  }
  ...
  public void getFloats(float[] vals, int offset, int length) {
    if (length > 0) {
      getRawValues();
      int cnt = length;
      if (cnt > rawValues.length) cnt = rawValues.length;
      for (int i = 0; i < cnt; i++) vals[offset + i] = (float) rawValues[i] * ADC_REF / ADC_RES;
      offset += cnt;
      length -= cnt;
      if (length > 0) {
  ...
  public boolean setType(int type) {
    ...
    while (curOffset == getCurrentOffset()) Delay.msDelay(1);
    return ret;
  }
  ...
  protected static int getPortType(int port) {
    if (port > PORTS || port < 0) return CONN_ERROR;
  ...
  protected static int getMotorPortType(int port) {
    if (port > MOTORS || port < 0) return CONN_ERROR;
