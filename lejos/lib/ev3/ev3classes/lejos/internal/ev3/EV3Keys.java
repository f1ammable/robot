...
public class EV3Keys implements Keys {
  ...
  public int getButtons() {
    ...
    for (; ; ) {
      state1 = checkButtons();
      if (state1 == state2) return state1 | simulatedState;
  ...
  private int checkButtons() {
    int state = 0;
    for (int i = 0; i < buttonState.capacity(); i++) if (buttonState.get(i) != 0) state |= 1 << i;
    return state;
  }
    ...
    public void run() {
      while (true) {
        ...
        for (int i = 0; i < NUM_KEYS; i++) {
          if ((state & (mask << PRESS_EVENT_SHIFT)) != 0
              || (state & (mask << RELEASE_EVENT_SHIFT)) != 0) {
            ;
