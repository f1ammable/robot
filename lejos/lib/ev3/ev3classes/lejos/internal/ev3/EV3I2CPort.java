 ...
 * NOTE: The EV3 iic kernel module provides the capability to make an i2c sensor have a similar
 * interface to that used for uart based sensors. In particular it provides a mechanism to have the
 * kernel poll the sensor. However this mode seems to be of limited use because most i2c sensors
 * provide multiple data values etc. Because of this we only implement the basic i2c interface.
 */
public class EV3I2CPort extends EV3IOPort implements I2CPort {
  ...
  public boolean open(int t, int p, EV3Port r) {
    if (!super.open(t, p, r)) return false;
  ...
  /**
   * High level i2c interface. Perform a complete i2c transaction and return the results. Writes the
   * specified data to the device and then reads the requested bytes from it.
   *
   ...
   */
  public synchronized void i2cTransaction(
      int deviceAddress,
      byte[] writeBuf,
      int writeOffset,
      int writeLen,
    ...
    if (result == STATUS_FAIL) throw new I2CException("I2C I/O error");
    if (result == STATUS_BUSY) throw new I2CException("I2C Bus busy");
    if (result == STATUS_OK) {
      // System.out.println("iic time " + (System.currentTimeMillis() - st));
      if (readLen > 0) System.arraycopy(cmd, 5, readBuf, readOffset, readLen);
      return;
    }
