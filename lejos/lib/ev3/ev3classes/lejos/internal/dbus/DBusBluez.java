...
public class DBusBluez {
  ...
  public boolean authenticateRemoteDevice(String deviceAddress, final String passkey)
      throws DBusException {

    String agentPath =
        "/org/lejos/authenticate/" + getAdapterID() + "/" + deviceAddress.replace(':', '_');

    dbusConn.exportObject(agentPath, new PinAgent(passkey));

    // System.out.println("Calling CreatedPairedDevive on " + deviceAddress + " using agent: " +
    // agentPath);
    try {
      ...
      for (Path devicePath : devices) {
        ...
        try {
          ...
          if (properties != null) {
            ...
            boolean paired =
                DBusProperties.getBooleanValue(properties, Device.Properties.Paired, false);
            boolean trusted =
                DBusProperties.getBooleanValue(properties, Device.Properties.Trusted, false);
