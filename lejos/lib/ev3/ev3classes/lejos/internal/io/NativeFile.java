...
/**
 * This class provides access to Linux files using native I/O operations. It is implemented using
 * the JNA package. The class is required because certain operations (like ioctl) that are required
 * by the Lego kernel module interface are not support by standard Java methods. In addition
 * standard Java memory mapped files do not seem to function correctly when used with Linux
 * character devices.
 *
 * <p>
 *
 * <p>Where possible standard Java classes and methods are used. JNA is used to extended the
 * standard Java interface as required.
 *
 * @author andy
 */
public class NativeFile {
  ...
  static class Linux_C_lib_DirectMapping {
    ...
    public native int ioctl(int fd, int cmd, byte[] arg);

    public native int ioctl(int fd, int cmd, Pointer p);

    public native int open(String path, int flags);

    public native int close(int fd);

    public native int write(int fd, Buffer buffer, int count);

    public native int read(int fd, Buffer buffer, int count);

    public native Pointer mmap(
        Pointer addr, NativeLong len, int prot, int flags, int fd, NativeLong off);
  ...
  /**
   * Map a portion of the associated file into memory and return a pointer that can be used to
   * access that memory.
   *
  ...
  public Pointer mmap(long len, int prot, int flags, long off) {
    Pointer p =
        clib.mmap(new Pointer(0), new NativeLong(len), prot, flags, fd, new NativeLong(off));
