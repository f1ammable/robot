...
public class RemoteRequestPort implements Port {
  ...
  public RemoteRequestPort(
      String name, int typ, int portNum, ObjectInputStream is, ObjectOutputStream os) {
    this.name = name;
    this.typ = typ;
  ...
  public <T extends IOPort> T open(Class<T> portclass) {
    ...
    switch (typ) {
      case SENSOR_PORT:
        ...
        break;
      case MOTOR_PORT:
        if (portclass == BasicMotorPort.class) p = new RemoteRequestMotorPort(is, os);
        else if (portclass == TachoMotorPort.class) p = new RemoteRequestMotorPort(is, os);
