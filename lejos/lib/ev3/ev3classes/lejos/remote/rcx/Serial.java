...
public class Serial {
  ...
  /**
   * Read an assembled packet. NRLink only seems to read one byte at a time reliably, and does not
   * return zero bytes.
   *
  ...
  public static boolean isPacketAvailable() {
    ...
    if (available > 1) {
      ...
      // LCD.refresh();
      if (complement(buf1[0], checkDigit) && (checkDigit & 0xFF) == (checkSum & 0xFF)) {
        gotPacket = true;
      } else {
