...
/**
 * RCXAbstractPort provides an interface similar to java.net.Socket Adapted from original code
 * created by the LEGO3 Team at DTU-IAU RCXAbstractPort implements input and output stream handling
 * and input buffering. It uses a packet handler for sending and receivng packets. This version is
 * abstract because it has no packet handler defined. Specific versions of RCXAbstractPort override
 * the constructor and set up the packet handler to use a specific protocol stack.
 *
...
public abstract class RCXAbstractPort {
  ...
  /**
   * Listener class runs a thread that reads and buffers bytes. Allows a maximum of two bytes in a
   * packet.
   */
  ...
  private class RCXInputStream extends InputStream {
    ...
    public int available() throws IOException {
      if (last < current) return bufferSize - (current - last);
      else return last - current;
    }
    ...
    public synchronized int read() throws IOException {
      ...
      synchronized (buffer) {
        ...
        if (current == bufferSize) current = 0;
    ...
    /**
     * Add a data byte to the stream This method should only be called by the RCXPort that created
     * the RCXInputStream
     *
