...
/**
 * Sends LCP requests to the NXT and receives replies. Uses an object that implements the NXTComm
...
public class NXTCommand implements NXTProtocol {
  ...
  private byte sendRequest(byte[] request, int replyLen) throws IOException {
    if (!open) throw new IOException("NXTCommand is closed");

    byte verify = 0; // default of 0 means success
    if (verifyCommand) request[0] = DIRECT_COMMAND_REPLY;

    byte[] reply =
        nxtComm.sendRequest(request, (request[0] == DIRECT_COMMAND_REPLY ? replyLen : 0));
  ...
  private byte sendSystemRequest(byte[] request, int replyLen) throws IOException {
    ...
    byte verify = 0; // default of 0 means success
    if (verifyCommand) request[0] = SYSTEM_COMMAND_REPLY;

    byte[] reply =
        nxtComm.sendRequest(request, (request[0] == SYSTEM_COMMAND_REPLY ? replyLen : 0));
  ...
  public FileInfo openRead(String fileName) throws IOException {
    ...
    if (reply[2] != ErrorMessages.SUCCESS) throw new LCPException(reply[2]);
   ...
   *
   * @param handle Handle number from the previous found file or from the Find First command.
   * @return fileInfo object giving details of the file
   */
   ...
   * off/on before you run another program.
   *
  ...
  /**
   * Put the NXT into SAMBA mode, ready to update the firmware. Marks this NXTCommand object as
   * closed. Does never never wait for a reply from the NXT.
   *
   * @throws IOException
   */
  ...
  public int readFile(byte handle, byte[] data, int offset, int length) throws IOException {
    ...
    while (remaining > 0) {
      ...
      byte[] request = {
        SYSTEM_COMMAND_REPLY, READ, handle, (byte) chunkLen, (byte) (chunkLen >>> 8)
      };
   ...
   * @return the message as an array of bytes, excluding the trailing null-terminator or null when
   *     queue is empty
   */
  public byte[] messageRead(byte remoteInbox, byte localInbox, boolean remove) throws IOException {
    byte[] request = {
      DIRECT_COMMAND_REPLY, MESSAGE_READ, remoteInbox, localInbox, (remove ? (byte) 1 : (byte) 0)
    };
    byte[] reply = nxtComm.sendRequest(request, 64);
    if (reply[2] == ErrorMessages.SPECIFIED_MAILBOX_QUEUE_IS_EMPTY) return null;
  ...
  private void checkStatusByte(byte[] reply) throws LCPException {
    byte code = reply[2];
    if (code != 0) throw new LCPException(code);
  }

  /**
   * Sends a message to an inbox on the NXT for storage(?) For future reference, message size must
   * be capped at 59 for USB. A null terminator is automatically appended and should not be included
   * in the message. UNTESTED
   *
   ...
   * @param mode - Setting the modes MOTORON, BRAKE, and/or REGULATED. This parameter is a bitfield,
   *     so to put it in brake mode and regulated, use BRAKEMODE + REGULATED
  ...
  public DeviceInfo getDeviceInfo() throws IOException {
    ...
    d.bluetoothAddress =
        Integer.toHexString(reply[18])
            + ":"
            + Integer.toHexString(reply[19])
            + ":"
            + Integer.toHexString(reply[20])
            + ":"
            + Integer.toHexString(reply[21])
            + ":"
            + Integer.toHexString(reply[22])
            + ":"
            + Integer.toHexString(reply[23])
            + ":"
  ...
  /**
   * Get the fimrware version. leJOS NXJ returns the version of the LEGO firmware that it emulates,
