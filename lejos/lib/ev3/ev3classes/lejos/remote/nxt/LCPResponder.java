...
public class LCPResponder extends Thread {
  ...
  /**
   * Create a Responder using the provided connector The connector is used to create the listening
   * connection used to accept remote commands.
   *
   * @param connector
   */
  ...
  /**
   * Method called when the responder is waiting for a new connection. Default action is to wait for
  ...
  /**
   * Method called to disconnect the responder connect. Default action is to close the underlying
   * connection object.
   */
  ...
  /**
   * Method called with a newly read command, before it is processed. Default action is to detect
   * invalid commands and if detected to drop the connection.
   *
  ...
  protected int preCommand(byte[] inMsg, int len) {
    if (len < 0 || inMsg[1] == LCP.START_PROGRAM) disconnect();
    return len;
  }
  ...
  protected void postCommand(byte[] inMsg, int inLen, byte[] replyMsg, int replyLen) {
    if (inMsg[1] == LCP.NXJ_DISCONNECT) disconnect();
  }
  ...
  /**
   * Terminate the responder. Abort any listening operation and close any open connections (this
   * will also abort any current read requests).
   */
