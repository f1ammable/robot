...
public class LCP {
  ...
  public static int emulateCommand(byte[] cmd, int cmdLen, byte[] reply) {
    ...
    reply[1] = cmdId;
    for (int i = 2; i < reply.length; i++) reply[i] = 0;
      ...
      case GET_DEVICE_INFO:
        {
          ...
          // byte [] address =
          // NXTCommDevice.stringToAddress(Bluetooth.getLocalDevice().getBluetoothAddress());
      ...
      case SET_OUTPUT_STATE:
        {
          ...
          for (int i = 0; i < 3; i++) {
            ...
            if (motorid == 0 || (motorid < 0 && i == 0)) m = Motor.A;
            else if (motorid == 1 || (motorid < 0 && i == 1)) m = Motor.B;
            else if (motorid == 2 || (motorid < 0 && i == 2)) m = Motor.C;
            ...
            if (tacholimit != 0) m.rotate(tacholimit, true); // Returns immediately
      ...
      case FIND_FIRST:
        {
          ...
          if (numFiles == 0) {
            ...
            for (int i = 0; i < fileNames[0].length(); i++)
              reply[4 + i] = (byte) fileNames[0].charAt(i);
            fileIdx = 1;
      ...
      case FIND_NEXT:
        {
          ...
          if (fileNames == null || fileIdx >= fileNames.length)
            ...
            for (int i = 0; i < fileNames[fileIdx].length(); i++)
              reply[4 + i] = (byte) fileNames[fileIdx].charAt(i);
            ...
            for (int i = 0; i < fileNames.length; i++) {
              if (fileName.equals(fileNames[i])) {
                ...
                for (int j = 0; j < fileName.length(); j++)
                  reply[j + 3] = (byte) fileName.charAt(i);
                deleted = true;
                fileNames = new String[--numFiles];
                for (int j = 0; j < numFiles; j++) fileNames[j] = files[j].getName();
                break;
      ...
      case NXJ_GET_DEFAULT_PROGRAM:
        {
          ...
          for (int i = 0; i < name.length() && i < 20; i++) reply[3 + i] = (byte) name.charAt(i);
          len = 23;
          break;
        }
      default:
