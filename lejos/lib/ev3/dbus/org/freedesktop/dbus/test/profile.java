...
public class profile {
  ...
  public static class Log {
    ...
    public double mean() {
      ...
      long sum = 0;
      for (int i = 0; i < current; i++) sum += deltas[i];
      return sum /= current;
    }

    public long min() {
      int m = Integer.MAX_VALUE;
      for (int i = 0; i < current; i++) if (deltas[i] < m) m = deltas[i];
      return m;
    }
    ...
    public double stddev() {
      double mean = mean();
      double sum = 0;
      for (int i = 0; i < current; i++) sum += (deltas[i] - mean) * (deltas[i] - mean);
  ...
  public static void main(String[] args) {
    try {
      if (0 == args.length) {
        ...
        System.out.println(
            "Syntax: profile"
                + " <pings|arrays|introspect|maps|bytes|lists|structs|signals|rate|strings>");
        System.exit(1);
      }
      ...
      if ("pings".equals(args[0])) {
        ...
        Peer p =
            conn.getRemoteObject("org.freedesktop.DBus.java.profiler", "/Profiler", Peer.class);
        Log l = new Log(count);
        ...
        System.out.print(
            "Sending array of " + STRING_ARRAY_LENGTH + " strings " + count + " times.");
        ProfilerInstance pi = new ProfilerInstance();
        conn.exportObject("/Profiler", pi);
        Profiler p =
            conn.getRemoteObject("org.freedesktop.DBus.java.profiler", "/Profiler", Profiler.class);
        ...
        for (int i = 0; i < MAP_LENGTH; i++) m.put("" + i, "hello");
        Log l = new Log(count);
        long t = System.currentTimeMillis();
        ...
        for (int i = 0; i < LIST_LENGTH; i++) v.add("hello " + i);
        Log l = new Log(count);
        long t = System.currentTimeMillis();
        ...
        Introspectable is =
            conn.getRemoteObject(
                "org.freedesktop.DBus.java.profiler", "/Profiler", Introspectable.class);
        Log l = new Log(count);
        long t = System.currentTimeMillis();
        String s = null;
        ...
        for (int i = 0; i < BYTES; i++) bs[i] = (byte) i;
        long t = System.currentTimeMillis();
        p.bytes(bs);
