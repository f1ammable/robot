...
public class NativeDomainSocket {
  ...
  public static class SockAddr extends Structure implements Structure.ByReference {
    ...
    public SockAddr(String name, boolean abs) {
      // System.out.println("Socket address created with address = " + addr + ", abstract = " +
      // abs);
  ...
  static class Linux_C_lib_DirectMapping {
    ...
    public native int setsockopt(int s, int level, int optname, byte[] optval, int optlen);

    public native int recv(int s, Buffer buf, int len, int flags) throws LastErrorException;

    public native int recvfrom(
        int s, Buffer buf, int len, int flags, SockAddr from, IntByReference fromlen);

    public native int send(int s, Buffer msg, int len, int flags) throws LastErrorException;
