...
/**
 * Filters sound ( like a high pass filter ) so that we do not hear two loud sounds within a certain
 * amount of time
 *
...
public class ClapFilter implements SampleProvider {
  ...
  /**
   * The constructor initialises the time that the last clap was heard to be sufficiently far in the
   * past that a first clap is always returned . The soundMode argument must be a sample provider of
   * an NXTSoundSensor
   *
  ...
  /**
   * The sample is fetched only if we have waited long enough If the sample is fetched and is loud
   * then we start the wait time again from now The level returned is 0 for no clap , and 1 for a
   * clap .
   *
